\documentclass[twoLevelNum]{helloWorld} %%% last changed 2014-08-20
...
\title[Higher Pro-Arrows: Modal System, Semantics and Emerging Syntax of NatpT]{Higher Pro-Arrows: Modal System, Semantics and Emerging Syntax of Naturality Pretype Theory}
...
\keywords{directed type theory, modal type theory, categorical semantics, higher category theory, proarrow equipments, parametricity, naturality, homotopy type theory, transpension}
...
None of these systems is superior to the other: while relations are strictly more general than morphisms, which are in turn more general than equivalences, we lose computational content when we only consider relations.
...
Finally, I discuss how the matter of fibrancy may be addressed in the future.
\end{abstract}
...
\todoi{Put the following in a separate section}
...
Concretely, the objects of the extended context will be pointed categories $(\cat C, c)$, and the morphisms $(F, \vfi)$ will consist of a functor $F : \cat C \to \cat D$ between the categories and a `heterogeneous arrow' $\vfi$ along it, and we have to commit to whether we want these heterogeneous arrows to point the same way as the functor, $\vfi \in \Hom_{\cat D}(Fc, d)$, or the opposite way, $\vfi \in \Hom_{\cat D}(d, Fc)$.
...
A sensible and algebraically relevant semantics for the extended context $\Gamma, \ctxmodnovar \mu x T$ (for a certain \emph{specific} modality $\mu$) is now as the pro-arrow equipment of pointed categories, whose:
\begin{itemize}
	\item objects are pointed categories,
	\item morphisms are point-\emph{preserving} functors,
	\item pro-arrows are dependently pointed profunctors (i.e.\ a pro-arrow $(\catP, \psi) : (\catC, c) \nrightarrow (\catD, d)$ is a profunctor $\catP : \catC \nrightarrow \catD$ with a designated heterogeneous morphism $\psi \in \catP(c, d)$),
	\item squares are point-preserving profunctor morphisms.
\end{itemize}
Each time, things are paired up whose orientation was coupled anyway: the direction of the mapping relation ${\mapsto} : \Obj(\cat C) \times \Obj(\catD) \to \Prop$ of a functor $F : \catC \to \catD$ is inevitably related to the direction of $F$ itself, while the direction of a heterogeneous arrow $\psi \in \catP(c, d)$ along a profunctor $\catP : \catC \nrightarrow \catD$ is also inevitably related to the direction of $\catP$.
The end result is a new equipment in which arrows and pro-arrows can still independently be flipped.

The modality $\mu$ will be the directed version of the \emph{\textbf{structural}} modality in Degrees of Relatedness \cite{reldtt}, by which algebraic structures depend on their structure (e.g.\ pointed categories on their designated point).
...
\begin{enumerate}
	...
	\item Restrict to those pro-arrows whose underlying pro-arrow in $\name{FPro}\,\catC$ actually arises from an arrow in $\catC$, which can be done by taking a pullback along $\catC \to \name{Pro}(\name{FPro}\,\catC)$.
\end{enumerate}
...
However, natural transformations are a rather restrictive concept, while extranatural and dinatural transformations do not always compose.\footnote{The type signature of an extranatural transformation specifies, besides the domain and codomain functors, a string diagram connecting each argument slot of the domain (codomain) functor to either a slot of opposite variance of the same functor or a slot of the same variance of the other functor. Extranatural transformations can be composed insofar as the composition of their string diagrams does not create closed `bubbles' \cite{extranat-compose}. Petri\'{c} provides a similar criterion for dinatural transformations \cite{g-dinat}, which McCusker and Santamaria independently reprove using Petri-nets \cite{composing-dinatural}.}
...
However, if we nest contravariance a bit deeper, strong dinaturality too proves inadequate for a general typing discipline: given functors $F, G, H : \catC \times \catC\op \to \Set$, the function
\[
	\name{comp} : \forall x . (G\,x\,x \to H\,x\,x) \times (F\,x\,x \to G\,x\,x) \to (F\,x\,x \to H\,x\,x) : (g, f) \mapsto g \circ f
\]
is \emph{not} strongly dinatural w.r.t.\ $x$, because the notion of heterogeneous equality used for the domain of $g$ differs from the one used for the codomain of $f$.
...
\todoi{Related work: \cite{vett,laretto-dinaturality}}
...
\todoi{
DirTT less advanced. Several reasons:
\begin{itemize}
	\item Hom-type: Bifibrations (did you mean two-sided?), naturality, parametricity. Mention treatment of modal inductive types. Interval
\end{itemize}
}
...
Andreas Nuyts holds a Postdoctoral Fellowship from the Research Foundation - Flanders (FWO; 1247922N, 12AB225N). This research is partially funded by the Research Fund KU Leuven.
