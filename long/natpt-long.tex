...
\begin{proposition} \label{thm:poresh-jetset-prism-commut}
	...
	\]
\end{proposition}
Note that a cartesian product with $\rival{\eqtysym}$ or $\rival{\topsym}$ is the same as a separated product, since neither jet setoid has any non-reflexive jets.
...
\begin{definition}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item sends $()$ to $\mu () := ()$,
			\item has an action on other cuboids that can be looked up in the following table\footnote{to be compared with the table in \cref{thm:poresh-jetset-prism-commut}.}, where $j = \getdeg(i \cdot \rho)$:%
			\footnote{Note that if $i \cdot \rho = \isopol_j$, then by \cref{eq:poresh-withl-crit2}, $b_j = \npolar$.}
			\[
				\begin{array}{c | l || c | c || c}
					& a_i\,{\rightarrow} & \multicolumn{2}{c ||}{\ypolar} & \npolar \\ \hline
