...
\subsection{Intervals and Prisms}
\begin{definition} \label{def:jetset-eqty}
	We define $\rival{\edgesym_\eqtydeg}$ as the singleton jet set.
	
	Under the strict quotienting \cref{ax:bishop}, it is relevant to specify that we take the singleton $\accol{0, 1}/\edgesym_\top$, a quotient by the total relation.
\end{definition}
\begin{proposition} \label{thm:poresh-jetset-intervals}
	...
	\footnote{By virtue of \cref{thm:poresh-adjoint}, all right hand sides are defined since $P_i \oriof \rho \neq \infrajetsym_j$ for $b_j = \ypolar$, and $P_i \oriof \rho \neq \edgesym_\top$.}
	\begin{equation}
		\jetset{\mu} \rival{\jetsym_i} \cong \rival{\jetsym_i \oriof \rho}, \quad
		\jetset{\mu} \rival{\tejsym_i} \cong \rival{\tejsym_i \oriof \rho}, \quad
		\jetset{\mu} \rival{\equijetsym_i} \cong \rival{\equijetsym_i \oriof \rho}.
		\label{eq:poresh-jetset-intervals}
	\end{equation}
	Under the strict quotienting \cref{ax:bishop}, these isomorphisms are equalities.
\end{proposition}
\begin{proof}
	Let $P \in \accol{\jetsym, \tejsym, \equijetsym}$.
	...
	\end{enumerate}
\end{proof}
\begin{lemma} \label{thm:poresh-jetset-cocontinuous}
	Let $\mu : \vec a \to \vec b$ be a polarized reshuffle.
	Then $\jetset{\mu}$ preserves colimits.
\end{lemma}
\todoi{This is FALSE: the colimit of $\accol{0 \jet_i 1} \supseteq \accol{0 \quad 1} \subseteq \accol{0 \tej_i 1}$ is $\accol{0 \equijet_i 1}$, and so is not preserved by $\mixpol_i$.}
\begin{proof}
	Take a diagram $D : \cat S \to \jetsetoid{\vec a}$.
	We prove that
	\[
		\theta : \mu \colim D \cong \colim (\mu \circ D).
	\]
	\Cref{thm:jetset:colim} tells us how to construct each side of the isomorphism.
	For elements of the carrier, we use a pair notation, i.e. $\iota_s(d) = \angles{s, d}$.
	
	The morphism to the left exists by the universal property of the colimit,
	with $\theta\inv \circ \iota^{\mu D}_s = \mu \iota^D_s : \mu\,D\,s \to \mu \colim D$.
	For the carrier, this means $\theta\inv\angles{s, [d]_\mu} = [\angles{s, d}]_\mu$.
	We need to show that $\theta$ is invertible.
