...
\newcommand{\thetitle}{Naturality Pretype Theory: \\ Technical Report \\ v0.1.1}
...
\begin{definition}\label{def:orisymbol}
	...
	We call a symbol $P_i$ \textbf{symmetric} if $\tejsym \cdot P_i = P_i$.
...
\begin{definition}\label{def:poreshcat}
	...
	Sometimes, we need to consider only reshuffles that have at least a certain number of left/right adjoints. The 2-posets that contain only those reshuffles (with the same order relation) will be denoted as $\poreshcatllr$, where the number of white bullets on the left/right is the minimal number of left/right adjoints.
\end{definition}
\begin{corollary}
...
\subsubsection{Adjunctions of Polarized Reshuffles}
...
\begin{proof}
	\begin{enumerate}
		...
		To see the former, we need to show that for all $P_i \in \orisymbol(\vec a)$, we have $P_i \leq (P_i \cdot \rho) \cdot \lambda$ which is equivalent to $P_i \cdot \rho \leq P_i \cdot \rho$ and hence true.
		
		To see the latter, we need to show that for all $Q_j$, we have $(Q_j \cdot \lambda) \cdot \rho \leq Q_j$ which is equivalent to $Q_j \cdot \lambda \leq Q_j \cdot \lambda$ and hence true.
...
\begin{example}
	...
	\[
		\Fsymresh \dashv \Usymresh \dashv \Csymresh
	\]
	where $\Usymresh : \vec a \to \vec b$.
	Each of these preserves all $\jetsym_j$ for $j \neq i$ as well as $\eqty$. Furthermore, we have
	\[
		\edgesym_i \cdot \Fsymresh = \infrajetsym_i, \qquad
		\jetsym_i \cdot \Usymresh = \edgesym_i, \qquad
		\edgesym_i \cdot \Csymresh = \equijetsym_i.
	\]
	From \cref{thm:poresh-adjoint}, we conclude that no further left or right adjoints exist. We have
	\[
		\Fsymresh \circ \Usymresh = \id, \qquad \Csymresh \circ \Usymresh = \id.
	\]
	Thus, we get an idempotent monad $\UFsymresh = \Usymresh \circ \Fsymresh$ and an idempotent comonad $\UCsymresh = \Usymresh \circ \Fsymresh$ such that $\UFsymresh \dashv \UCsymresh$.
	Both preserve all $\jetsym_j$ for $j \neq i$ as well as $\eqty$. Furthermore, we have
	\[
		\jetsym_i \cdot \UFsymresh = \infrajetsym_i, \qquad
		\jetsym_i \cdot \UCsymresh = \equijetsym_i.
	\]
\end{example}
\begin{example}
	Let $\vec b$ be obtained from $\vec a$ by deleting the anpolarity with degree $i$. Let $n := \lenpol{\vec a} = \lenpol{\vec b} + 1$.
	\[
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\begin{itemize}
				...
				\item If $a_i = \npolar$, then
				\[
					\cohfget_i = \cohpi_{i+1} \dashv \cohcodisc_i = \cohdisc_{i+1} \dashv \cohcopi_i = \cohfget_{i+1}, \qquad
					\sharp_i = \shp_{i+1} \dashv \coshp_i = \flat_{i+1},
				\]
				and we can continue.
			\end{itemize}
		\end{itemize}
