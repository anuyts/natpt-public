...
\begin{definition} \label{def:interval}
	...
	\begin{itemize}
		...
		\item The \textbf{opposite $i$-jet interval} $\rival{\jet_i}$ is defined as the jet set of mode $\vec a$ with carrier $\accol{0, 1}$ and relations generated by $0 \tej_ 1$.
...
\begin{definition} \label{def:prism}
	...
	We define the \textbf{opposite $i$-twisted prism $X \multip \rival{\tej_i}$} on $X$ as the jet set of mode $\vec a$ with
	\begin{itemize}
		\item carrier $X \times \accol{0, 1}$,
		\item jet relations generated by the following requirements:
		\begin{itemize}
	...
	We have $\loch \multip \rival{\tej_i} = \Op_i(\loch \multip \rival{\jet_i})$.
...
\begin{definition} \label{def:jet-cube}
	...
	More generally, we define the set of \textbf{opposable jet cubes of mode $\vec a$} as the set of lists of elements of $\set{\jet_i, \tej_i}{a_i = \ypolar} \cup \set{\edge_i}{a_i = \npolar}$, but we denote them as
...
\begin{definition} \label{def:jet-cube-erasure}
	We define the \textbf{jet-erasure function} $\erasejc{\loch}$, which sends (opposable) jet cubes of any mode to cubes (i.e. objects of any of the cube categories defined in \cref{sec:cubes}), by
	\[
		\erasejc{()} = (),
		\qquad \qquad
...
\begin{definition} \label{def:jet-cube-cat}
	...
	We define the category $\jetcubeopt{\eps}{M}{\vec a}$ (or $\ojetcubeopt{\eps}{M}{\vec a}$) of \textbf{(opposable) affine/cartesian jet $M$-cubes of mode $\vec a$} as the full subcategory of $\cubecat^\eps_M \times_\Set \jetset{\vec a}$ on (opposable) jet cubes, as justified by \cref{thm:jet-cube-pullback}.
\end{definition}
\todoi{TODO}
%\begin{definition} \label{def:jet-cube-cat}
%	Let $\vec a$ be a mode and $\eps \in \accol{\affine, \cartes}$.
...
Our calculus will consider morphisms in a larger full subcategory:
\begin{definition} \label{def:polarized-jet-cube-cat}
	The \textbf{category of polarized jet cubes} $\ojetcubeopt{\eps}{M}{\vec a}$ is defined as the full subcategory of $\cubecat^\eps_M \times_{\Set} \jetset{\vec a}$ on objects generated by the terminal jet cube $()$, the twisted prism functors (\cref{def:prism-cube}) and the opposite endofunctors $\Op_i := (\Id, \Op_i)$ on $\cubecat^\eps_M \times_{\Set} \jetset{\vec a}$.
\end{definition}
...
\end{proof}
\begin{corollary}
	Objects of $\ojetcubeopt{\eps}{M}{\vec a}$ are given by the grammar
	\[
...
\begin{remark}
	...
	\end{itemize}
\end{remark}
\todoi{Define $\shp_i$ and $\shp_i^\ypolar$}
