...
\begin{theorem}\label{thm:poresh-adjoint}
	...
	\todoi{Old stuff}
	\begin{itemize}
		\item A reshuffle $\rho$ has a left adjoint $\lambda$ if and only if $(\eqty \cdot \rho) = (\eqty)$, where $Q_j \cdot \lambda$ is the greatest symbol $P_i \in \orisymbol(\vec a)$ such that $P_i \cdot \rho \leq Q_j$ (even when $Q_j \not\in \orisymbolfwd(\vec b)$).
		\item A reshuffle $L$ has a right adjoint $R$ if and only if $j \cdot L < \top$ for all $j < \top$, where $i \cdot R$ is the least index $j \in \accol{\eqty, 0, \ldots, m, \top}$ such that $i \leq j \cdot L$ (even when $i = \top$).
	\end{itemize}
\end{theorem}
\begin{proof}
	\begin{enumerate}
		...
		\begin{itemize}
			...
			\begin{align*}
				...
				&= \max \bigcup \left\{
					...
					\end{array}
				\right\} \\
				& \qquad \qquad \text{(Add sets bounded from above by first set.)} \\
				...
				&= \max \set{P_i \in \orisymbol(\vec a)}{\jetsym_i \cdot \rho \leq \infrajetsym_j} \\
				&= \ell(\infrajetsym_j).
			\end{align*}
		\end{itemize}
		...
		\todoi{Old stuff}
		The equivalence is unsatisfiable if $(\eqty \cdot R) \neq (\eqty)$, because then $(\eqty \cdot L)$ is undefined.
		
		It is also unsatisfiable if $m \cdot L = \top$ or $(\eqty \cdot L) = \top$ because then we get $\top \cdot R < \top$ which contradicts the definition $\top \cdot R := \top$.
	\end{enumerate}
\end{proof}
