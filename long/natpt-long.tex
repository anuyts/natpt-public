...
The more complex but better behaved family of base categories are categories of \textbf{jet cubes} $\jetcubeopt \eps M {\vec a}$ (\cref{sec:jetcubes}).
...
\todoi{I had some doubt about usability of jet jewels, but I think it's okay.
...
\begin{definition} \label{def:jet-jewel}
	...
	\begin{itemize}
		...
		\item for every $0 \leq i < n$, the relation $\jet_i^*$ is a total pre-order on any connected component of $X$, i.e. if $x \infrajet_i^* y$, then $x \jet_i^* y$ or $x \tej_i^* y$.
	\end{itemize}
	The \textbf{category of jet jewels} $\jetjewel{\vec a}$ is defined as the full subcategory of $\jetset{\vec a}$ on jet jewels.
\end{definition}
Note that the second condition is vacuous if $a_i = \npolar$.
\begin{corollary} \label{thm:lift-to-jet-jewels}
	Jet jewels are closed under the functors $\Op_i, \Fsym_i, \Usym_i, \Csym_i, \loch \multip \rival{\jet_i}$ and $\loch \multip \rival{\tej_i}$. \qed
\end{corollary}
...
\begin{proposition} \label{thm:monad-coercions-faithful}
	The obvious functor $I : \cubecat_M^\delta \to \cubecat_N^\eps$ where
	\begin{itemize}
		\item $\delta, \eps \in \accol{\affine, \cartes}$ and $\delta \leq \eps$ according to the order $\affine \leq \cartes$,
...
\begin{corollary} \label{thm:monads-faithful}
	The functor $\EP : \cubecat^\eps_M \to \Set$ is faithful for each $M \in \accol{\bipointed, \bipointedsym, \distlattice, \booleanalg}$.
\end{corollary}
\begin{proof}
...
\begin{definition} \label{def:jet-cube-cat}
	...
	the category $\trijetcubeopt{\eps}{M}{\vec a}$ of \textbf{trioriented $\vec a$-jet-$M$-cubes}
...
\begin{corollary} \label{thm:jet-cubes-are-jewels}
	The functor $\JEP : \trijetcubeopt{\eps}{M}{\vec a} \to \jetset{\vec a}$ (and hence its restriction to forward jet cubes) factors over the inclusion $\jetjewel{\vec a} \hookrightarrow \jetset{\vec a}$.
\end{corollary}
\begin{proof}
	By induction on the dimension and using \cref{thm:lift-to-jet-jewels}, it is clear that for any trioriented jet cube $W$, the jet set $\JEP(W)$ is a jet jewel, which factors the action on objects. The action on morphism factors because the inclusion is full.
\end{proof}
...
\begin{theorem} \label{thm:affine-automatic}
	...
	\[
		\trijetcubeopt \affine M {\vec a} \cong \trijetcubeopt \cartes M {\vec a},
		\qquad \qquad
		\jetcubeopt \affine M {\vec a} \cong \jetcubeopt \cartes M {\vec a},
	\]
...
\begin{proposition}
	The point category (terminal category) is isomorphic to $\jetcubeopt{\eps}{M}{[]}$.
\end{proposition}
\begin{proof}
	It is clear that $()$ is the only object.
	The only endomorphism of $()$ in $\cubecat^\eps_M$ is the identity, and $\erasejc \loch$ is faithful, so there is only one morphism.
\end{proof}
...
\begin{proposition} \label{thm:jetcubes:comparison:bipointedsym}
	The category $\cubecat_\bipointedsym^\affine$ is isomorphic to $\jetcubeopt{\affine}{\bipointedsym}{[\npolar]}$ and $\jetcubeopt{\cartes}{\bipointedsym}{[\npolar]}$.
\end{proposition}
\begin{proof}
	The latter two categories are isomorphic by \cref{thm:affine-automatic}.
	It is clear that $\erasejc{\loch} : \jetcubeopt{\affine}{\bipointedsym}{[\npolar]} \to \cubecat_\bipointedsym^\affine$ is bijective on objects.
...
\begin{proposition} \label{thm:jetcubes:comparison:bipointed}
	The category $\cubecat_\bipointed^\affine$ is isomorphic to $\jetcubeopt{\affine}{\bipointed}{[\npolar]}$ and $\jetcubeopt{\cartes}{\bipointed}{[\npolar]}$.
\end{proposition}
\begin{proof}
...
\begin{itemize}
	\item De Morgan cubes $\cubecat^\cartes_\demorgan$ \cite{cubical} as $\jetcubeopt{\cartes}{\demorgan}{[\npolar]}$,
	\item Cartesian cubes $\cubecat^\cartes_\bipointed$ as $\jetcubeopt{\cartes}{\demorgan}{[\npolar]}$,
	\item Depth $n$ cubes \cite{reldtt,reldtt-techreport} as $\jetcubeopt{\cartes}{\bipointed}{[\npolar]^n}$, where $\vec x^n$ denotes the $n$-fold repetition of the list $\vec x$,
	\item \ldots
\end{itemize}
However, one cannot:
\begin{proposition}
...
\begin{definition}
	The category $\dcubeopt \eps M n$ has:
	\begin{itemize}
	...
	\end{itemize}
	Clearly this category comes with a faithful functor $\erasejc \loch : \dcubeopt \eps M n \to \cubecat^\eps_M$.
\end{definition}
...
\begin{proposition} \label{thm:jetcubes:comparison:depth-bipointedsym}
	The category $\dcubeopt \affine \bipointedsym n$ is isomorphic to the category $\jetcubeopt \eps \bipointedsym {[\npolar]^n}$ for $\eps \in \accol{\cartes, \affine}$.
\end{proposition}
\begin{proof}
	By \cref{thm:affine-automatic}, the value of $\eps$ does not matter, so let us set $\eps = \affine$.
	We construct a functor $F : \jetcubeopt \affine \bipointedsym {[\npolar]^n} \to \dcubeopt \affine \bipointedsym n$ such that $\erasejc \loch \circ F = \erasejc \loch$:
	\begin{itemize}
...
\begin{proposition} \label{thm:jetcubes:comparison:depth-bipointed}
	The category $\dcubeopt \affine \bipointed n$ is isomorphic to the category $\jetcubeopt \eps \bipointed {[\npolar]^n}$ for $\eps \in \accol{\cartes, \affine}$.
\end{proposition}
\begin{proof}
...
\end{definition}
\begin{proposition}
	 $\twistedcubegrp$ is isomorphic to $\jetcubeopt \cartes \booleanalg {[\ypolar]}$.
\end{proposition}
\begin{proof}
	Clearly, the objects correspond.
	The morphisms $V \to W$ of $\jetcubeopt \cartes \booleanalg {[\ypolar]}$ are morphisms $f : \JEP(V) \to \JEP(W)$ such that $Uf : \EP(\erasejc V) \to \EP(\erasejc W)$ lifts to a morphism of cubes, which it always does by \cref{thm:booleanalg-ff}.
\end{proof}
...
\begin{definition} \cite[def.\ 34]{pinyo-twisted} \label{def:twisted-tri}
	The category $\twistedcubetri$ has as objects $[\ypolar]$-jet-cubes (i.e.\ natural numbers) and as morphisms $V \to W$ all jet set morphisms (i.e. graph morphisms) $\JEP(V) \to \JEP(W)$ or, equivalently by the previous proposition, all jet cube morphisms $V \to W$ in $\jetcubeopt \cartes \booleanalg {[\ypolar]}$,	
...
\end{corollary}
\begin{proposition}
	$\twistedcubetri$ is isomorphic to $\jetcubeopt \affine \bipointedsym {[\ypolar]}$.
\end{proposition}
\begin{proof}
	Since the rules mentioned in \cref{def:twisted-tri} pertain to the calculus of $\trijetcubeopt \affine \bipointedsym {[\ypolar]}$ and in fact always yield premises/conclusions about forward cubes for conclusions/premises about forward cubes, and since $\jetcubeopt \affine \bipointedsym {[\ypolar]}$ is a faithful subcategory of $\jetcubeopt \cartes \booleanalg {[\ypolar]}$ by \cref{thm:monad-coercions-faithful}, it is clear that the identity-on-objects functor $\twistedcubetri \to \jetcubeopt \affine \bipointedsym {[\ypolar]}$ is faithful, i.e.\ we can think of $\twistedcubetri$ as a wide subcategory of $\jetcubeopt \affine \bipointedsym {[\ypolar]}$.
	
	To show fullness, we note the following facts about the calculus for $\trijetcubeopt \affine \bipointedsym {[\ypolar]}$:
	\begin{enumerate}
	...
	\end{enumerate}
	This means we can always rewrite a derivation tree in the calculus for $\trijetcubeopt \affine \bipointedsym {[\ypolar]}$ of a morphism in $\jetcubeopt \affine \bipointedsym {[\ypolar]}$ to use only the prescribed rules. 
\end{proof}
