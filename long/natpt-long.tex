...
\begin{definition} \label{def:orientation-psh}
	...
	The action on morphisms $\oripsh(p)$ is denoted as $\loch \circ p$ and given in \cref{fig:polarity-orientation}; it is monotonically increasing in both arguments.\footnote{So the domain of $\oripsh : \polcat\op \to \Poset$ has reversed 1-cells but not reversed 2-cells / inequalities.}
\end{definition}
\begin{figure}[htb]
	\[
...
\end{proof}
\begin{corollary}
	The presheaf of orientations $\oripsh$ is isomorphic to the Yoneda-embedding $\yoneda(\ypolar) = \Hom(\loch, \ypolar)$ of $\ypolar$ in two ways: by identifying $\pospol$ with either $\jetsym$ or $\tejsym$. \qed
\end{corollary}
\begin{corollary}
...
\begin{definition}
	...
	\begin{enumerate}
		\item $\jetsym \circ p \leq \jetsym \circ q$ and $\jetsym \circ p \leq \tejsym \circ q$,
		\item $\jetsym \circ p \leq \jetsym \circ q$ and $\tejsym \circ p \leq \jetsym \circ q$,
		\item $\jetsym \circ p \leq \equijetsym \circ q$,
		\item $\infrajetsym \circ p \leq \jetsym \circ q$,
		\item $\infrajetsym \circ p \leq \equijetsym \circ q$,
...
\begin{proof}[Proof of equivalence]
	\begin{description}
		...
		\item[$1 \Leftrightarrow 3$] By the universal property of the meet, and the fact that $\loch \circ q$ preserves meets.
		\item[$2 \Leftrightarrow 4$] By the universal property of the join, and the fact that $\loch \circ p$ preserves joins.
		\item[$1 \wedge 2 \Leftrightarrow 5$] By similar reasoning as the two previous points.
		\item[$1 \Rightarrow 6$] Condition 1 implies $p \leq q$ because $\jetsym \circ \loch$ is an isomorphism of posets. If $p \leq q$ but neither is symmetric, then $p = q \in \accol{\pospol \neq \negpol}$, contradicting 1.
...
\end{definition}
\begin{definition}
	We already know that $\Degeqtop(\vec a, b)$ is a covariant 2-functor of $b \in \Obj(\polcat)$. We make it a contravariant 2-functor of $\vec a \in \Obj(\poreshcat)$, i.e. a profunctor $\Degeqtop : \poreshcat\op \times \polcat \to \Poset$, with the functorial action given by $\loch \bindop \loch$.
\end{definition}
...
\begin{definition}
	...
	We define a transformation, natural in $\vec a$ and extranatural in $b$:
	\[
		\loch \oriof \loch : \oripsh(b) \times \poldeg(\vec a, b) \to \orideg(\vec a) : P \oriof (\pdeg p i) = \odeg{(P \circ p)}{i}.
	\]
\end{definition}
\todoi{I lost track of the systematics here.}
...
\begin{theorem}\label{thm:poresh-adjoint}
	\begin{enumerate}
		...
		\begin{equation} \label{eq:poresh-adjoint-crit}
			\forall P_i \in \oripsh(\vec a), Q_j \in \oripsh(\vec b). P_i \leq Q_j \cdot \lambda \Leftrightarrow P_i \cdot \rho \leq Q_j.
		\end{equation}
		\item By consequence:
		\begin{itemize}
			...
			\begin{align}
				...
				&\text{for all } \jetsym_i \in \oripshfwd(\vec a) \text{ with } \jetsym_i \cdot \rho = \infrajetsym_j \text{, we have } b_j = \npolar. \label{eq:poresh-left-adjoint-crit2}
			...
			\end{align}
			In this case, $Q_j \cdot \lambda$ is the greatest symbol $P_i \in \oripsh(\vec a)$ such that $P_i \cdot \rho \leq Q_j$ (even when $Q_j \not\in \oripshfwd(\vec b)$).
			
			\item A reshuffle $\lambda$ has a right adjoint $\rho$ if and only if
			\begin{align}
				&\text{for all } \jetsym_j \in \oripshfwd(\vec b), \text{we have } \jetsym_j \cdot \lambda < \topsym, \text{and} \label{eq:poresh-right-adjoint-crit1} \\
				&\text{for all } \jetsym_j \in \oripshfwd(\vec b) \text{ with } \jetsym_j \cdot \lambda = \equijetsym_i \text{, we have } a_i = \npolar. \label{eq:poresh-right-adjoint-crit2}
			\end{align}
			In this case, $P_i \cdot \rho$ is the least symbol $Q_j \in \oripsh(\vec b)$ such that $P_i \leq Q_j \cdot \lambda$ (even when $P_i \not\in \oripshfwd(\vec a)$).
		\end{itemize}
	\end{enumerate}
\end{theorem}
\begin{proof}
	\begin{enumerate}
		...
		To see the former, we need to show that for all $P_i \in \oripshfwd(\vec a)$, we have $P_i \leq (P_i \cdot \rho) \cdot \lambda$ which is equivalent to $P_i \cdot \rho \leq P_i \cdot \rho$ and hence true.
		
		To see the latter, we need to show that for all $Q_j \in \oripshfwd(\vec b)$, we have $(Q_j \cdot \lambda) \cdot \rho \leq Q_j$ which is equivalent to $Q_j \cdot \lambda \leq Q_j \cdot \lambda$ and hence true.
		...
		\[
			\ell : \oripsh(\vec b) \to \oripsh(\vec a) : Q_j \mapsto \max \set{P_i \in \oripsh(\vec a)}{P_i \cdot \rho \leq Q_j}.
		\]
		...
		\[
			\forall Q_j \in \oripsh(\vec b). \exists P_i \in \oripsh(\vec i).P_i \cdot \rho \leq Q_j.
		\]
		...
		Its restriction to $\oripshfwd(\vec b)$ defines a polarized reshuffle $\lambda \in \Hom(\vec a, \vec b)$.
		We have to show that $\ell$ the bind of $\lambda$ if and only if \cref{eq:poresh-left-adjoint-crit2} holds.
		\begin{itemize}
			\item To prove `only if', assume that \cref{eq:poresh-left-adjoint-crit2} does not hold (which is decidable). Then there exists some $\jetsym_i \in \oripshfwd(\vec a)$ such that $\jetsym_i \cdot \rho = \infrajetsym_j$ with $b_j = \ypolar$.
			...
			\begin{align*}
				...
				&= \tejsym \cdot \max \set{P_i \in \oripsh(\vec a)}{P_i \cdot \rho \leq \jetsym_j} \\
				& \qquad \qquad \text{(Definition of $\ell$)} \\
				&= \max \set{\tejsym \cdot P_i}{P_i \in \oripsh(\vec a) \text{ with } P_i \cdot \rho \leq \jetsym_j} \\
				& \qquad \qquad \text{($\tejsym \cdot \loch : \oripsh(\vec a) \to \oripsh(\vec a)$ preserves $\leq$.)} \\
				&= \max \set{P'_i \in \oripsh(\vec a)}{(\tejsym \cdot P'_i) \cdot \rho = \tejsym \cdot (P'_i \cdot \rho) \leq \jetsym_j} \\
				& \qquad \qquad \text{(By taking $P'_i := \tejsym \cdot P_i$)} \\
				&= \max \set{P'_i \in \oripsh(\vec a)}{P'_i \cdot \rho \leq \tejsym_j} \\
				...
				&= \max \set{P_i \in \oripsh(\vec a)}{P_i \cdot \rho \leq \jetsym_j} \wedge \max \set{P_i \in \oripsh(\vec a)}{P_i \cdot \rho \leq \tejsym_j} \\
				&= \max \set{P_i \in \oripsh(\vec a)}{P_i \cdot \rho \leq \jetsym_j} \cap \set{P_i \in \oripsh(\vec a)}{P_i \cdot \rho \leq \tejsym_j} \\
				& \qquad \qquad \text{(Both sets are downward closed.)} \\
				&= \max \set{P_i \in \oripsh(\vec a)}{P_i \cdot \rho \leq \jetsym_j \text{ and } P_i \cdot \rho \leq \tejsym_j} \\
				&= \max \set{P_i \in \oripsh(\vec a)}{P_i \cdot \rho \leq \equijetsym_j} \\
				&= \ell(\equijetsym_j), \\
				% ------------------------
				...
				& \qquad \qquad \text{(Definition of $\lambda$, result above.)} \\
				&= \max \set{P_i \in \oripsh(\vec a)}{P_i \cdot \rho \leq \jetsym_j} \vee \max \set{P_i \in \oripsh(\vec a)}{P_i \cdot \rho \leq \tejsym_j} \\
				&= \bigvee \set{P_i \in \oripsh(\vec a)}{P_i \cdot \rho \leq \jetsym_j} \vee \bigvee \set{P_i \in \oripsh(\vec a)}{P_i \cdot \rho \leq \tejsym_j} \\
				& \qquad \qquad \text{(The maxima exist, so they are equal to the joins (suprema).)} \\
				&= \bigvee \set{P_i \in \oripsh(\vec a)}{P_i \cdot \rho \leq \jetsym_j} \cup \set{P_i \in \oripsh(\vec a)}{P_i \cdot \rho \leq \tejsym_j} \\
				& \qquad \qquad \text{(Associativity.)} \\
				&= \bigvee \set{P_i \in \oripsh(\vec a)}{P_i \cdot \rho \leq \jetsym_j \text{ or } P_i \cdot \rho \leq \tejsym_j} \\
				&= \bigvee \set{P_i \in \oripsh(\vec a)}{P_i \cdot \rho \leq \infrajetsym_j} \\
...
%				&= \equijetsym \cdot \max \set{P_i \in \oripsh(\vec a)}{P_i \cdot \rho \leq \jetsym_j} \\
%				&= \max \set{\equijetsym \cdot P_i}{P_i \in \oripsh(\vec a) \text{ with } P_i \cdot \rho \leq \jetsym_j} \\
%				& \qquad \qquad \text{($\equijetsym \cdot \loch : \oripsh(\vec a) \to \oripsh(\vec a)$ preserves $\leq$.)} \\
%				&= \max \bigcup \left\{
%					\begin{array}{l}
...
%				&= \infrajetsym \cdot \max \set{P_i \in \oripsh(\vec a)}{P_i \cdot \rho \leq \jetsym_j} \\
%				&= \max \set{\infrajetsym \cdot P_i}{P_i \in \oripsh(\vec a) \text{ with } P_i \cdot \rho \leq \jetsym_j} \\
%				& \qquad \qquad \text{($\infrajetsym \cdot \loch : \oripsh(\vec a) \to \oripsh(\vec a)$ preserves $\leq$.)} \\
%				&= \max \bigcup \left\{
%					\begin{array}{l}
%						\set{\equijetsym_i}{0 \leq i < \lenpol{\vec a} \text{ with } \equijetsym_i \cdot \rho \leq \jetsym_j}, \\
...
%				\right\} \\
%				& \qquad \qquad \text{(Add sets bounded from above by last set.)} \\
%				&= \max \set{P_i \in \oripsh(\vec a)}{\equijetsym_i \cdot \rho \leq \jetsym_j} \\
%				& \qquad \qquad \text{(Regroup cases.)} \\
%				&= \max \set{P_i \in \oripsh(\vec a)}{\deg(\jetsym_i \cdot \rho) \leq j} \\
...
%				&= \max \set{P_i \in \oripsh(\vec a)}{P_i \cdot \rho \leq \infrajetsym_j} \\
%				&= \ell(\infrajetsym_j).
%			\end{align*}
		\end{itemize}
		...
		We would like to define in a first step a function
		\[
			r : \oripsh(\vec a) \to \oripsh(\vec b) : P_i \mapsto \min \set{Q_j \in \oripsh(\vec b)}{P_i \leq Q_j \cdot \lambda}.
		\]
		...
		\[
			\forall P_i \in \oripsh(\vec a) . \exists Q_j \in \oripsh(\vec b). P_i \leq Q_j \cdot \lambda.
		\]
		...
		Its restriction to $\oripshfwd(\vec a)$ defines a polarized reshuffle $\rho \in \Hom(\vec b, \vec a)$.
		...
		\begin{itemize}
			...
			Then there exists some $\jetsym_j \in \oripshfwd(\vec b)$ such that $\jetsym_j \cdot \lambda = \topsym$.
			...
			Then there exists some $\jetsym_j \in \oripshfwd(\vec b)$ such that $\jetsym_j \cdot \rho = \equijetsym_i$ with $a_i = \ypolar$.
			...
			\begin{align*}
				...
				&= \min \set{Q_j \in \oripsh(\vec b)}{\jetsym_i \leq Q_j \cdot \lambda} \vee \min \set{Q_j \in \oripsh(\vec b)}{\tejsym_i \leq Q_j \cdot \lambda} \\
				&= \min \set{Q_j \in \oripsh(\vec b)}{\jetsym_i \leq Q_j \cdot \lambda} \cap \set{Q_j \in \oripsh(\vec b)}{\tejsym_i \leq Q_j \cdot \lambda} \\
				&\qquad \qquad \text{(Both sets are upward closed.)} \\
				&= \min \set{Q_j \in \oripsh(\vec b)}{\jetsym_i \leq Q_j \cdot \lambda \text{ and } \tejsym_i \leq Q_j \cdot \lambda} \\
				&= \min \set{Q_j \in \oripsh(\vec b)}{\infrajetsym_i \leq Q_j \cdot \lambda} \\
				&= r(\infrajetsym_i) \\
				% ----------------
				...
				&\qquad \qquad \text{(Definition of $\rho$, result above.)} \\
				&= \min \set{Q_j \in \oripsh(\vec b)}{\jetsym_i \leq Q_j \cdot \lambda} \wedge \min \set{Q_j \in \oripsh(\vec b)}{\tejsym_i \leq Q_j \cdot \lambda} \\
				&= \bigwedge \set{Q_j \in \oripsh(\vec b)}{\jetsym_i \leq Q_j \cdot \lambda} \wedge \bigwedge \set{Q_j \in \oripsh(\vec b)}{\tejsym_i \leq Q_j \cdot \lambda} \\
				&\qquad \qquad \text{(The minima exist, so they are equal to the meets (infima).)} \\
				&= \bigwedge \set{Q_j \in \oripsh(\vec b)}{\jetsym_i \leq Q_j \cdot \lambda} \cup \set{Q_j \in \oripsh(\vec b)}{\tejsym_i \leq Q_j \cdot \lambda} \\
				&\qquad \qquad \text{(Associativity.)} \\
				&= \bigwedge \set{Q_j \in \oripsh(\vec b)}{\jetsym_i \leq Q_j \cdot \lambda \text{ or } \tejsym_i \leq Q_j \cdot \lambda} \\
				&= \bigwedge \set{Q_j \in \oripsh(\vec b)}{\equijetsym_i \leq Q_j \cdot \lambda} \\
...
%				&= \infrajetsym \cdot \min \set{Q_j \in \oripsh(\vec b)}{\jetsym_i \leq Q_j \cdot \lambda} \\
%				&= \min \set{\infrajetsym \cdot Q_j}{Q_j \in \oripsh(\vec b) \text{ with } \jetsym_i \leq Q_j \cdot \lambda} \\
%				& \qquad \qquad \text{($\infrajetsym \cdot \loch : \oripsh(\vec b) \to \oripsh(\vec b)$ preserves $\leq$.)} \\
%				&= \min \bigcup \left\{
%					\begin{array}{l}
...
%				&= \equijetsym \cdot \min \set{Q_j \in \oripsh(\vec b)}{\jetsym_i \leq Q_j \cdot \lambda} \\
%				&= \min \set{\equijetsym \cdot Q_j}{Q_j \in \oripsh(\vec b) \text{ with } \jetsym_i \leq Q_j \cdot \lambda} \\
%				& \qquad \qquad \text{($\equijetsym \cdot \loch : \oripsh(\vec b) \to \oripsh(\vec b)$ preserves $\leq$.)} \\
%				&= \min \bigcup \left\{
%					\begin{array}{l}
%						\set{\equijetsym_j}{0 \leq j < \lenpol{\vec b} \text{ with } \jetsym_i \leq \equijetsym_j \cdot \lambda}, \\
...
%				\right \} \\
%				& \qquad \qquad \text{(Add sets bounded from below by first set.)} \\
%				&= \min \set{Q_j \in \oripsh(\vec b)}{\jetsym_i \leq \infrajetsym_j \cdot \lambda} \\
%				& \qquad \qquad \text{(Regroup cases.)} \\
%				&= \min \set{Q_j \in \oripsh(\vec b)}{i \leq \deg(\jet_j \cdot \lambda)} \\
...
%				&= \min \set{Q_j \in \oripsh(\vec b)}{\equijetsym_i \leq Q_j \cdot \lambda} \\
%				&= r(\equijetsym_i). \\
%			\end{align*}
		\end{itemize}
	\end{enumerate}
...
\begin{proof}
	...
	\begin{description}
		...
		\item[\framebox{$\equijetsym_j$}] \todoi{We cannot rule out that $\jetsym_{i'} \cdot \mu \in \accol{\jetsym_j, \tejsym_j}$ for $i' > i$.}
		
		\item[\framebox{$\jetsym_j$}] 
	\end{description}
