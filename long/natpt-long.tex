...
\todoi{Sometimes, we need to consider only reshuffles that have at least a certain number of left/right adjoints. The 2-posets that contain only those reshuffles (with the same order relation) will be denoted as $\poreshcat\withllr$, where the number of white bullets on the left/right is the minimal number of left/right adjoints.}
\begin{theorem}\label{thm:poresh-adjoint}
	\begin{enumerate}
		...
		\begin{itemize}
			...
			\begin{align}
				&\eqtysym \oriof \rho = \eqtysym, \text{and} \label{eq:poresh-left-adjoint-crit1} \\
			...
			\end{align}
			In this case, for any $Q_j \in \oridegeqtop(\vec b)$, we find $Q_j \oriof \lambda$ as the greatest oriented degree $P_i \in \oridegeqtop(\vec a)$ such that $P_i \oriof \rho \leq Q_j$.
			
			\item A reshuffle $\lambda$ has a right adjoint $\rho$ if and only if
			\begin{align}
				&\text{for all } j \in \Degeq(\vec b), \text{we have } j \cdot \lambda < \topsym, \text{and} \label{eq:poresh-right-adjoint-crit1} \\
				&\text{for all } j \in \Degeq(\vec b) \text{ with } j \cdot \lambda = \mixpol_i \text{, we have } a_i = \npolar. \label{eq:poresh-right-adjoint-crit2}
			\end{align}
			In this case, for any $P_i \in \oridegeqtop(\vec a)$, we find $P_i \oriof \rho$ as the least oriented degree $Q_j \in \oridegeqtop(\vec b)$ such that $P_i \leq Q_j \oriof \lambda$.
		\end{itemize}
	\end{enumerate}
\end{theorem}
\begin{proof}
	\begin{enumerate}
		...
		\begin{align*}
			...
			&\quad\Leftrightarrow\quad
			\jetsym \oriof \pospol_i \circ \rho \leq \jetsym \oriof \pospol_i \circ \rho
			& \text{(\cref{eq:poresh-adjoint-crit})} \\
			&\quad\Leftrightarrow\quad
			\text{true}.
		\end{align*}
		...
		\begin{align*}
			...
			& \text{($\jetsym \oriof \loch$ is an isomorphism)} \\
			&\quad\Leftrightarrow\quad
			\jetsym \oriof \pospol_j \circ \lambda \leq \jetsym \oriof \pospol_j \circ \lambda
			& \text{(\cref{eq:poresh-adjoint-crit})} \\
			&\quad\Leftrightarrow\quad
			\text{true}.
		\end{align*}
...
\todoi{Update notation}
		...
		\[
			\ell : \oridegeqtop(\vec b) \to \oridegeqtop(\vec a) : Q_j \mapsto \max \set{P_i \in \oridegeqtop(\vec a)}{P_i \oriof \rho \leq Q_j}.
		\]
		A left adjoint $\lambda \dashv \rho$ exists if and only if this function is definable and is the action of some polarized reshuffle on oriented degrees.
		%
		However, $\ell$ is only defined if the set of which we take the maximum\footnote{Which differs from the join in that it needs to be one of the operands.}, is non-empty\footnote{In which case there is indeed a maximum, since the set is closed under joins, since $\loch \oriof \rho$ respects joins (\cref{thm:poldeg-poresh-lattice}).}.
		As such, existence of a left adjoint requires that
		\[
			\forall Q_j \in \oridegeqtop(\vec b). \exists P_i \in \oridegeqtop(\vec i).P_i \oriof \rho \leq Q_j.
		\]
		Without loss of generality, we can instantiate both $P_i$ and $Q_j$ with the minimal element $\eqtysym$, so existence of a left adjoint requires that $\eqtysym \oriof \rho = \eqtysym$ (\cref{eq:poresh-left-adjoint-crit1}).
		
		We now assume that this is the case, so that $\ell$ is defined.
		We define $\lambda$ by $j \cdot \lambda = \pospol_j$ \todoi{You're mixing up orientations and polarities here.}
