...
\subsection{Categorical Properties} \label{sec:jetset:cat}
\begin{proposition} \label{thm:jetset:colim}
	Let $D : \cat S \to \jetset{\vec a}$ be a diagram.
...
\begin{definition} \label{def:sssprod}
	...
	\begin{itemize}
		...
		\begin{corollary}
			...
			\]
			where $\Usym_{\vec x \sqsubseteq \vec y} : \jetset{\vec x} \to \jetset{\vec y}$ is the forgetful functor, forgetting symmetry at the degrees where $x_i = \npolar \neq y_i = \ypolar$.
		\end{corollary}
...
\section{Semantics on Jet Sets} \label{sec:poresh-jetset}
...
\begin{definition}
	...
	\begin{enumerate}
		\item We define a pseudoreflexive category-enriched graph morphism (reflexive under the strict quotienting \cref{ax:bishop}) $\jetsetsym : \poreshcat \to \Cat$ from the 2-category of polarized reshuffles $\Cat$:
		\begin{itemize}
			\item Sending an object $\vec a$ to $\jetset{\vec a}$.
			\item Sending a morphism $\mu : \vec a \to \vec b$ to $\jetset{\mu} : \jetset{\vec a} \to \jetset{\vec b}$ where $\jetset{\mu}$ (more briefly denoted as $\mu$) sends a jet set $X$ to the jet set $\mu X$ whose
			\begin{itemize}
			...
			\end{itemize}
			$\jetset{\mu}$ is easily seen to have a functorial action on jet set morphisms which preserves identity and composition.
			\item Sending a unique proof of inequality $\mu \leq \nu : \vec a \to \vec b$ to an obvious natural transformation $\ket{\mu \leq \nu} : \mu \to \nu : \jetset{\vec a} \to \jetset{\vec b}$ whose underlying function sends $[x]_\mu$ to $[x]_\nu$, which is necessarily a larger equivalence class.
		\end{itemize}
	...
	\end{enumerate}
\end{definition}
\begin{proof}[Proof of strict 2-functoriality of $\jetsetsym \noquot$]
	The functorial action of $\jetsetsym\noquot(\mu)$ on $X$ only affects the jet relations of $X$.
	Let $\vec a \xrightarrow{\mu} \vec b \xrightarrow{\nu} \vec c$ and $X \in \jetset{\vec a}$.
	We have
	\[
...
\begin{corollary} \label{thm:poresh-jetset-with-adjoint}
	We have:
	\begin{enumerate}
		\item A strict 2-functor $\jetsetsym\withr : \poreshcat\withr \to \Cat$ that restricts $\jetsetsym\noquot$ and therefore essentially restricts $\jetsetsym$,
		\item A pseudofunctor $\jetsetsym\withl : \poreshcat\withl \to \Cat$ that restricts $\jetsetsym$.
	\end{enumerate}
\end{corollary}
\begin{proof}
	\begin{enumerate}
		\item Obviously, we can restrict $\jetsetsym\noquot : \poreshcat\noquot \to \Cat$ along the 2-poset inclusion $\poreshcat\withr \subseteq \poreshcat\noquot$, producing another strict 2-functor.
		\item Obviously, we can restrict $\jetsetsym : \poreshcat \to \Cat$ along $\poreshcat\withl \subseteq \poreshcat$.
		Now if $\lambda \dashv \rho$ in $\poreshcat$, then $\jetsetsym\withr(\lambda) \cong \jetsetsym(\lambda) \dashv \jetsetsym(\rho) = \jetsetsym\withl(\rho)$.
		Then pseudofunctoriality of $\jetsetsym\withl$ follows from strict 2-functoriality of $\jetsetsym\withr$ and uniqueness of the adjoint. \qedhere
	\end{enumerate}
\end{proof}

As \cref{def:cat-enriched-graph} already foreboded, we have to make an abhorrent constatation for the most general operation $\jetsetsym : \poreshcat \to \Cat$, which however becomes irrelevant when we restrict to jet topes and jet cubes:
\begin{proposition}
	$\jetsetsym : \poreshcat \to \Cat$ does not preserve composition.
\end{proposition}
\begin{proof}
	Consider the following two mappings of the same jet set $X \in \jetset{[\npolar, \ypolar]}$:
	\begin{align*}
		\xymatrix{
			& {[\npolar, \ypolar]} &
			&&& %-----------
...
\begin{example} \label{ex:poresh-sym-jetset}
	...
	\]
\end{example}
\begin{proposition} \label{thm:poresh-jetset-nonexact}
	A functor of the form $\jetset{\mu} : \jetset{\vec a} \to \jetset{\vec b}$ for some $\mu : \vec a \to \vec b$ does not necessarily preserve finite limits, nor finite colimits.	
\end{proposition}
\begin{proof}
...
\end{definition}
\begin{proposition} \label{thm:poresh-jetset-intervals}
	...
	\begin{equation}
		\begin{array}{c c c}
			\jetset{\mu} \rival{\jetsym_i\pmry} \cong \rival{\jetsym_i \oriof \rho},
			& \jetset{\mu} \rival{\tejsym_i\pmry} \cong \rival{\tejsym_i \oriof \rho},
			& \jetset{\mu} \rival{\equijetsym_i\pmry} \cong \rival{\equijetsym_i \oriof \rho},
			\\[1em]
			& \jetset{\mu} \rival{\edgesym_i\pmrn} \cong \rival{\edgesym_i \oriof \rho}.
		\end{array}
		\label{eq:poresh-jetset-intervals}
	\end{equation}
...
\begin{proof}
	...
	\end{enumerate}
\end{proof}
\begin{proposition} \label{thm:poresh-jetset-prism-commut}
	Let $\mu : \vec a \to \vec b$ be a polarized reshuffle with right adjoint $\mu \dashv \rho$, and $i \in \Deg(\vec a)$.
	Then the action of $\jetset{\mu}$ on prisms can be looked up, up to isomorphism\footnote{On the nose under the strict quotienting \cref{ax:bishop}.}, in the following table, where $j = \getdeg(i \cdot \rho)$:%
...
\begin{proof}
	Let $P \in \accol{\jetsym\pmr{a_i}, \tejsym\pmr{a_i}}$, and $X \in \jetset{\vec a}$.
	We prove that $\mu(X \multip \rival{P_i}) \in \jetset{\vec b}$ is isomorphic to the prescribed jet set.
	...
	\begin{enumerate}
		...
		By the same reasoning as in the proof of \cref{thm:poresh-jetset-intervals}, the assumption is equivalent to $P_i \oriof \rho \leq \edgesym_\eqtydeg$, hence $i \cdot \rho = \nonpol_\eqtydeg$.
		...
		Write $Y \in \jetset{\vec b}$ for the jet set prescribed by the table.
...
\begin{theorem} \label{thm:poresh-jettope}
	...
	Then the functor $\mu : \jetset{\vec a} \to \jetset{\vec b}$ sends jet topes to jet topes if and only if $\rho$ has a further right adjoint $\mu\pmrhom{\vec a}{\vec b} \dashv \rho\pmrhom{\vec b}{\vec a} \dashv \tau\pmrhom{\vec a}{\vec b}$.
	
	By consequence, there is a pseudofunctor $\jettopesym : \poreshcat\withrr \to \Cat$ sending
	\begin{itemize}
		\item an object $\vec a$ to $\jettope{\vec a}$,
		\item a morphism $\mu : \vec a \to \vec b$ to the restriction of $\jetsetsym\withr(\mu) : \jetset{\vec a} \to \jetset{\vec b}$ to $\jettope{\vec a}$,
		\item a 2-cell $\alpha : \mu \leq \nu : \vec a \to \vec b$ to the restriction of $\jetsetsym\withr(\alpha)$ to $\jettope{\vec a}$.
	\end{itemize}
\end{theorem}
\begin{proof}
	...
	\begin{itemize}
		...
		Then by \cref{thm:poresh-jetset-intervals}, $\mu \rival{\jetsym_{\predeg(\top)}} = \rival{\edgesym_\top}$, where $\rival{\jetsym_{\predeg(\top)}}$ is a jet tope but $\rival{\edgesym_\top}$ is not, as it is not $(n-1)$-connected (if $n > 0$) or not a subsingleton (if $n = 0$).
		...
		Then by \cref{thm:poresh-jetset-intervals}, $\mu \rival{\jetsym_i} = \rival{\equijetsym_j}$, where $\rival{\jetsym_i}$ is a jet tope but $\rival{\equijetsym_j}$ is not, as it is not $j$-antisymmetric.
	\end{itemize}
	...
	\begin{enumerate}
		...
		Then by \cref{thm:poresh-jetset-trcl-from-trcl}, we have $x \mathrel{(\infrajetsym_j \oriof \mu)^*} y$ in $X$ irrespective of the choice of representants.
	...
	\end{enumerate}
\end{proof}
\begin{lemma} \label{thm:poresh-jetset-equality-eq-from-trcl}
	Let $\mu \in \Hom_\poreshcat(\vec a, \vec b)$ and $X \in \jetset{\vec a}$.
...
\end{proof}
\begin{lemma} \label{thm:poresh-jetset-rel-from-trcl}
	Let $\mu \in \Hom_\poreshcat(\vec a, \vec b)$ and $X \in \jetset{\vec a}$.
...
\begin{proof}
	...
	\]
\end{proof}
\begin{corollary} \label{thm:poresh-jetset-trcl-from-trcl}
	Let $\mu \in \Hom_\poreshcat(\vec a, \vec b)$ and $X \in \jetset{\vec a}$.
...
\begin{proof}
	...
	\begin{align*}
		...
		& \text{(\cref{thm:poresh-jetset-intervals})} \\
...
Since jet cubes are realized as jet sets by repeated application of twisted prism functors to the terminal jet set (\cref{def:jep}), we can deduce the action of $\mu$ on jet cubes from its action on twisted prisms, which is given by \cref{thm:poresh-jetset-prism-commut}.
...
\begin{definition}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item sends $()$ to $\mu () := ()$,
			\item has an action on other cubes that can be looked up in the following table\footnote{to be compared with the table in \cref{thm:poresh-jetset-prism-commut}.}, where $j = \getdeg(i \cdot \rho)$:%
			\footnote{Note that if $i \cdot \rho = \isopol_j$, then by \cref{eq:poresh-withl-crit2}, $b_j = \npolar$.}
			\[
				\begin{array}{c | l || c | c || c}
					& a_i\,{\rightarrow} & \multicolumn{2}{c ||}{\ypolar} & \npolar \\ \hline
				...
				\end{array}
			\]
			\item thus satisfies $\JEP(\mu W) \cong \mu \JEP(W)$ by \cref{thm:poresh-jetset-prism-commut},
			...
			This definition is clearly natural on cubes, therefore on sets, and therefore (by faithfulness of $U : \jetset{\vec a} \to \Set$) also on jet sets. By conclusion, it is natural in $\jetcube{\orifb}{\affine}{M}{\vec a}$.
		\end{itemize}
	\end{itemize}
\end{definition}
\begin{corollary}
