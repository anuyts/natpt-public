...
\begin{theorem}\label{thm:poresh-adjoint}
	\begin{enumerate}
		...
		\begin{itemize}
			...
			\end{align}
			\item For either polarmode $c$, we have $\loch \circ \rho \dashv \loch \circ \lambda : \poldegeqtop(\vec b, c) \to \poldegeqtop(\vec a, c)$:
			\begin{align}
...
\begin{definition} \label{def:suc-degree}
	If $\vec a$ is a polarmode mask, then $\Degeqtop(\vec a)$ is a finite linear order.
	\begin{enumerate}
...
%		\text{for all } i \in \Degeq(\vec a) \text{ with } (i \cdot \rho) = q_j\pmrhom{b_j}{a_i}, \text{the polarity $q$ has a left adjoint.}
	...
	\begin{align*}
		...
		& \text{(By \cref{eq:poldeg-poresh}. Sym.\ pol.\ degrees are comparable to everything)}
		\\ &\quad\Leftrightarrow\quad
...
\begin{proof}
	...
	\end{align*}
	\todoi{Use $\isopol \dashv \mixpol$ instead.}
	and
	\begin{align*}
		&\quad\mathrel{\phantom{\Leftrightarrow}}\quad
