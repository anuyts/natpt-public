...
\begin{definition}\label{def:orisymbol}
	...
	We define a non-commutative multiplication operation $\loch \cdot \loch : \orisymbolnodegree \times \orisymbolnodegree \to \orisymbolnodegree$ by
	\[
	...
	\]
	In particular, if $i = \npolar$, we have $P \cdot \edgesym_i = \edgesym_i$.
...
\begin{proof}
	\begin{enumerate}
		...
		Assume given a reshuffle $\rho \in \Hom(\vec b, \vec a)$.
		We would like to define in a first step a function
		\[
			\loch \cdot \bar \lambda : \orisymbol(\vec b) \to \orisymbol(\vec a) : Q_j \mapsto \bigvee \set{P_i \in \orisymbol(\vec a)}{P_i \cdot \rho \leq Q_j}.
		\]
		A left adjoint exists if and only if this function is definable and the bind of some polarized reshuffle.
		
		However, $\loch \cdot \bar \lambda$ is only defined if the set of which we take the join, is non-empty.
		...
		We now assume that this is the case, so that $\loch \cdot \bar \lambda$ is defined.
		Its restriction to $\orisymbolfwd(\vec b)$ defines a polarized reshuffle $\lambda \in \Hom(\vec a, \vec b)$.
		We have to show that $\loch \cdot \bar \lambda$ the bind of $\lambda$ if and only if \cref{eq:poresh-left-adjoint-crit2} holds.
		
		Assume it does not. Then there exists some $\jetsym_i \in \orisymbolfwd(\vec a)$ with $a_i = \ypolar$ such that $\jetsym_i \cdot \rho = \infrajetsym_j$.
		Then $\infrajetsym_j \cdot \bar \lambda \geq \infrajetsym_i$, but $\jetsym_j \cdot \lambda = \jetsym_j \cdot \bar \lambda < \equijetsym_i$.
		This is impossible if $\loch \cdot \bar \lambda$ is the bind of $\lambda$.
