...
%	Before that, we put, if existent, the sole variable $\dm k_2$ such that $e_{\dm k_2}$ mentions the second to last variable of degree $i$ in $V$. Etc.
	...
	\begin{itemize}
		...
		If $\dm i : \rival{\tej_i}$, we apply \ruleref{inv:fwd} and push down the introduced negation, after which we do not resort to the induction hypothesis but proceed below.\footnote{Alternatively, we could adapt the proof below to the case where $\dm i : \rival{\tej_i}$.}
...
\begin{lemma} \label{thm:char-twisted-cube-to-line}
	...
	Then $\dm i \psub{\vfi}$ is of the form
	\[
		\dm i \psub{\vfi} = (\ldots((\lnot^{p_1} \dm j_1 \mathrel{R_1} \lnot^{p_2} \dm j_2) \mathrel{R_2} \lnot^{p_3} \dm j_3) \ldots \mathrel{R_{n-1}} \lnot^{p_n} \dm j_n) \mathrel{R_n} c
	\]
	with $c, p_k \in \accol{0, 1}$ and $R_k \in \accol{{\vee}, {\wedge}, K}$ where we define $x \mathrel K y := y$.
\end{lemma}
Note that $t \mathrel{R} c$ is either equal to $t$ or to $c$.
\begin{proof}
...
\begin{lemma} \label{thm:char-jet-cube-to-line}
	In $\trijetcubeopt \affine \booleanalg {\vec a}$, consider $\vfi : V \to (\dm i : \rival{\jet_i})$.
	...
	\]
	with $c, p_k \in \accol{0, 1}$ and $R_k \in \accol{{\vee}, {\wedge}, K}$ where we define $x \mathrel K y := y$, and every $h_k$ is a composition of functions of the form $\loch \mathrel{S} t$ with $t$ any affine boolean expression mentioning only $i$-symmetric variables and $S \in \accol{{\vee}, {\wedge}, K}$.
\end{lemma}
\begin{proof}
	...
	Let $n > 0$.
	We reduce $\dm i \psub \vfi$ to an expression $e$ by pushing down all negations, simplifying all conjunctions and disjunctions with constants, and using associativity to flatten conjunctions and disjunctions.
	Thus, the syntax tree of $e$ has a variable finite branching factor, nodes labelled alternatingly with $\vee$ and $\wedge$, and leaves labelled with (negations of) variables from $V$.
	Write $\dm j_k'$ to mean $\dm j_k$ if $\dm j_k : \rival{\jet_i}$ and to mean $\lnot \dm j_k$ if $\dm j_k : \rival{\tej_i}$.
	...
	We prove the induction step again by induction, now on the number of leaves of $e$.
	...
	\begin{itemize}
		...
		However, if $e$ is a conjunction, then $t$ is a disjunction, and we can set the operand that does not contain $\dm j_n$ to 1. The dual holds if $e$ is a disjunction.
