...
%			& \text{($\jetsym \oriof \loch$ is an isomorphism)} \\
		...
		\begin{itemize}
			\item To prove `only if', assume that \cref{eq:poresh-withl-crit2} does not hold (which is decidable). Then there exists some $i \in \Degeq(\vec a)$ such that $i \cdot \rho\pmrhom{\vec b}{\vec a} = \isopol_j\pmrhom{b_j}{a_i} \in \poldegeqtop(\vec b, a_i)$ with $b_j = \ypolar$.
