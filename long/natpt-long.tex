...
%			& \text{($\jetsym \oriof \loch$ is an isomorphism)} \\
		...
		\begin{itemize}
			...
			\begin{itemize}
				...
				\[
					\forall i \in \Degeq(\vec a). \qquad (\pospol_i)\pmrhom{\vec a}{a_i} \leq ((\pospol_i)\pmrhom{\vec a}{a_i} \circ \rho \pmrhom{\vec b}{\vec a}) \circ \lambda \pmrhom{\vec a}{\vec b}
				\]
				Let $\posdegeq(\vec a, c)$ be the full subposet of $\poldegeqtop(\vec a, c)$ on objects of the form $(\pospol_i)\pmrhom{\vec a}{c}$.
				
				
				\todoi{Old}
				Using \cref{thm:deg-vs-poldeg-poresh}, we rewrite the right hand side as $((\pospol_i)\pmrhom{\vec a}{a_i} \circ \rho \pmrhom{\vec b}{\vec a}) \circ \lambda \pmrhom{\vec a}{\vec b}$.
				It then suffices to prove the more general inequality
				\begin{equation}
					\forall c \in \Polarmode, p_i \in \poldegeqtop(\vec a, c). \qquad
					(p_i)\pmrhom{\vec a}{c} \leq ((p_i)\pmrhom{\vec a}{c} \circ \rho \pmrhom{\vec b}{\vec a}) \circ \lambda \pmrhom{\vec a}{\vec b}.
				\end{equation}
				It is a general fact about adjunctions $L \dashv R$ that there is a 1-1 correspondence between natural transformations $FR \to G$ and natural transformations $F \to GL$.
				Applying this to $\loch \circ \rho \dashv \ell_c$, where we restrict one category to the image of $\loch \circ \rho$, we may equivalently prove \todoi{You need to be more careful here, in order to guarantee that $q$ has a left adjoint.}
				\begin{equation}
					\forall c \in \Polarmode, q_j \in \Image(\loch \circ \rho) \subseteq \poldegeqtop(\vec b, c). \qquad
					\ell_c(q_j)\pmrhom{\vec a}{c} \leq (q_j)\pmrhom{\vec b}{c} \circ \lambda \pmrhom{\vec a}{\vec b}.
				\end{equation}
				The right hand side expands to $(q_j)\pmrhom{\vec b}{c} \circ \lambda \pmrhom{\vec a}{\vec b} = q\pmrhom{b_j}{c} \circ \ell_{b_j}(\pospol_j)\pmrhom{\vec a}{b_j}$ by \cref{eq:poldeg-poresh} and the definition of $\lambda$.
				...
				We set $\hat i = i$ and proceed by case distinction on $b_j$ and $q\pmrhom{b_j}{c}$.
				\begin{description}
					\item[\framebox{$p\pmrhom{a_i}{c} = \nonpol\pmrhom{\npolar}{c}$}]
				\end{description}
			\end{itemize}
			
			\item \todoi{Old}
			
			To prove `if', assume that \cref{eq:poresh-withl-crit2} holds. We prove that $\ell_c$ is the action of $\lambda$.
			
			It is sufficient to prove this for $c = \ypolar$.
			Indeed, we can reduce the case where $c = \npolar$ to the other case.
			For all masks $\vec x$, the action $\pospol \pmrny \circ \loch : \poldegeqtop(\vec x, \npolar) \to \poldegeqtop(\vec x, \ypolar)$ is injective and monotonically increasing (and therefore maximum-preserving).
			We have $\pospol \pmrny \circ \ell_\npolar((q_j)\pmrhom{\vec b}{\npolar}) = \ell_\ypolar(\pospol \pmrny \circ (q_j)\pmrhom{\vec b}{\npolar})$:
			
			
			%since there is just one composition table for polarities (\cref{fig:polarity-orientation}) and no additional equalities are introduced if the codomain $c$ becomes $\npolar$.
			
			It is clear from the definition of $\ell$ that $\ell_c(\nonpol_\top\pmrhom{\npolar}{c}) = \nonpol_\top\pmrhom{\npolar}{c}$, and for all other degrees we have $\ell_c(\pospol_j\pmrhom{b_j}{c}) = \pospol_j\pmrhom{b_j}{c} \circ \lambda\pmrhom{\vec a}{\vec b}$ by definition of $\lambda$ and \cref{thm:deg-vs-poldeg-poresh}.
			The remaining cases are $\ell_c(\negpol_j\pmrhom{b_j}{c})$ with $b_j = c = \ypolar$, $\ell_c(\mixpol_j\pmrhom{b_j}{c})$ with $b_j = \ypolar$, and $\ell_c(\isopol\pmrhom{b_j}{c})$ with $b_j = \ypolar$.
...
%			For other degrees $j$, we have furthermore
			\begin{align*}
				(\negpol_j)\pmrhom{\vec b}{\ypolar} \circ \lambda\pmrhom{\vec a}{\vec b}
				&= \negpol\pmrhom{b_j}{\ypolar} \circ (j \cdot \lambda)\pmrhom{\vec a}{b_j}
				= \negpol\pmrhom{b_j}{\ypolar} \circ \ell_{b_j}(\pospol_j\pmrhom{b_j}{b_j})\pmrhom{\vec a}{b_j} \\
				& \qquad \qquad \text{(Definition of $\lambda$)} \\
				&= \negpol\pmrhom{b_j}{\ypolar} \circ \max \set{p_i \in \poldegeqtop(\vec a, b_j)}{(p_i)\pmrhom{\vec a}{b_j} \circ \rho\pmrhom{\vec b}{\vec a} \leq (\pospol_j)\pmrhom{\vec b}{b_j}} \\
				& \qquad \qquad \text{(Definition of $\ell$)} \\
				&= \max \set{\negpol\pmrhom{b_j}{\ypolar} \circ (p_i)\pmrhom{\vec a}{b_j}}{p_i \in \poldegeqtop(\vec a, b_j) \text{ with } (p_i)\pmrhom{\vec a}{b_j} \circ \rho\pmrhom{\vec b}{\vec a} \leq (\pospol_j)\pmrhom{\vec b}{b_j}} \\
				& \qquad \qquad \text{($\negpol \circ \loch : \poldegeqtop(\vec a, b_j) \to \poldegeqtop(\vec a, \ypolar)$ preserves $\leq$.)} \\
				&= \max \set{p'_i \in \poldegeqtop(\vec a, \ypolar)}{\negpol\pmrhom{\ypolar}{b_j} \circ (p'_i)\pmrhom{\vec a}{\ypolar} \circ \rho\pmrhom{\vec b}{\vec a} \leq (\pospol_j)\pmrhom{\vec b}{b_j}} \\
				& \qquad \qquad \text{(By taking $(p'_i)\pmrhom{\vec a}{\ypolar} := \negpol\pmrhom{b_j}{\ypolar} \circ (p_i)\pmrhom{\vec a}{b_j}$; recall that $b_j = \ypolar$ too)} \\
				&= \max \set{p'_i \in \poldegeqtop(\vec a, \ypolar)}{(p'_i)\pmrhom{\vec a}{\ypolar} \circ \rho\pmrhom{\vec b}{\vec a} \leq \negpol_j\pmrhom{\vec b}{\ypolar}} \\
				& \qquad \qquad \text{(Involution $\negpol\pmrhom{b_j}{\ypolar} \circ \loch$ preserves and reflects $\leq$.)} \\
				&= \ell_\ypolar(\negpol_j) = \ell_c(\negpol_j), \\
				% ------------------------
