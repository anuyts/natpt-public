...
We propose two families of base categories, parametrized by the mask $\vec a$, and both are built using the category $\jetgph{\vec a}$ of \textbf{jet sets} over $\vec a$ (\cref{sec:jetgph}).
...
\section{Jet Sets} \label{sec:jetgph}
\subsection{Definitions} \label{sec:jetgph:def}
\begin{definition} \label{def:jetgph}
	Let $\vec a$ be a mask. An \textbf{$\vec a$-jet-set} is a set $X$ equipped with $\lenpol{\vec a}$ (decidable, proof-irrelevant) reflexive relations $\jet_i$ where
	\begin{itemize}
	...
	The category of $\vec a$-jet-sets is called $\jetgph{\vec a}$.
\end{definition}
...
%\todoi{Should there be a prism functor for $\rival{\equijet_i}$? It would only be well-defined on symmetric jet sets, the category of which is $\jetgph{\vec a'}$ where $a'_i = \npolar$, so it is derived from $\loch \multip \rival{\edge_i}$ and the forgetful functor $U^\npolar_i$.}
...
\begin{definition}
	...
	In a jet set $X$, write $\jet_i^*$ for the transitive closure of the $\jet_i$, and similarly $\infrajet_i^*$ for the transitive closure of $\infrajet_i$, which is the symmetric transitive closure of $\jet_i$.
	\begin{itemize}
...
\end{corollary}
\begin{proof}
	By induction on the dimension and using \cref{thm:lift-to-jet-topes}, it is clear that for any jet cube $W$, the jet set $\JEP(W)$ is a jet tope, which factors the action on objects. The action on morphisms factors because $\jettope{\vec a}$ is a full subcategory of $\jetgph{\vec a}$.
\end{proof}
...
%	Before that, we put, if existent, the sole variable $\dm k_2$ such that $e_{\dm k_2}$ mentions the second to last variable of degree $i$ in $V$. Etc.
	...
	\begin{itemize}
		\item If [$\dm i : \rival{\jet_i\pmr{a_i}}$ and $e_{\dm i} = 0$] or [$\dm i : \rival{\tej_i\pmr{a_i}}$ and $e_{\dm i} = 1$], then $\vfi$ being a jet cube morphism $V \to W = (U, \dm i : \_)$ (equivalently: $\EP(\vfi)$ being a jet set morphism $\JEP(V) \to \JEP(W)$) is equivalent to $(\novar / \dm i) \circ \vfi$ being a jet cube morphism $V \to \Opcube_i(U)$, so we can apply \ruleref{src:fwd} or \ruleref{src:bck}.
		
		\item If [$\dm i : \rival{\jet_i\pmr{a_i}}$ and $e_{\dm i} = 1$] or [$\dm i : \rival{\tej_i\pmr{a_i}}$ and $e_{\dm i} = 0$], then $\vfi$ being a jet cube morphism $V \to W = (U, \dm i : \_)$ (equivalently: $\EP(\vfi)$ being a jet set morphism $\JEP(V) \to \JEP(W)$) is equivalent to $(\novar / \dm i) \circ \vfi$ being a jet cube morphism $V \to U$, so we can apply \ruleref{tgt:fwd} or \ruleref{tgt:bck}.
		...
		\]
		We know that the black arrows are all jet cube morphisms, and the vertical arrows all yield full jet set morphisms (\cref{def:full-jetgphhom}).
...
\begin{definition} \cite[def.\ 34]{pinyo-twisted} \label{def:twisted-tri}
	The category $\twistedcubetri$ has as objects $[\ypolar]$-jet-cubes (i.e.\ natural numbers) and as morphisms $V \to W$ all jet set morphisms (i.e. graph morphisms) $\JEP(V) \to \JEP(W)$ (equivalently by the previous proposition, all jet cube morphisms $V \to W$ in $\jetcubefwd \cartes \booleanalg {[\ypolar]}$)	
...
In the following sections, we investigate how polarized reshuffles can act on jet sets (\cref{sec:poresh-jetgph}), jet topes and presheaves over these  (\cref{sec:poresh-jettope-jtset}), and jet cubes and presheaves over these (\cref{sec:poresh-jetcube-jcset}).
...
\section{Semantics on Jet Sets} \label{sec:poresh-jetgph}
...
\begin{definition}
	...
	\begin{enumerate}
		...
		\begin{itemize}
			...
			\item Sending a morphism $\mu : \vec a \to \vec b$ to $\jetgph{\mu} : \jetgph{\vec a} \to \jetgph{\vec b}$ where $\jetgph{\mu}$ (more briefly denoted as $\mu$) sends a jet set $X$ to the jet set $\mu X$ whose
			\begin{itemize}
			...
			\end{itemize}
			$\jetgph{\mu}$ is easily seen to have a functorial action on jet set morphisms which preserves identity and composition.
...
\end{proposition}
\begin{proof}
	Consider the following two mappings of the same jet set $X \in \jetgph{[\npolar, \ypolar]}$:
	\begin{align*}
		\xymatrix{
			& {[\npolar, \ypolar]} &
			&&& %-----------
	...
	\]
	In fact, if we directly apply the composite $\reshlist{\nonpol_0\pmrnn}{\mixpol_1\pmryn}$ to $X$, given the precise definition above, we obtain the jet set where $[a] \not\edge_0 [c]$.
\end{proof}
...
\begin{definition} \label{def:interval-eqty} \label{def:interval-top}
	We define $\rival{\edgesym_\eqtydeg}$ as the singleton jet set $\accol{0, 1}/\edgesym_\top$ (a quotient by the total relation)\footnote{The choice of singleton is especially relevant under the strict quotienting \cref{ax:bishop}.}, and $\rival{\topsym}$ as the discrete jet set $\accol{0 \quad 1}$.
\end{definition}
...
\begin{proposition} \label{thm:poresh-jetgph-prism-commut}
	...
	\]
\end{proposition}
Note that a cartesian product with $\rival{\eqtysym}$ or $\rival{\topsym}$ is the same as a separated product, since neither jet set has any non-reflexive jets.
...
\begin{proof}
	...
	We prove that $\mu(X \multip \rival{P_i}) \in \jetgph{\vec b}$ is isomorphic to the prescribed jet set.
	...
	\begin{enumerate}
		...
		Write $Y \in \jetgph{\vec b}$ for the jet set prescribed by the table.
...
For this reason, not unlike our usage of jet setoids in \cref{sec:poresh-jetgph}, we will take a detour via alternative categories that are equivalent to the categories of jet-topic sets but where our semantics do not act on adjoint triples as in \cref{thm:poresh-jtset:triple}, but rather on adjoint pairs (\cref{sec:poresh-jtset:pair-sin}).
That is:
\begin{itemize}
...
Since jet cubes are realized as jet sets by repeated application of twisted prism functors to the terminal jet set (\cref{def:jep}), we can deduce the action of $\mu$ on jet cubes from its action on twisted prisms, which is given by \cref{thm:poresh-jetgph-prism-commut}.
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\begin{itemize}
				...
				\end{itemize}
				\item It is the case that $\EP(\erasejc{\mu(\vfi)})$ is in fact a morphism of jet sets $\JEP(\mu V) \to \JEP(\mu W)$;
			...
			\begin{itemize}
				...
				\item This makes $\EP(\erasejc{\ket{\mu \leq \nu}})$ a jet set morphism, precisely because $\mu \leq \nu$, thus defining $\JEP(\ket{\mu \leq \nu})$ and completing also the definition of $\ket{\mu \leq \nu}$.
			\end{itemize}
			
			This definition is clearly natural on cubes, therefore on sets, and therefore (by faithfulness of $U : \jetgph{\vec a} \to \Set$) also on jet sets. By conclusion, it is natural in $\jetcube{\orifb}{\affine}{M}{\vec a}$.
		\end{itemize}
	\end{itemize}
\end{definition}
\begin{corollary}
