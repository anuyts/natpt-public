...
\begin{figure} \label{fig:calculus}
	...
	\[
		\inferencel{exchange}{
			\sez \vfi : V \to (W, \dm j : \rival{\edge_i}, U, \dm i : \rival{\edge_i})
		}{
			\sez \vfi : V \to (W, \dm i : \rival{\edge_i}, U, \dm j : \rival{\edge_i})
		}{}
	\]
	\[
...
Note that \cref{conv:sort-cubes} implies that all variables in $U$ are also of degree $i$.
...
\begin{theorem}[Completeness] \label{thm:calculus-complete}
	For any morphism $\hat \vfi : V \to W$ in $\jetcubeopt \affine M {\vec a}$, writing $\vfi = \erasejc{\hat \vfi}$, we have $\sez \vfi : V \to W$.
\end{theorem}
\begin{proof}
	...
	We proceed by inspecting $\dm i \psub \vfi$.
	By definition of $\cubecat_M^\affine$, it can be written as an affine boolean expression.
	After pushing down negations, eliminating double negations, and simplifying conjunctions and disjunctions with $0$ and $1$, we obtain an expression $e$ that is either $0$, $1$ or a binary tree with $\vee$ and $\wedge$ in the nodes and (negations of) variables of $V$ in the leaves.
	
	If [$\dm i : \rival{\jet_i}$ and $t = 0$] or [$\dm i : \rival{\tej_i}$ and $t = 1$], then $\vfi : V \to W = (U, \dm i : \_)$ being a jet cube morphism (equivalently: $\EP(\vfi)$ being a jet set morphism) is equivalent to $(\novar / \dm i) \circ \vfi : V \to \Opcube_i(U)$ being a jet cube morphism, so we can apply \ruleref{zero:fwd} or \ruleref{one:bck}.
	
	If [$\dm i : \rival{\jet_i}$ and $t = 1$] or [$\dm i : \rival{\tej_i}$ and $t = 0$], then $\vfi : V \to W = (U, \dm i : \_)$ being a jet cube morphism (equivalently: $\EP(\vfi)$ being a jet set morphism) is equivalent to $(\novar / \dm i) \circ \vfi : V \to U$ being a jet cube morphism, so we can apply \ruleref{one:fwd} or \ruleref{zero:bck}.
