...
\begin{definition}\label{def:orisymbol}
	...
	\[
		\orisymbol(\vec a) = \set{P_i}{P \in \orisymbol, 0 \leq i < \lenpol{\vec a}} \cup \accol{\eqty, \topsym},
	\]
	...
	\[
		\orisymbolfwd(\vec a) = \set{\jetsym_i}{0 \leq i < \lenpol{\vec a}} \cup \accol{\eqty} \subseteq \orisymbol(\vec a).
	\]
	(Note the removal of $\topsym$.)
	
	On $\orisymbol(\vec a)$, we define a partial order generated by
	\[
		\eqty \leq P_i \leq \topsym, \qquad
	...
	We write $P_i \ll Q_j$ if $\infrajetsym \cdot P_i \leq \equijetsym \cdot Q_j$, which is the case if either: $P_i = \eqty$, $Q_j = \topsym$, $i < j$ or if $i = j$ and either $P = \equijetsym$ or $Q = \infrajetsym$ (both of which are true if $a_i = \npolar$).
\end{definition}
\begin{definition}\label{def:poreshcat}
	...
	\]
	such that if $P_i < Q_j$ then $P_i \cdot \mu \ll Q_j \cdot \mu$, such that $\eqty \cdot \mu$ is symmetric and such that if $b_i = \npolar$ then $\edgesym_i \cdot \mu$ is symmetric.
	
	We also denote $\mu$ by explicitly listing its actions as
	\[
		\mu = \reshlist{\eqty \cdot \mu}{\jetsym_0 \cdot \mu, \ldots, \jetsym_{\lenpol{\vec b}-1} \cdot \mu}.
	\]
	
	The \textbf{identity} reshuffle $\id : \vec a \to \vec a$ is given by
	\[
		\id = \reshlist{\eqty}{\jetsym_0, \ldots, \jetsym_{\lenpol{\vec a}-1}},
	\]
...
\begin{theorem}\label{thm:poresh-adjoint}
	\begin{enumerate}
		...
		\begin{itemize}
			...
			\begin{align}
				&\eqty \cdot \rho = \eqty, \text{and} \label{eq:poresh-left-adjoint-crit1} \\
...
\begin{proof}
	\begin{enumerate}
		...
		\]
		Without loss of generality, we can instantiate both $P_i$ and $Q_j$ with the minimal element $\eqty$, so existence of a left adjoint requires that $\eqty \cdot \rho = \eqty$ (\cref{eq:poresh-left-adjoint-crit1}).
...
\begin{example} \label{ex:cohesion}
	...
	\[
		\cohfget_i = \reshlist{\eqty}{\jetsym_0, \ldots, \jetsym_{i-1}, \jetsym_{i+1}, \ldots, \jetsym_{n-1}} : \vec a \to \vec b
	\]
	\begin{itemize}
		...
		\begin{align*}
			...
			\dashv
			\cohdisc_0 &= \reshlist{\eqty}{\eqty, \jetsym_0, \ldots, \jetsym_{n-2}} &: \vec b \to \vec a, \\
			\dashv
			\cohfget_0 &= \reshlist{\eqty}{\jetsym_1, \ldots, \jetsym_{n-1}} &: \vec a \to \vec b.
		\end{align*}
		...
		\begin{align*}
			...
			\dashv
			\flat_0 = \cohdisc_0 \cohfget_0 &= \reshlist{\eqty}{\eqty, \jetsym_1, \ldots, \jetsym_{n-1}} : \vec a \to \vec a.
		\end{align*}
		\item If $i > 0$, then $\cohfget_i$ has a left adjoint $\cohdisc_i \dashv \cohfget_i$ where
		\[
			\cohdisc_i = \reshlist{\eqty}{\jetsym_0, \ldots, \jetsym_{i-1}, \infrajetsym_{i-1}, \jetsym_i, \ldots, \jetsym_{n-2}} : \vec b \to \vec a.
		\]
		We have $\cohfget_i \cohdisc_i = \id_{\vec b}$. We get an idempotent comonad
		\[
			\flat_i = \cohdisc_i \cohfget_i = \reshlist{\eqty}{\jetsym_0, \ldots, \jetsym_{i-1}, \infrajetsym_{i-1}, \jetsym_{i+1}, \ldots, \jetsym_{n-1}} : \vec a \to \vec a
		\]
		\begin{itemize}
			...
			\begin{align*}
				\cohpi_i &= \reshlist{\eqty}{\jetsym_0, \ldots, \jetsym_{i-2}, \infrajetsym_i, \jetsym_{i+1}, \ldots, \jetsym_{n-1}}
				&: \vec a \to \vec b, \\
				\dashv
				\cohdisc_i &= \reshlist{\eqty}{\jetsym_0, \ldots, \jetsym_{i-2}, \edgesym_{i-1}, \edgesym_{i-1}, \jetsym_i, \ldots, \jetsym_{n-2}}
				&: \vec b \to \vec a, \\
				\dashv
				\cohfget_i &= \reshlist{\eqty}{\jetsym_0, \ldots, \jetsym_{i-2}, \edgesym_{i-1}, \jetsym_{i+1}, \ldots, \jetsym_{n-1}}
				&: \vec a \to \vec b.
			\end{align*}
			We have $\cohpi_i \cohdisc_i = \cohfget_i \cohdisc_i = \id_{\vec b}$. We get adjoint idempotent (co)monads
			\begin{align*}
				\shp_i = \cohdisc_i \cohpi_i &= \reshlist{\eqty}{\jetsym_0, \ldots, \jetsym_{i-2}, \infrajetsym_i, \infrajetsym_i, \jetsym_{i+1}, \ldots, \jetsym_{n-1}} &: \vec a \to \vec a, \\
				\dashv
				\flat_i = \cohdisc_i \cohfget_i &= \reshlist{\eqty}{\jetsym_0, \ldots, \edgesym_{i-1}, \edgesym_{i-1}, \jetsym_{i+1}, \ldots, \jetsym_{n-1}} &: \vec a \to \vec a
			\end{align*}
			\begin{itemize}
		...
		\begin{align*}
			\cohfget_{n-1} &= \reshlist{\eqty}{\jetsym_0, \ldots, \jetsym_{n-2}} &: \vec a \to \vec b, \\
			\dashv
			\cohcodisc_{n-1} &= \reshlist{\eqty}{\jetsym_0, \ldots, \jetsym_{n-2}, \topsym} &: \vec b \to \vec a,
		\end{align*}
		and there are no further right adjoints.
		We have $\cohfget_{n-1} \cohcodisc_{n-1} = \id_{\vec b}$.
		We get an idempotent monad
		\[
			\sharp_{n-1} = \cohcodisc_{n-1} \cohfget_{n-1} = \reshlist{\eqty}{\jetsym_0, \ldots, \jetsym_{n-2}, \topsym} : \vec a \to \vec a.
		\]
		\item If $i < n-1$, then $\cohfget_i$ has a right adjoint $\cohfget_i \dashv \cohcodisc_i$ with
		\[
			\cohcodisc_i = \reshlist{\eqty}{\jetsym_0, \ldots, \jetsym_{i-1}, \equijetsym_{i}, \jetsym_{i}, \ldots, \jetsym_{n-2}} : \vec b \to \vec a.
		\]
		We have $\cohfget_i \cohcodisc_i = \id_{\vec b}$. We get an idempotent monad
		\[
			\sharp_i = \cohcodisc_i \cohfget_i = \reshlist{\eqty}{\jetsym_0, \ldots, \jetsym_{i-1}, \equijetsym_{i+1}, \jetsym_{i+1}, \ldots, \jetsym_{n-1}} : \vec a \to \vec a.
		\]
		\begin{itemize}
			...
			\begin{align*}
				\cohfget_i &= \reshlist{\eqty}{\jetsym_0, \ldots, \jetsym_{i-1}, \edgesym_{i+1}, \jetsym_{i+2}, \ldots, \jetsym_{n-1}} &: \vec a \to \vec b, \\
				\dashv
				\cohcodisc_i &= \reshlist{\eqty}{\jetsym_0, \ldots, \jetsym_{i-1}, \edgesym_{i}, \edgesym_{i}, \jetsym_{i+1}, \ldots, \jetsym_{n-2}} &: \vec b \to \vec a, \\
				\dashv
				\cohcopi_i &= \reshlist{\eqty}{\jetsym_0, \ldots, \jetsym_{i-1}, \equijet_i, \jetsym_{i+2}, \ldots, \jetsym_{n-1}} &: \vec a \to \vec b.
			\end{align*}
			We have $\cohfget_i \cohcodisc_i = \cohcopi_i \cohcodisc_i = \id_{\vec b}$. We get adjoint idempotent (co)monads
			\begin{align*}
				\sharp_i = \cohcodisc_i \cohfget_i &= \reshlist{\eqty}{\jetsym_0, \ldots, \jetsym_{i-1}, \edgesym_{i+1}, \edgesym_{i+1}, \jetsym_{i+2}, \ldots, \jetsym_{n-1}} &: \vec a \to \vec a, \\
				\dashv
				\coshp_i = \cohcodisc_i \cohcopi_i &= \reshlist{\eqty}{\jetsym_0, \ldots, \jetsym_{i-1}, \equijet_{i}, \equijet_{i}, \jetsym_{i+2}, \ldots, \jetsym_{n-1}} &: \vec a \to \vec a.
			\end{align*}
			\begin{itemize}
...
\begin{example} \label{ex:pjb-paramty}
	...
	\begin{align*}
		...
		\dashv
		\cohdisc_0^{\maskopfont{PJB}} &= \reshlist{\eqty}{\eqty, \eqty, \eqty, \nwpathsym_0, \nwjetsym_0, \nwbridgesym_0} &: \vec b \to \vec a, \\
		\dashv
		\cohpi{}_0^{\maskopfont{JB};}{}_1^{\maskopfont{P}} =
		\cohfget_0^{\maskopfont{PJB}} &= \reshlist{\eqty}{\nwpathsym_1, \nwjetsym_1, \nwbridgesym_1} &: \vec a \to \vec b, \\
		\dashv
		\cohdisc{}_0^{\maskopfont{JB};}{}_1^{\maskopfont{P}} =
		\cohcodisc_0^{\maskopfont{PJB}} &= \reshlist{\eqty}{\nwpathsym_0, \nwpathsym_0, \nwpathsym_0, \nwpathsym_0, \nwjetsym_0, \nwbridgesym_0} &: \vec b \to \vec a, \\
		\dashv
...
%		\dashv
%		\cohdisc_0 &= \reshlist{\eqty}{\eqty, \eqty, \eqty, \nwrodsym_0, \nwpathsym_0, \nwjetsym_0} &: \vec b \to \vec a, \\
%		\dashv
%		\cohfget_0 &= \reshlist{\eqty}{\nwrodsym_1, \nwpathsym_1, \nwjetsym_1} &: \vec a \to \vec b, \\
%		\dashv
%		\cohcodisc_0 &= \reshlist{\eqty}{\nwrodsym_0, \nwrodsym_0, \nwrodsym_0, \nwrodsym_0, \nwpathsym_0, \nwjetsym_0} &: \vec b \to \vec a, \\
%		&\dashv
%		\reshlist{\eqty}{\nwrodsym_0, \nwpathsym_1, \nwjetsym_1} &: \vec a \to \vec b, \\
%		&\dashv
...
\begin{definition} \label{def:poresh-jetset}
	We define a pseudofunctor $\jetsetsym : \poreshcat \to \Cat$:
	\begin{itemize}
		...
		\begin{itemize}
			\item carrier is the carrier of $X$, quotiented out by the transitive closure of $\eqty \cdot \mu$ in case $\eqty \cdot \mu \neq \eqty$,
			\item $\hat x \jet_i^{\mu X} \hat y$ if and only if $\hat x$ and $\hat y$ have representants $x$ and $y$ related by $\jetsym_i \cdot \mu$ in $X$.
			This is in turn equivalent to saying that all representants of $\hat x$ and $\hat y$ are related by $(\eqty \cdot \mu)^* \otimes (\jetsym_i \cdot \mu) \otimes (\eqty \cdot \mu)^*$ in $X$, where $\otimes$ is composition of relations via existential quantification.
		\end{itemize}
		This is easily seen to be a functor.
		2-functoriality is not strict due to the quotienting, but pseudofunctoriality is not endangered.
		\item Sending a unique proof of inequality $\mu \leq \nu : \vec a \to \vec b$ to an obvious natural transformation $\ket{\mu \leq \nu} : \mu \to \nu : \jetset{\vec a} \to \jetset{\vec b}$.
	\end{itemize}
	By a manual intervention, we decided not to quotient if $\eqty \cdot \mu = \eqty$.
	Thus, by \cref{thm:poresh-adjoint}, the restriction of $\jetsetsym$ to $\poreshcat\withl$ is a strict 2-functor as it does not quotient at all.
\end{definition}
\begin{corollary} \label{thm:poresh-jetset:bind}
	Let $\mu : \vec a \to \vec b$ and $X \in \jetset{\vec a}$.
	For an arbitrary $Q_j \in \orisymbol(\vec b)$, we have that $\hat x \mathrel{Q_j}^{\mu X} \hat y$ if and only if $\hat x$ and $\hat y$ have representants $x$ and $y$ related by $Q_j \cdot \mu$. (Here, we interpret $\eqty$ as equality and $\topsym$ as the relation `true'.) \qed
\end{corollary}
\begin{example} \label{ex:poresh-op-jetset}
	The functor $\Op_i$ from \cref{def:op} arises from the reshuffle in \cref{ex:poresh-op} as $\jetset{\Op_i}$.
\end{example}
\begin{example} \label{ex:poresh-sym-jetset}
	The functors $\Fsym_i \dashv \Usym_i \dashv \Csym_i$ and $\UFsym_i \dashv \UCsym_i$ from \cref{def:sym-forget,def:sym-free,def:sym-cofree} arise from the reshuffles in \cref{ex:poresh-sym} as $\jetset{\Fsymresh_i} \dashv \jetset{\Usymresh_i} \dashv \jetset{\Csymresh_i}$ and $\jetset{\UFsymresh_i} \dashv \jetset{\UCsymresh_i}$.
\end{example}
...
\begin{proposition} \label{thm:poresh-jetset-intervals}
	...
	\begin{equation}
		\mu \rival{\jetsym_i} \cong \rival{\jetsym_i \cdot \rho}, \qquad
		\mu \rival{\tejsym_i} \cong \rival{\tejsym_i \cdot \rho}, \qquad
		\mu \rival{\equijetsym_i} \cong \rival{\equijetsym_i \cdot \rho},
		\label{eq:poresh-jetset-intervals}
	\end{equation}
	where we define $\rival{\topsym}$ as the jet set with carrier $\accol{0, 1}$ on which all $i$-jet relations are reflexive, and $\rival{\eqty}$ as the terminal jet set.%
	\footnote{By virtue of \cref{thm:poresh-adjoint}, we do not have to define $\rival{\infrajet_j}$ for $b_j = \ypolar$.}
\end{proposition}
\begin{proof}
	Suppose $\jetsym_i \cdot \rho = \eqty$. Then each of the right hand sides in \cref{eq:poresh-jetset-intervals} is the terminal jet set.
	From \cref{thm:poresh-adjoint}, we know that $\eqty \cdot \mu \geq \jetsym_i$ and therefore, by symmetry of $\eqty \cdot \mu$, that $\eqty \cdot \mu \geq \infrajetsym_i$.
	Then $0$ and $1$ are related by $\eqty \cdot \mu$ in $\rival{\jetsym_i}$, $\rival{\tejsym_i}$ and $\rival{\equijetsym_i}$ and therefore identified in the jet sets obtained by applying $\mu$ to the left hand sides of \cref{eq:poresh-jetset-intervals}.
	
	Suppose $\jetsym_i \cdot \rho > \eqty$. Then the right hand sides of \cref{eq:poresh-jetset-intervals} have carrier $\accol{0, 1}$.
	From \cref{thm:poresh-adjoint}, we know that $\eqty \cdot \mu < \jetsym_i$ and therefore, by symmetry of $\eqty \cdot \mu$, that $\eqty \cdot \mu < \equijetsym_i$.
	Then $0$ and $1$ are \emph{not} related by $\eqty \cdot \mu$ in $\rival{\jetsym_i}$, $\rival{\tejsym_i}$ and $\rival{\equijetsym_i}$ and therefore \emph{not} identified in the jet sets obtained by applying $\mu$ to the left hand sides of \cref{eq:poresh-jetset-intervals}.
	Thus, the carriers of the left hand sides are either $\accol{0, 1}$ or that same set quotiented by the equality relation.

	\todoi{Prove this}
\end{proof}
\begin{proposition} \label{thm:poresh-jetset-prism-commut}
	Let $\mu : \vec a \to \vec b$ be a polarized reshuffle with left adjoint $\kappa \dashv \mu$. Then we have
	\begin{align*}
		\jetset{\mu} \circ (\loch \multip \rival{\jetsym_i}) &= (\loch \multip \rival{\jetsym_i \cdot \kappa}) \circ \jetset{\mu},
		\\
		\jetset{\mu} \circ (\loch \multip \rival{\tejsym_i}) &= (\loch \multip \rival{\tejsym_i \cdot \kappa}) \circ \jetset{\mu}.
	\end{align*}
\end{proposition}
\begin{proof}
	\todoi{Prove this. The RHS is not even necessarily well-defined.}
\end{proof}
