...
\subsection{Definitions} \label{sec:poreshcat:def}
...
\begin{definition} \label{def:polarity-cat}
	...
	\begin{itemize}
		...
		We call these polarities: $\isopol$ isopolar, $\pospol$ positive, $\negpol$ negative, $\mixpol$ mixed-polar and $\nonpol$ non-polar.
	\end{itemize}
\end{definition}
...
\begin{proposition}
	...
	\begin{itemize}
		\item If any of the operands is either $\isopol$ or $\mixpol$, then the result is $\isopol$ or $\mixpol$, whichever occurs most to the right.
...
\begin{definition}
	...
	We define a transformation, extranatural in $b$ (\cref{thm:ori-poldeg-extranat}):
	\begin{equation}
...
\begin{theorem}
	...
	\begin{itemize}
		\item sends the object (the `depth') $d \geq -1$ to the anpolarity mask $I(d) = [\npolar]^{d+1}$,
...
For us, pro-arrows will live at a different degree than arrows, and our \emph{polarities} (as opposed to polarized reshuffles) are concerned with only a single degree.
...
Thus, we stick to the equality $\isopol \circ \mixpol = \mixpol$.

\todoi{Are you swapping $\isopol$ and $\mixpol$????}

\todoi{Spell out}
\begin{itemize}
	\item Simplification if everything is symmetric (reshuffles)
	\item Simplification if everything is directed. ($\Hom_\polcat(\ypolar, \ypolar)$)
	\item Discuss $\Hom_\poreshcat([\ypolar], [\ypolar])$.
\end{itemize}
