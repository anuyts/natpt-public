...
\begin{definition} \label{def:jet-jewel}
	...
	An $\vec a$-jet-set $X$ is a \textbf{jet jewel} if it is:
	\begin{enumerate}
		\item finite,
		\item $(n-1)$-connected,
...
\end{definition}
The conceptual reason for asking $i$-antisymmetry is to exclude the equijet interval $\rival{\equijetsym_i}$, so that the only jet jewel intervals at directed degrees are $\rival{\jetsym_i}$ and $\rival{\tejsym_i}$.
...
\begin{proof}
	...
	We prove the other properties in the definition of a jet jewel:
	\begin{description}
...
%		\item[$\UCsym_{i \neq n-1}$] Since $\UCsym_i = \Usym_i \circ \Csym_i$.
		...
		\end{itemize}
		For $j$-orientability, $j$-acyclicity and $j$-antisymmetry, we consider 3 situations:
		\begin{description}
			...
			For $j$-acyclicity, note that $(x, u) \jet_j^* (y, v)$ if and only if $x \jet_j^* y$ and $(x, u) \equijet_j^* (y, v)$ if and only if $x \equijet_j^* y$. Thus, $j$-acyclicity is also inherited from $X$.
			...
			For $i$-antisymmetry, $(x, u) \equijet_i (y, v)$ implies $u = v$, so this is also inherited from $i$-antisymmetry of $X$.
		\end{description}
