...
In \cref{sec:jetcubes:def}, we define a family of \emph{jet cube} categories $\jetcube{\myorix}{\mycubex}{M}{\vec a}$ furthermore parametrized by a mask $\vec a$ and a choice $\myorix \in \accol{\fwdori, \triori}$ of orientation: forward or trioriented.
...
\begin{definition} \label{def:decr-jet-cube-cat}
	...
	For $\myorix \in \accol{\fwdori, \triori}$ and $\mycubex \in \accol{\affine, \cartes}$, we write $\decrjetcube{\myorix}{\mycubex}{M}{\vec a}$ for the full subcategory of $\jetcube{\myorix}{\mycubex}{M}{\vec a}$ on decreasing cubes.
\end{definition}
\begin{corollary} \label{thm:decr-jet-cube-cat}
	By \cref{thm:cube-commute-diff-degree}, the inclusion is essentially surjective (in fact split essentially surjective by the existence of sorting algorithms) and thus an equivalence of categories. \qedhere
\end{corollary}
...
\begin{definition} \label{def:calculus}
	For $M \in \accol{\bipointedsym, \booleanalg}$, any mask $\vec a$ and for any two objects $V, W \in \Obj(\decrjetcubetri \affine M {\vec a})$, we define a proof-irrelevant predicate on morphisms $\vfi : \erasejc V \to \erasejc W$, denoted $\sez \vfi : V \to W$, inductively generated by the inference rules in \cref{fig:calculus}.
\end{definition}
...
Note that decreasingness implies that all variables in $U_1$ are also of type $\rival{\equijet_i}$.
...
In either case, it turns out that whether the last term reduces to $0$ or $1$ on any end point of the source cube, is sufficiently irregular that the remainder of $\vfi$ can only depend on $i$-symmetric variables.
...
\begin{proof}
	...
	\begin{itemize}
		...
		(Note that, since we are dealing with decreasing cubes, all variables in $U_1$ also have type $\rival{\equijet_i}$.)
...
%			Either way, we get $\vec v \jet_j \vec v'$ in $\JEP(\UFsymcube_i V)$ and therefore $f(\vec v) \jet_j f(\vec v')$ in $\JEP(W)$ where $f = \EP(\vfi)$.
		...
		\item Soundness of \ruleref{conn:prism:tgt-neutral} is proven analogous to that of \ruleref{conn:prism:src-neutral}, adapting the highlighted parts.
