...
%	Before that, we put, if existent, the sole variable $\dm k_2$ such that $e_{\dm k_2}$ mentions the second to last variable of degree $i$ in $V$. Etc.
	...
	\begin{itemize}
		...
		If $\dm i : \rival{\tej_i}$, we apply \ruleref{inv:fwd} and do not resort to the induction hypothesis but proceed below.
		We now assume that $\dm i : \rival{\jet_i}$.
		\begin{itemize}
			\item Assume $a_i = \ypolar$ and $V$ contains non-equijet variables of degree $i$.
			Let $\dm k$ be the last variable of $V$.
			Then we show that $e_{\dm i}$ is of one of the following forms:
			\begin{itemize}
				\item $s \wedge t$ or $s \vee t$ where $s$ does not mention non-equijet variables of degree $i$,
				\item $\dm k' \wedge t$ or $\dm k' \vee t$ where $\dm k' = \dm k$ if $\dm k : \rival{\jet_i}$ and $\dm k' = \lnot \dm k$ if $\dm k : \rival{\tej_i}$.
			\end{itemize}
			\todoi{Show this}
