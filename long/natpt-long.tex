...
\begin{corollary} \label{thm:setoid-equiv-set}
	The \emph{setoid-enriched} quotient functor $\setoidquot : \Setoid \to \setoiddisc_1 \Set$ and the discrete functor $\setoiddisc : \setoiddisc_1 \Set \to \Setoid$ form an equivalence of $\Setoid$-enriched categories (where $\setoiddisc \setoidquot$ is only naturally \emph{equivalent}, not naturally isomorphic, to the identity functor). \qed
\end{corollary}
...
\begin{corollary} \label{thm:jetsetoid-equiv-jetset}
	The functors $\setoidquot$ and $\setoiddisc$ and the equivalence from \cref{thm:setoid-equiv-set} carry over to an equivalence of setoid-enriched categories $\jetsetoid{\vec a} \simeq \setoiddisc_1 \jetset{\vec a}$. \qed
\end{corollary}
\begin{definition}
...
\begin{proof}
	...
	We compute the sets of 0-jets of its images under either the above functor or its square:\todo{Specify presheaf and (co)end notations somewhere.}
	\begin{align*}
...
\begin{corollary} \label{thm:jet-cuboid-pullback}
	...
	Moreover, the isomorphisms in \cref{def:cuboid-ep} and \cref{def:jet-cuboid-jep} are compatible along $U : \jetset{\vec a} \to \Set$ and $\erasejc \loch$.
\end{corollary}
...
\subsubsection{Semantics on Jet Cuboids} \label{sec:poresh-jetcuboid}
\begin{definition}
	We define a strict 2-functor $\jetcuboid{\orifb}{\affine}{M}{\loch} : \poreshcat\withrr \to \Cat$ which sends:
	\begin{itemize}
...
\subsubsection{Semantics on Jet Cubes} \label{sec:poresh-jetcube:sub}
\todoi{Use $\setoidquot$}
\todoi{Also allow dropping the backward morphisms.}
...
%\subsection{Adjoint Pair Semantics on Sinistrified Modal Jet Cubical Sets} \label{sec:poresh-jcset:pair-sin}
%
%\subsection{Adjoint Pair Semantics on Jet Cubical Sets} \label{sec:poresh-jcset:pair}
