...
\begin{proposition} \label{thm:lift-to-jet-cubes}
	...
	We have $\Fsymcube_i \dashv \Usymcube_i$ and thus an idempotent monad $\UFsymcube_i := \Usymcube_i \circ \Fsymcube_i$.
\end{proposition}
\begin{proof}
...
\begin{convention}
	When presenting the calculus, with permission from \cref{thm:cube-commute}, we will order the dimensions of an opposable jet cube by \emph{decreasing} degree.
\end{convention}
\begin{remark}
...
Symmetrization (needed when you encounter $\rival{\equijet_i}$):
\[
	\inference{
		\vfi : \Fsymcube_i V \to W
	}{
		\vfi : V \to \Usymcube_i W
	}{}
\]
...
\[
	\inference{
		\vfi : \UFsymcube_i(\UFsymcube_j U, V) \to W
	}{
...
Forward disjunction and backward conjunction:\footnote{It may be helpful to note that the start of the Hamiltonian path of a forward jet cube always has coordinates $(1, \ldots, 1, 0)$.}
\[
	\inference{
...
\todoi{Use rule names (zero:bck etc)}
\todoi{Soundness}
\todoi{Completeness}
