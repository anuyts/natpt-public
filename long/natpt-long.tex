...
\newcommand{\thetitle}{Models of Naturality Pretype Theory: \\ Technical Report \\ v0.3}
\newcommand{\theauthors}{Andreas Nuyts\thanks{Andreas Nuyts holds a Postdoctoral Fellowship from the Research Foundation - Flanders (FWO; 1247922N, 12AB225N). This research is partially funded by the Research Fund KU Leuven.} \\ DistriNet, KU Leuven, Belgium}
\begin{document}
...
\chapter*{Metatheory} \label{ch:metatheory}
We work in a constructive metatheory (e.g.\ type theory) with propositional extensionality.
We assume a sufficiently high tower of universes, in the style of Grothendieck's axiom of universes or a type theoretic adaptation of it. \todoi{Maybe we can be more precise when the report is done?}

\section*{Strict Quotienting Axiom}
We express the following axiom, which we will \emph{not} assume by default, and which is validated by a setoid model:
\begin{axiom}[Strict quotienting axiom] \label{ax:bishop}
	\begin{enumerate}
		\item Let $X$ be a set. Then $X$ and its trivial quotient $X/{=}_X$ are \emph{equal}.
		\item Let $X$ be a set, and $R$ and $S$ an equivalence relations on $X$ such that $R \Rightarrow S$.
			Denote $S/R$ for the equivalence relation on $X/R$ such that $(S/R)([x]_R, [y]_R) \Leftrightarrow S(x, y)$.
			Then the sets $X/S$ and $(X/R)/(S/R)$ are \emph{equal}.
	\end{enumerate}
\end{axiom}
...
\subsection{On Adjoints}
\begin{definition}
	Let $\catC$ be a 2-category.
	Morphisms $\lambda : \Hom_\catC(y, x)$ and $\rho : \Hom_\catC(x, y)$ are \textbf{adjoint} ($\lambda \dashv \rho$) if and only if they are equipped with a \textbf{unit} 2-cell $\eta : \id_y \to \rho \lambda$ and a \textbf{co-unit} 2-cell $\eps : \lambda \rho \to \id_x$ such that
	\[
		(\eps \star \lambda) \circ (\lambda \star \eta) = \id_\lambda, \qquad
		(\rho \star \eps) \circ (\eta \star \rho) = \id_\rho.
	\]
\end{definition}
In the 2-category $\Cat$, this corresponds to the usual notion of adjoint functors.

\begin{corollary}
	2-functors and pseudofunctors preserve adjunctions.
\end{corollary}
\begin{proposition}
	Let $\catC$ be a 2-category, $\lambda \in \Hom_\catC(y, x)$ and $\rho \in \Hom_\catC(x, y)$. The following adjunctions are equivalent, where we denote morphisms in $\catC\op$ and 2-cells in $\catC\co$ with a bar:
	\[
		\begin{array}{l | c c c l | c c r c r | c c r c l}
			\text{in cat.}
			& \multicolumn{4}{c |}{\text{adjunction}}
			& \multicolumn{5}{c |}{\text{unit}}
			& \multicolumn{5}{c}{\text{co-unit}}
			\\ \hline
			\catC
			& \lambda &\dashv& \rho &: x \to y
			& \eta &:& \id_y &\to& \rho \lambda
			& \eps &:& \lambda \rho &\to& \id_x
			\\
			\catC\op
			& \overline \rho &\dashv& \overline \lambda &: x \to y
			& \eta &:& \id_y &\to& \overline \lambda \overline \rho = \overline{\rho \lambda}
			& \eps &:& \overline \rho \overline \lambda = \overline{\lambda \rho} &\to& \id_x
			\\
			\catC\co
			& \rho &\dashv& \lambda &: y \to x
			& \overline \eps &:& \id_x &\to& \lambda \rho
			& \overline \eta &:& \rho \lambda &\to& \id_y
			\\
			\catC\coop
			& \overline \lambda &\dashv& \overline \rho &: y \to x
			& \overline \eps &:& \id_x &\to& \overline \rho \overline \lambda = \overline{\lambda \rho}
			& \overline \eta &:& \overline \lambda \overline \rho = \overline{\rho \lambda} &\to& \id_y.
		\end{array}
	\]
	This also indicates how 2-functors and pseudofunctors of different variance affect adjunctions. \qed
\end{proposition}
\begin{corollary} \label{thm:adjunctions-under-op}
	In a 2-category $\catC$, assume we have an adjunction
	\[
		\lambda : \Hom_\catC(y, x)
		\qquad \dashv \qquad
		\rho : \Hom_\catC(x, y).
	\]
	\begin{enumerate}
		\item For a 2-functor $P : \catC\op \to \Cat$, i.e.\ a $\Cat$-valued presheaf, we get:
		\[
			P(\rho) : P(x) \to P(y)
			\qquad \dashv \qquad
			P(\lambda) : P(y) \to P(x)
		\]
		with unit $P(\eta)$ and co-unit $P(\eps)$.
		
		\item In particular, if $z \in \Obj(\catC)$, this is true for the representable presheaf $\Hom_\catC(\loch, z) : \catC\op \to \Cat$:
		\[
			(\loch \circ \rho) : \Hom_\catC(y, z) \to \Hom_\catC(x, z)
			\qquad \dashv \qquad
			(\loch \circ \lambda) : \Hom_\catC(x, z) \to \Hom_\catC(y, z)
		\]
		with unit $(\loch \star \eta)$ and co-unit $(\loch \star \eps)$.
		
		As an adjunction in $\Cat$, it can be characterized as a natural isomorphism of $\Hom$-sets, in this case sets of 2-cells:
		\[
			\Hom_{\Hom_\catC(x, z)}(\vfi \circ \rho, \chi) \cong
			\Hom_{\Hom_\catC(y, z)}(\vfi, \chi \circ \lambda),
		\]
		naturally in $\vfi : \Hom_\catC(y, z)$ and $\chi : \Hom_\catC(x, z)$.
		
		\item In particular, if $\catC = \Cat$ and $L \dashv R : \cat X \to \cat Y$, we get a 1-to-1-correspondence of natural transformations:
		\[
			FR \to G \qquad \text{and} \qquad F \to GL,
		\]
		naturally in $F : \cat Y \to \cat Z$ and $G : \cat X \to \cat Z$.
		\qed
	\end{enumerate}
\end{corollary}

\section{Reindexing Limits and Colimits}
\begin{proposition} \label{thm:reindex-lim}
	Assume an adjunction $L \dashv R : \catC \to \catD$, and a functor $F : \catC \to \catE$. Then we have:
	\[
		\lim_{c \in \catC} Fc \cong \lim_{d \in \catD} FLd,
	\]
	if either limit exists.
\end{proposition}
\begin{proof}
	We show that they have the same inward morphisms and therefore satisfy the same universal property.
	Write $K_e : \catD \to \catE$ for the constant functor on $e$.
	We have a chain of isomorphisms, natural in $e \in \Obj(\catE)$:
	\begin{align*}
		&{} \mathrel{\phantom{\quad\cong\quad}}
		\Hom_\catE(e, \lim_{c \in \catC} Fc)
		\\ &{} \quad\cong\quad
		\forallend c \in \catC . \Hom_\catE(e, Fc)
		& \text{(universal prop.)}
		\\ &{} \quad=\quad
		\forallend c \in \catC . \Hom_\catE(K_e Rc, Fc)
		\\ &{} \quad\cong\quad
		\forallend d \in \catD . \Hom_\catE(K_e d, FLd)
		& \text{(\cref{thm:adjunctions-under-op})}
		\\ &{} \quad=\quad
		\forallend d \in \catD . \Hom_\catE(e, FLd)
		\\ &{} \quad\cong\quad
		\Hom_\catE(e, \lim_{d \in \catD} FLd). &\text{(universal prop.) } \qedhere
	\end{align*}
\end{proof}
\begin{proposition} \label{thm:reindex-lim}\label{thm:reindex-colim}
	Assume an adjunction $L \dashv R : \catC \to \catD$, and a functor $G : \catD \to \catE$. Then we have:
	\[
		\colim_{d \in \catC} Gd \cong \colim_{c \in \catC} GRd,
	\]
	if either colimit exists.
\end{proposition}
\begin{proof}
	We show that they have the same outward morphisms and therefore satisfy the same universal property.
	Write $K_e : \catC \to \catE$ for the constant functor on $e$.
	We have a chain of isomorphisms, natural in $e \in \Obj(\catE)$:
	\begin{align*}
		&{} \mathrel{\phantom{\quad\cong\quad}}
		\Hom_\catE(\colim_{d \in \catD} Gd, e)
		\\ &{} \quad\cong\quad
		\forallend d \in \catD . \Hom_\catE(Gd, e)
		& \text{(universal prop.)}
		\\ &{} \quad=\quad
		\forallend d \in \catD . \Hom_\catE(Gd, K_e Ld)
		\\ &{} \quad\cong\quad
		\forallend c \in \catC . \Hom_\catE(GRc, K_e c)
		& \text{(\cref{thm:adjunctions-under-op})}
		\\ &{} \quad=\quad
		\forallend c \in \catC . \Hom_\catE(GRc, e)
		\\ &{} \quad\cong\quad
		\Hom_\catE(\colim_{c \in \catC} GRc, e). &\text{(universal prop.) } \qedhere
	\end{align*}
\end{proof}

\section{Presheaf Notations}
\label{sec:notations}
\label{sec:notations:psh}
...
%\todoi{It's only used in one place, so its feasible to put end/coend on top still.}
...
We propose two families of base categories, parametrized by the mask $\vec a$, and both are built using the category $\jetgph{\vec a}$ of \textbf{jet graphs} over $\vec a$ (\cref{sec:jetgph}).
Jet graphs themselves are not suitable as a base category, e.g.\ because they are closed under coproducts, which the Yoneda-embedding will never preserve.
...
For these reasons, we alternatively propose to use categories of \textbf{jet topes} $\jettope{\vec a}$ (\cref{sec:jettope}). These are full subcategories of $\jetgph{\vec a}$ on objects that satisfy certain somewhat arbitrary well-behavedness criteria.
...
\section{Jet Graphs} \label{sec:jetgph}
\subsection{Definitions} \label{sec:jetgph:def}
\begin{definition} \label{def:jetgph}
	Let $\vec a$ be a mask. An \textbf{$\vec a$-jet-graph} is a set $X$ equipped with $\lenpol{\vec a}$ (decidable, proof-irrelevant) reflexive relations $\jet_i$ where
	\begin{itemize}
	...
	\end{itemize}
	A \textbf{morphism} of $\vec a$-jet-graphs is a function that preserves all the jet and edge relations.
	
	The category of $\vec a$-jet-graphs is called $\jetgph{\vec a}$.
\end{definition}
...
\end{notation}
\begin{definition} \label{def:full-jetgphhom}
	A jet graph morphism is called \textbf{full} if it reflects all jet relations.
\end{definition}
\begin{definition} \label{def:jet-surjective}
	A jet graph morphism $f : X \to Y$ is called \textbf{jet-surjective} if it is surjective as a function, and moreover for any $\vec y \jet_j \vec y'$ in $Y$, there exist $\vec x \jet_j \vec x'$ in $X$ such that $f(\vec x) = \vec y$ and $f(\vec x') = \vec y'$. 
\end{definition}
\begin{definition} \label{def:transitive-jetgph}
	A jet graph is called \textbf{transitive} if each of the $i$-jet relations is transitive (i.e. a pre-order and, if $i = \npolar$, an equivalence relation).
\end{definition}
The following proposition will not really be used directly, but is a nice encouragement:
\begin{proposition} \label{thm:jetgph-equipment}
	Let $X$ be a transitive $\vec a$-jet-graph and $0 \leq i < j < \lenpol{\vec a}$.
...
\begin{definition} \label{def:op}
	Let $\vec a$ be a mask, $i < \lenpol{\vec a}$ and $X \in \Obj(\jetgph{\vec a})$.
	We define the \textbf{$i$-opposite $\Op_i(X)$ of $X$} as the jet graph with the same carrier and relations as $X$ except that the $i$-jet relation is reversed: $x \jet_i^{\Op_i(X)} y$ if and only if $x \tej_i^X y$.
	This defines a functor $\Op_i(X) : \jetgph{\vec a} \to \jetgph{\vec a}$.
...
\begin{definition}
	...
	If $\vec a \sqsubset_i \vec b$, then we write $\Usym_i : \jetgph{\vec a} \to \jetgph{\vec b}$ for the forgetful functor which forgets the symmetry of $\jet_i$.
\end{definition}
\begin{proposition} \label{def:sym-free} \label{def:sym-cofree}
	The forgetful functor $\Usym_i$ is part of an adjoint triple $\Fsym_i \dashv \Usym_i \dashv \Csym_i$ where $\Fsym_i$ and $\Csym_i$ take a $\vec b$-jet-graph $X$ to a $\vec a$-jet-graph of the same carrier with the same $j$-jet relations for $j \neq i$ but where
	\begin{itemize}
...
\begin{definition} \label{def:sym-closure-of-sym}
	We extend the definition of $\UFsym_i$ and $\UCsym_i$ to endofunctors on $\jetgph{\vec b}$ where $b_i$ can be \emph{any} polarmode:
	\begin{itemize}
...
\begin{definition} \label{def:interval}
	...
	\begin{itemize}
		\item The \textbf{$i$-jet interval} $\rival{\jet_i}$ is defined as the $\vec a$-jet-graph with carrier $\accol{0, 1}$ and relations generated by $0 \jet_i^{a_i} 1$.
		\item The \textbf{opposite $i$-jet interval} $\rival{\tej_i}$ is defined as the $\vec a$-jet-graph with carrier $\accol{0, 1}$ and relations generated by $0 \tej_i^{a_i} 1$.
		\item The \textbf{$i$-equijet interval} $\rival{\equijet_i}$ is defined as the $\vec a$-jet-graph with carrier $\accol{0, 1}$ and relations generated by $0 \equijet_i^{a_i} 1$.
	\end{itemize}
...
\begin{definition} \label{def:prism}
	Let $\vec a$ be a mask, $i \in \Deg(\vec a)$ and $X \in \Obj(\jetgph{\vec a})$.
	We define the \textbf{$i$-twisted-prism $X \multip \rival{\jet_i}$} on $X$ as the $\vec a$-jet-graph with
	\begin{itemize}
		\item carrier $X \times \accol{0, 1}$,
		\item jet relations generated by the following requirements:
		\begin{itemize}
			\item $(\loch, 0) : \Op_i\pmr{a_i}(X) \to X \multip \rival{\jet_i\pmr{a_i}}$ is a jet graph morphism,
			\item $(\loch, 1) : X \to X \multip \rival{\jet_i\pmr{a_i}}$ is a jet graph morphism,
			\item $(x, 0) \jet_i\pmr{a_i} (x, 1)$ for all $x \in X$.
		\end{itemize}
	\end{itemize}
	This defines the \textbf{$i$-twisted-prism functor} $\loch \multip \rival{\jet_i\pmr{a_i}} : \jetgph{\vec a} \to \jetgph{\vec a}$.
	
	We define the \textbf{opposite $i$-twisted-prism $X \multip \rival{\tej_i}$} on $X$ as the jet graph of mask $\vec a$ with
	\begin{itemize}
		\item carrier $X \times \accol{0, 1}$,
		\item jet relations generated by the following requirements:
		\begin{itemize}
			\item $(\loch, 0) : X \to X \multip \rival{\tej_i\pmr{a_i}}$ is a jet graph morphism,
			\item $(\loch, 1) : \Op_i\pmr{a_i}(X) \to X \multip \rival{\tej_i\pmr{a_i}}$ is a jet graph morphism,
			\item $(x, 0) \tej_i\pmr{a_i} (x, 1)$ for all $x \in X$.
		\end{itemize}
	\end{itemize}
	This defines the \textbf{opposite $i$-twisted-prism functor} $\loch \multip \rival{\tej_i\pmr{a_i}} : \jetgph{\vec a} \to \jetgph{\vec a}$.
...
%\todoi{Should there be a prism functor for $\rival{\equijet_i}$? It would only be well-defined on symmetric jet graphs, the category of which is $\jetgph{\vec a'}$ where $a'_i = \npolar$, so it is derived from $\loch \multip \rival{\edge_i}$ and the forgetful functor $U^\npolar_i$.}
...
\begin{corollary} \label{thm:commute-diff-degree}
	Let $F_i$ be a functor between jet graph categories of any of the following forms: $\Op_i\pmr{a_i}$, $\Fsym_i$, $\Usym_i$, $\Csym_i$, $\loch \multip \rival{\jet_i\pmr{a_i}}$, $\loch \multip \rival{\tej_i\pmr{a_i}}$, $\loch \multip \rival{\edge_i\pmr{\npolar}}$.
	Let $G_j$ be a functor between jet graph categories also of one of these forms, but for a different degree $j$.
...
\subsection{Categorical Properties} \label{sec:jetgph:cat}
\begin{proposition} \label{thm:jetgph:colim}
	Let $D : \cat S \to \jetgph{\vec a}$ be a diagram.
	Then the colimit $\colim D$ is constructed as follows:
	\begin{itemize}
		\item Its carrier is the colimit of the carriers.
		\item Its jet relations are generated by the fact that each of the injections is a jet morphism. \qed
	\end{itemize}
\end{proposition}
...
\begin{definition}
	...
	In a jet graph $X$, write $\jet_i^*$ for the transitive closure of the $\jet_i$, and similarly $\infrajet_i^*$ for the transitive closure of $\infrajet_i$, which is the symmetric transitive closure of $\jet_i$.
	\begin{itemize}
...
%\begin{definition} 
%	A jet graph is called \textbf{$i$-orientable} if the $i$-jet relation has no cycles, i.e. sequences
%	\[
...
\begin{definition} \label{def:jet-tope}
	...
	An $\vec a$-jet-graph $X$ is a \textbf{jet tope} if it is:
	\begin{enumerate}
	...
	\end{enumerate}
	The \textbf{category of jet topes} $\jettope{\vec a}$ is defined as the full subcategory of $\jetgph{\vec a}$ on jet topes.
\end{definition}
...
Having an $i$-equijet interval object in the base category, will cause presheaves to contain `formal' $i$-equijets as an additional symmetric relation, stricter than the symmetric interior of the $i$-jet relation, which we wish to avoid.
\begin{proposition} \label{thm:lift-to-jet-topes}
	Jet topes are closed under the functors $\Op_i$, $\Fsym_i$,
	%$\Usym_i$, $\Csym_{i \neq n-1}$, $\UFsym_i$, $\UCsym_{i \neq n-1}$,
	$\loch \multip \rival{\jet_i\pmry}$, $\loch \multip \rival{\tej_i\pmry}$ and $\loch \multip \rival{\edge_i\pmrn}$.
\end{proposition}
\begin{proof}
...
%		\item[$\Usym_i$] This functor does not modify the jet graph, so the conditions are preserved.
...
%		\item[$\UCsym_{i \neq n-1}$] Since $\UCsym_i = \Usym_i \circ \Csym_i$.
		\item[\framebox{$\loch \multip \rival{\jet_i\pmry}$}] To see $(n-1)$-connectedness, note that:
		\begin{itemize}
		...
		\end{description}
		\item[\framebox{$\loch \multip \rival{\tej_i\pmry}$}] By an analogous argument.
		\item[\framebox{$\loch \multip \rival{\edge_i\pmrn}$}] By an analogous argument. \qedhere
	\end{description}
\end{proof}
...
\begin{definition} \label{def:jet-cube}
	...
	Moreover, if $a_i = \ypolar$, we demand that the symbols $\jet_i\pmr{\ypolar}$ and $\tej_i\pmr{\ypolar}$ do not appear to the left of any symbol $\equijet_i\pmr{\ypolar}$.
...
\begin{definition} \label{def:symmetric-var}
	...
	Thus, if $\dm i$ is $i$-directed, then $a_i = \ypolar$ and either $\dm i : \rival{\jet_i\pmr{\ypolar}}$ or $\dm i : \rival{\tej_i\pmr{\ypolar}}$.
\end{definition}
...
\begin{definition} \label{def:jep}
	For any $(\myorix, \vec a)$-jet-cube $W$, we define the $\vec a$-jet-graph $\JEP(W)$ as follows:
	\[ \begin{array}{l c l l}
		\JEP(()) &=& \top, \\
...
\begin{corollary} \label{thm:jet-cube-pullback}
	...
	\[
			\xymatrix{
				...
				\\
				&
				\cubecat^\mycubex_M \times_\Set \jetgph{\vec a}
					\ar@{.>}[rr]
					\ar@{.>}[d]
					\xydrcorner
				&&
...
\begin{definition} \label{def:jet-cube-cat}
	...
	as the full subcategory of $\cubecat^\mycubex_M \times_\Set \jetgph{\vec a}$ on $(\myorix, \vec a)$-jet-cubes, as justified by \cref{thm:jet-cube-pullback}.
	The functions $\JEP$ and $\erasejc{\loch}$ are correspondingly extended to functors.
\end{definition}
\begin{corollary} \label{thm:jet-cubes-are-topes}
	For $\myorix \in \accol{\orif, \orifb}$, the functor $\JEP : \jetcube{\myorix}{\mycubex}{M}{\vec a} \to \jetgph{\vec a}$ factors over the inclusion $\jettope{\vec a} \hookrightarrow \jetgph{\vec a}$.
\end{corollary}
\begin{proof}
	By induction on the dimension and using \cref{thm:lift-to-jet-topes}, it is clear that for any jet cube $W$, the jet graph $\JEP(W)$ is a jet tope, which factors the action on objects. The action on morphisms factors because $\jettope{\vec a}$ is a full subcategory of $\jetgph{\vec a}$.
\end{proof}
...
\begin{proposition} \label{thm:lift-to-jet-cubes}
	The following functors on $\vec a$-jet-graphs lift over $\JEP$ to functors on $(\myorix, \vec a)$-jet-cubes under the following conditions:
	\begin{center}
...
\end{proposition}
\begin{proof}
	The functors $\Fsym_i$, $\Op_i$ and $\Usym_i$ have no effect on the carrier, so they certainly lift to $\cubecat_M^\mycubex$, hence to the pullback $\cubecat_M^\mycubex \times_\Set \jetgph{\vec a}$.
	\begin{itemize}
	...
	Since the categories of jet cubes are fully faithful subcategories of the pullback $\cubecat_M^\mycubex \times_\Set \jetgph{\vec a}$, it suffices to build them there.
	They were already established in $\jetgph{\vec a}$ by \cref{def:sym-free,def:sym-cofree}.
	...
	Hence, they also lift to the pullback $\cubecat_M^\mycubex \times_\Set \jetgph{\vec a}$.
...
\end{proposition}
\begin{proof}
	Since the categories of jet cubes are fully faithful subcategories of the pullback $\cubecat_M^\mycubex \times_\Set \jetgph{\vec a}$, it suffices to prove the natural isomorphism there.
	The isomorphism was already established in $\jetgph{\vec a}$ by \cref{thm:commute-diff-degree}, and the effect on the carrier is either nothing (when at most one prism functor is involved) or swapping components (when both functors are prism functors).
...
\begin{proof}
	...
	The isomorphism was already established in $\jetgph{\vec a}$ by \cref{thm:commute-symmetric}, and the effect on the carrier is swapping components, which lifts to $\cubecat_M^\mycubex$.
\end{proof}
...
\begin{proof}
	...
	%There, 
	We know that $\jetcube \myorix \affine M {\vec a}$ is a subcategory of $\jetcube \myorix \cartes M {\vec a}$.
	So we need to show that any morphism in $\jetcubetri \cartes M {\vec a}$ is in fact affine.
	...
	Then $\JEP(\hat \vfi)$ cannot be a jet graph morphism as $\JEP(W)$ has no diagonals. This is a contradiction.
\end{proof}
...
Since the forgetful functor $U : \jetgph{\vec a} \to \Set$ is faithful, so is $\erasejc \loch : \jetcube{\myorix}{\affine}{M}{\vec a} \to \cubecat^\affine_M$.
...
In either case, it turns out that whether the last term reduces to $0$ or $1$ on any end point of the source cube, is sufficiently irregular that $i$-directed terms in the remainder of $\vfi$ can only depend on $i$-symmetric variables.\footnote{This is formalized in \cref{thm:left-of-connection}.}
...
\end{remark}
\begin{proof}
	Note that what really needs to be proven is that $\sez \vfi : V \to W$ implies that $\EP(\vfi)$, which a priori is a function from the set $\EP(\erasejc V) = U(\JEP(V))$ to $\EP(\erasejc W) = U(\JEP(W))$, is in fact a morphism of jet graphs $\JEP(V) \to \JEP(W)$.
	We prove this, of course, by induction on the derivation of the inductive predicate.
	
	\begin{itemize}
		\item For \ruleref{terminal}, note that $\JEP(())$ is the terminal jet graph.
		...
		\item For \ruleref{inv:fwd}, by postcomposition, it suffices to show that $\zeta : (\id_W, \lnot \dm i / \dm i) : \erasejc{(W, i : \rival{\tej_i})} \to \erasejc{(W, i : \rival{\jet_i})}$ is a morphism of jet cubes, i.e.\ that $\EP(\zeta) : (\vec w, u) \mapsto (\vec w, \lnot u)$ is a morphism of jet graphs $\JEP(W) \multip \rival{\tej_i} \to \JEP(W) \multip \rival{\jet_i}$.
		...
		Thus, we need to prove that $\EP(\id, \dm i / \novar) : (\vec v, w) \mapsto \vec v$ is a jet graph morphism $\JEP(V, \dm i : \rival{R_i})
...
%		Then $f := \JEP(\hat \vfi)$ is a jet graph morphism $\JEP(\UFsymcube_i(\UFsymcube_j U, V)) \to \JEP(W)$.
%		We need to show that $g : (\vec u, w, \vec v) \mapsto (f(\vec u, \vec v), w)$ is a jet graph morphism $\JEP(U, \dm j : \rival{\jet_j}, V) \to \JEP(W, \dm i : \rival{\jet_i}) = \JEP(W) \multip \rival{\jet_i}$.
%		
...
%			The fact that $f$ is a jet graph morphism implies that $f(\vec u, \vec v) \jet_k f(\vec u, \vec v')$ and even $f(\vec u, \vec v) \equijet_i f(\vec u, \vec v')$ if $k = i$.
...
%		
%		\item For \ruleref{conj:fwd}, assume given a jet cube morphism $\hat \vfi : \UFsymcube_i V \to W$ (write $\vfi = \erasejc{\hat \vfi}$ and $\EP(\vfi) = f$) and boolean expressions $s$ and $t$ such that $\EP(\vfi, s/\dm i, t/\dm j)$ is a jet graph morphism $\JEP(V) \to \JEP(W, \dm i : \rival{\jet_i}, \dm j : \rival{\jet_i}) = \JEP(W) \multip \rival{\jet_i} \multip \rival{\jet_i}$.
%		We show that $\EP(\vfi, s \wedge t / \dm k)$ is a jet graph morphism $\JEP(V) \to \JEP(W, \dm k : \rival{\jet_i}) = \JEP(W) \multip \rival{\jet_i}$.
%		Assume we have $v \jet_k v'$ in $\JEP(V)$. Then because $\EP(\vfi, s/\dm i, t/\dm j)$ is a jet graph morphism, we are in one of the following situations:
%		\begin{itemize}
...
%		\item We prove soundness of \ruleref{truth-table} by postcomposition with a jet cube morphism that erases to $(\id, \vec t[\vec{\dm i}]/\vec{\dm j}) : (W, \dm i_1 : \rival{\edge_i}, \ldots, \dm i_n : \rival{\edge_i}) \to (W, \dm j_1 : \rival{\edge_i}, \ldots, \dm j_m : \rival{\edge_i})$, i.e. we prove that $\EP(\id, \vec t[\vec{\dm i}]/\vec{\dm j}) : (\vec w, \vec u) \mapsto (\vec w, \vec t(\vec u))$ is a jet graph morphism $\JEP(W, \dm i_1 : \rival{\edge_i}, \ldots, \dm i_n : \rival{\edge_i}) \to \JEP(W, \dm j_1 : \rival{\edge_i}, \ldots, \dm j_m : \rival{\edge_i})$.
%		
...
As of yet, when we say that a variable appears in an affine boolean term, or in a cube morphism $\vfi$ which is a list of affine boolean terms, we mean that the term (or one of the terms) \emph{depends} on it as per \cref{thm:bool:term-dependencies}.

\paragraph{Understanding jet cube morphisms}
\begin{lemma}[Variable not used at its own degree] \label{thm:left-of-concursor}
	In $\jetcubetri \affine M {\vec a}$ with $M \in \accol{\bipointedsym, \booleanalg}$,
	%assuming \cref{conv:sort-cubes},
	the following holds:
	If a cube morphism $\vfi$ is a jet cube morphism $V = (V_0, \dm j : \rival{P_j\pmr{a_j}}, V_1) \to W$ with $P \in \accol{\jet, \tej, \equijet}$,
	and if either of the following conditions hold:
	\begin{itemize}
		\item $\dm j$ does not appear in $\vfi$,
		\item $\dm j$ appears in $\dm i \psub \vfi$ where $W = (W_0, \dm i : \rival{Q_i\pmr{a_i}}, W_1)$ with $Q \in \accol{\jet, \tej, \equijet}$ and $j > i$,
	\end{itemize}
	then $\vfi$ is also a jet cube morphism
	$\tilde V := (\UFsymcube_j(V_0), \dm j : \rival{\equijet_j\pmr{a_j}}, V_1) \to W$.
\end{lemma}
...
\begin{proof}
	...
	We now show that $\vfi$ is a jet cube morphism $\tilde V \to W$, i.e.\ that $f := \EP(\vfi)$ is a jet graph morphism $\JEP(\tilde V) \to \JEP(W)$.
	Pick a non-reflexive jet $\vec v = (\vec v_0, u, \vec v_1) \jet_k \vec v' = (\vec v_0', u', \vec v_1')$ in $\tilde V$; we show that $f(\vec v) \jet_k\pmr{a_k} f(\vec v')$.
	...
	\begin{description}
		...
		In this case, we have $\vec v_0 \infrajet_j\pmr{a_j} \vec v_0'$ in $V_0$, $u = u'$, $\vec v_1 = \vec v_1'$, $\vec v \infrajet_j\pmr{a_j} \vec v'$ in $V$ and $f(\vec v) \infrajet_j\pmr{a_j} f(\vec v')$ in $W$.
		If $\vfi$ does not depend on $\dm k$, then $f(\vec v) = f(\vec v')$ and therefore $f(\vec v) \jet_j\pmr{a_j} f(\vec v')$.
		...
		\begin{itemize}
			...
			\item If $\dm l$ is of degree $\ell > i$, then it is part of $W'$. We then have $\EP(\rho)(\vec v_0, \vec v_1) \overset{\neq}{\equijet}_j \EP(\rho)(\vec v_0', \vec v_1)$ because $\rho$ is a jet cube morphism $(\UFsymcube_j V_0, V_1) \to W'$.
			Since $\EP(\rho) = \EP(\pi) \circ \EP(\vfi) \circ \EP(u/\dm j)$ and the components forgotten by $\EP(\pi)$ are identical,
	...
	\end{description}
\end{proof}
\begin{lemma}[Characterization of same-degree directed affine boolean jet cube terms] \label{thm:char-twisted-cube-to-line}
	In $\jetcubetri \mycubex \booleanalg {\vec a}$ with $\mycubex \in \accol{\affine, \cartes}$ and $a_i = \ypolar$, let $V$ be a jet cube with only $i$-directed variables called (from left to right) $\dm j_1, \ldots, \dm j_n$, and consider $\vfi : V \to (\dm i : \rival{P_i\pmr{\ypolar}})$ with $P \in \accol{\jet, \tej}$.
...
%
	...
	Assume $n > 0$.
	%, implying that $a_i = \ypolar$.
	The jet graph $\JEP(V)$ has $2^n$ elements and a unique Hamiltonian path of $i$-jets.
	The function $f := \JEP(\vfi)$ sends this Hamiltonian path to a path in $\JEP(\dm i : \rival{\jet_i}) = \accol{0 \jet_i\pmr{\ypolar} 1}$.
	Thus, $f$ is entirely determined by the step in the Hamiltonian path where the image of $f$ flips from $0$ to $1$.
	Write $\dm j_k'$ to mean $\dm j_k$ if $\dm j_k : \rival{\jet_i\pmr{\ypolar}}$ and to mean $\lnot \dm j_k$ if $\dm j_k : \rival{\tej_i\pmr{\ypolar}}$.
	There are 5 possible scenarios:
	\begin{itemize}
		\item The entire path is sent to $0$. Then $\dm i \psub \vfi = 0$.
		%$ = \_ \opK 0$.
		\item The entire path is sent to $1$. Then $\dm i \psub \vfi = 1$.
		%$ = \_ \opK 1$.
	...
	\end{itemize}
\end{proof}
\begin{lemma}[Characterization of directed affine boolean jet cube terms] \label{thm:char-jet-cube-to-line}
	In $\jetcubetri \affine \booleanalg {\vec a}$ where $a_i = \ypolar$, consider $\vfi : V \to (\dm i : \rival{P_i\pmr{\ypolar}})$ with $P \in \accol{\jet, \tej}$.
	...
	(If $n = 0$, we regard any term $t$ as being of the above form.)
\end{lemma}
\begin{proof}
	We assume $P = {\jet}$, the proof for $P = {\tej}$ is analogous.

	First of all, the $i$-equijet relation as well as all $\ell$-jet relations for $\ell < i$ are discrete (coincide with the equality relation) in $\JEP(\dm i : \rival{\jet_i}) = \accol{0 \jet_i 1}$, so that $\JEP(\vfi)$ must be constant on $i$-equijet- or $\ell$-jet-connected components, implying that $\vfi$ cannot depend on those variables.
	...
	The form of \cref{thm:char-twisted-cube-to-line} then dictates that $\dm i \psub \vfi$ depends on a final segment of the $i$-directed variables in $V$ (since the outermost usage of $\opK$ cuts off an initial segment).
	Then $\vfi$ factors over the map $\psi : V \to X$ that weakens over the initial segment of $i$-directed variables that $\vfi$ does not depend on, and again, without loss of generality, we assume that $V = W$, i.e.\ that $\vfi$ uses \emph{all} $i$-directed variables in $V$.
	...
	If $e$ does not mention any $i$-directed variables, then it is of the required form.
	...
	\begin{itemize}
		\item If all $i$-directed variables appear in the same operand, say $d_1$, then by \cref{thm:boolexp-to-const-assignment} there is an assignment $\tau$ of all the dependencies of $d_2, \ldots, d_m$ which reduces them all to $\neutralA$, the neutral element of $\opA$.
		By soundness of our calculus, there is a jet cube $V'$ (consisting of all the non-assigned variables of $V$ at the same degrees) such that $\sigma : V' \to V$ is a jet cube morphism, and we have $\dm i \psub{\vfi \circ \tau} = [d_1]$.
		Then by the induction hypothesis, $d_1$ must be of the required form, and therefore so is $e$.
		\item If $\lnot^{p_n} \dm j_n$ is a direct operand of $e = e' \opA \lnot^{p_n} \dm j_n$, then by soundness of our calculus, there is a jet cube $V'$ (consisting of all the variables of $V$ but $\dm j$ at the same degrees) such that $(\lnot^{p_n} \neutralA / \dm j_n) : V' \to V$ is a jet cube morphism, and we have $\dm i \psub{\vfi \circ (\lnot^{p_n}\neutralA / \dm j_n)} = [e']$.
		Then by the induction hypothesis, $e'$ must be of the required form w.r.t.\ $\dm j_1, \ldots, \dm j_n$, and therefore so is $e$. \qedhere
	\end{itemize}
%	
%	\todoi{Old version}
%	
...
%	\end{itemize}
\end{proof}
\begin{lemma}[Connections are wild] \label{thm:left-of-connection}
	In $\convjetcubetri \affine \booleanalg {\vec a}$, let $\hat \vfi : V \to W$ be a jet cube morphism and write $\vfi = \erasejc{\hat \vfi}$.
	Let $W = (W_0, \dm i : \rival{Q_i\pmr{\ypolar}})$ with $Q \in \accol{\jet, \tej}$ and $a_i = \ypolar$.
	...
	\footnote{We could in principle allow $\equijet$ but it is easy to see that $\vfi$ being a jet cube morphism (equivalently, $\EP(\vfi)$ being a jet graph) implies $P^1, \ldots, P^n \in \accol{\jet, \tej}$.}
	Define $\tilde V = (\UFsymcube_i V_0, \dm j_1 : \rival{P^1_i}, \ldots, \dm j_n : \rival{P^n_i}, V_1)$, i.e.\ every variable of degree $i$ to the left of $\dm j_1$ gets promoted to an equijet variable.
	Then $\vfi$ is a jet cube morphism $\tilde V \to W$.
\end{lemma}
%The lemma allows $P^1, \ldots, P^n \in \accol{\jet, \tej, \equijet}$, but it is easy to see that $\vfi$ being a jet cube morphism (equivalently, $\EP(\vfi)$ being a jet graph) implies $P^1, \ldots, P^n \in \accol{\jet, \tej}$.
...
\begin{proof}
	...
	Commutativity of the diagram and the fact that $\JEP(c/\dm i)$ is a full jet graph morphism, imply that $\chi$, too, is a jet cube morphism.
	
	We now show that $\vfi$ is a jet cube morphsim $\tilde V \to W$.
	Let $\vec v \jet_k\pmr{a_k} \vec v'$ in $\tilde V$.
	We prove that $f(\vec v) \jet_k\pmr{a_k} f(\vec v')$ where $f = \EP(\vfi)$.
	If $\vec v = \vec v'$ then this is trivial, so let $\dm l$ be the variable where they differ.
	If $k \neq i$ or $\dm l \not\in V_0$ then we have $\vec v \jet_k\pmr{a_k} \vec v'$ in $V$ so this is preserved by $f$.
	
	Thus, it sufficies to assume that $k = i$ and $\dm l \in V_0$, which implies that $\dm l$ is not a dependency of $e$.
	This implies that $e \psub{\vec v} = e \psub{\vec v'} =: c$, or differently put $\dm i \psub{f(\vec v)} = \dm i \psub{f(\vec v')} = c$.
	We have $\vec v \infrajet_i\pmr{\ypolar} \vec v'$ in $V$, say $\vec v \mathrel{S}_i\pmr{\ypolar} \vec v'$ where $S \in \accol{\jet, \tej}$.
	This is preserved by $f$, so $f(\vec v) \mathrel{S}_i\pmr{\ypolar} f(\vec v')$.
	Because $(c/\dm i)$ is a full jet graph morphism, writing $p = \EP(\dm i / \novar)$, we can conclude that $p(f(\vec v)) \mathrel{S}_i\pmr{\ypolar} p(f(\vec v'))$ in $(\Opcube_i)^{1-c}(W_0)$.\footnote{Or $(\Opcube_i)^{c}(W_0)$ if $Q = \tejsym$; for simplicity we assume $Q = \jetsym$.}
	
	Let $\vec u$ and $\vec u'$ be the restriction of $\vec v$ and $\vec v'$ to the variables in $U$, let $\vec \zeta$ be the (common) restriction of $\vec v$ and $\vec v'$ to the variables $\vec{\dm j}$, and $\vec \kappa$ be the (common) restriction of $\vec v$ and $\vec v'$ to the variables $\vec{\dm k}$.
	Thus, $\vec v = \EP(\vec \zeta/\vec{\dm j}, \vec \kappa/\vec{\dm k})(\vec u)$ and $\vec v' = \EP(\vec \zeta/\vec{\dm j}, \vec \kappa/\vec{\dm k})(\vec u')$.
...
%	\end{itemize}
	...
	Indeed, these truth tables can only align if $m = 0$ and $n \leq 1$.
	...
	Thus, $\chi$ provides us the jet pointing the other way, and we can conclude that $g(\vec u) \equijet_i\pmr{\ypolar} g(\vec u')$.
	Composing with $\EP(c/\dm i)$ for our original $c$ yields $f(\vec v) \equijet_i\pmr{\ypolar} f(\vec v')$.
\end{proof}
...
\begin{proof}
	...
	We show that $(\novar/\dm i) \circ \vfi$ is a jet cube morphism $\UFsymcube_i V \to W_0$.
	Take a jet $v \jet_j\pmr{a_j} v'$ in $\JEP(\UFsymcube_i V)$.
	We prove that $p(f(v)) \jet_j\pmr{a_j} p(f(v'))$ in $\JEP(W_0)$.
	If $v = v'$, then this is trivial, so let $\dm k$ be the variable where they differ.
	If $j \neq i$, then we have $v \jet_j\pmr{a_j} v'$ in $V$ as well as $\tilde V$ and the result follows because $\vfi : V \to W$ is a jet cube morphism, so we assume that $j = i$.
	Then $v \equijet_i\pmr{\ypolar} v'$ in $\JEP(\UFsymcube_i V)$, since we are in an $i$-symmetrically closed cube.
	If $\vfi$ does not depend on $\dm k$, then $f(v) = f(v')$ so the result still holds by reflexivity.
	Let $\dm l$ be the variable such that $\dm l \psub{\vfi}$ depends on $\dm k$.
	If $\dm l = \dm i$, then after forgetting $\dm i$ have $p(f(v)) = p(f(v'))$, so the result still holds by reflexivity.
	If $\dm l \neq \dm i$, then $\dm k$ is not a dependency of $\dm i$ and so it has the same `type' in $\tilde V$ and in $\UFsymcube_i V$.
	Then we have $v \equijet_i\pmr{\ypolar} v'$ in $\JEP(\tilde V)$ and hence $f(v) \equijet_i\pmr{\ypolar} f(v')$ in $\JEP(W)$ and $p(f(v)) \equijet_i\pmr{\ypolar} p(f(v'))$ because $p$ preserves $i$-equijets.
%	
%	\todoi{Old stuff}
%	
%	We first show that $(\novar / \dm i) \circ \vfi$ is a jet cube morphism $\tilde V \to W_0$.
%	Write $f = \EP(\vfi)$ and $p = \EP(\novar / \dm i)$.
%	Pick a non-reflexive jet $\vec v \jet_j\pmr{a_j} \vec v'$ in $\tilde{V}$.
%	We show that $p(f(\vec v)) \jet_j\pmr{a_j} p(f(\vec v'))$ in $W_0$.
%	We know that $f(\vec v) \jet_j\pmr{a_j} f(\vec v')$ in $W$.
%	If $j \neq i$, it follows that $p(f(\vec v)) \jet_j\pmr{a_j} p(f(\vec v'))$ in $W_0$.
%	If $j = i$, it follows that $p(f(\vec v)) \infrajet_i\pmr{\ypolar} p(f(\vec v'))$ in $W_0$.
...
\begin{proof}
	...
	We prove completeness by induction on the number of nodes and leaves (added up) in the tuple $(e_{\dm k})_{\dm k \in W}$, plus the number of \emph{directed} degrees in the mask.
	
	If $W = ()$, then use \ruleref{terminal}.
	
	If $a_i = \ypolar$ and the last variable in $W$ is $\dm i : \rival{\equijet_i\pmr{\ypolar}}$, then $W$ is of the form $\Usymcube_i(U)$ and we can use the induction hypothesis for the corresponding morphism $\Fsymcube_i(V) \to U$ and we can use the rule \ruleref{symmetrize}.
	
	In the remaining case, the last variable in $W$ is not an equijet dimension at a directed degree, i.e.\ it is of the form $\dm i : \rival{\jet_i\pmr{a_i}}$ or $\dm i : \rival{\tej_i\pmr{a_i}}$.
	
	If the last variable in $V$ is of degree strictly lower (stronger) than the degree of $\dm i$, then in order to be a jet graph morphism, $\JEP(\hat \vfi)$ cannot depend on that variable, so we can invoke \ruleref{wkn} until the last variable in $V$ is of degree at least $i$. We do not resort to the induction hypothesis but proceed below.
	By \cref{def:conv-jet-cube-cat}, we can thus assume that all variables in $V$ are of degree at least $i$.
...
%	Before that, we put, if existent, the sole variable $\dm k_2$ such that $e_{\dm k_2}$ mentions the second to last variable of degree $i$ in $V$. Etc.
	...
	\begin{itemize}
		\item If [$\dm i : \rival{\jet_i\pmr{a_i}}$ and $e_{\dm i} = 0$] or [$\dm i : \rival{\tej_i\pmr{a_i}}$ and $e_{\dm i} = 1$], then $\vfi$ being a jet cube morphism $V \to W = (U, \dm i : \_)$ (equivalently: $\EP(\vfi)$ being a jet graph morphism $\JEP(V) \to \JEP(W)$) is equivalent to $(\novar / \dm i) \circ \vfi$ being a jet cube morphism $V \to \Opcube_i(U)$, so we can apply \ruleref{src:fwd} or \ruleref{src:bck}.
		
		\item If [$\dm i : \rival{\jet_i\pmr{a_i}}$ and $e_{\dm i} = 1$] or [$\dm i : \rival{\tej_i\pmr{a_i}}$ and $e_{\dm i} = 0$], then $\vfi$ being a jet cube morphism $V \to W = (U, \dm i : \_)$ (equivalently: $\EP(\vfi)$ being a jet graph morphism $\JEP(V) \to \JEP(W)$) is equivalent to $(\novar / \dm i) \circ \vfi$ being a jet cube morphism $V \to U$, so we can apply \ruleref{tgt:fwd} or \ruleref{tgt:bck}.
		
		\item If $\dm i : \rival{\jet_i\pmr{a_i}}$ and $e_{\dm i} = \lnot \dm j$, then $\vfi$ being a jet cube morphism $V \to W = (U, \dm i : \rival{\jet_i\pmr{a_i}})$ is equivalent to $(\lnot \dm i / \dm i) \circ \vfi$ being a jet cube morphism $V \to (U, \dm i : \rival{\tej_i\pmr{a_i}})$, so we can apply \ruleref{inv:fwd}.
		Similarly, if $\dm i : \rival{\tej_i\pmr{a_i}}$ and $e_{\dm i} = \lnot \dm j$, we can apply \ruleref{inv:bck}.
		
		\item If $\dm i : \rival{\jet_i\pmr{a_i}}$ ($\dm i : \rival{\tej_i\pmr{a_i}}$ is handled analogously) and $e_{\dm i} = \dm j$ where $V$ specifies that $\dm j$ has degree $j \geq i$, then we know that $\vfi = (\chi, \dm j / \dm i)$ is a jet cube morphism $V = (V_0, \dm j : \rival{R_j\pmr{a_j}}, V_1) \to W = (U, \dm i : \rival{\jet_i\pmr{a_i}})$ for some $R \in \accol{\jet, \tej, \equijet}$.
		...
		\]
		We know that the black arrows are all jet cube morphisms, and the vertical arrows all yield full jet graph morphisms (\cref{def:full-jetgphhom}).
		This implies that the dashed arrows also lift to jet graph morphisms, i.e.\ are jet cube morphisms.
		...
		\begin{itemize}
			...
			This implies that $\EP(\vfi)$ sends every $i$-jet of the form $(\vec v_0, r, \vec v_1) \jet_i\pmr{a_i} (\vec v_0, r, \vec v_1')$ in $\JEP(V_0, V_1)$ -- which points the other way in $\JEP(\Opcube_i(\Opcube_i V_0, V_1))$ -- to an $i$-equijet $(\EP(\chi)(\vec v_0, \vec v_1), r) \equijet_i\pmr{a_i} (\EP(\chi)(\vec v_0, \vec v'_1), r)$ in $\JEP(U, \dm i : \rival{\jet_i\pmr{a_i}})$.
			\begin{itemize}
				\item If $a_i = \ypolar$, then for any bit-assignment $\vec v_1$ of the variables in $V_1$, we get a jet cube morphism $(\chi \circ (\vec v_1/V_1), \dm j / \dm i) : (\Opcube_i)^p(V_0, \dm j : \rival{R_i\pmr{\ypolar}}) \to (U, \dm i : \rival{\jet_i\pmr{\ypolar}})$, where $p$ is the number of source constants in $\vec v$ (i.e.\ $0$ for $\rival{\jet_i\pmr{\ypolar}}$ and $1$ for $\rival{\tej_i\pmr{\ypolar}}$).
...
%				\end{itemize}
		...
		If $a_i = \ypolar$ and $\dm i : \rival{\tej_i\pmr{\ypolar}}$, we apply \ruleref{inv:bck} and push down the introduced negation, after which we do not resort to the induction hypothesis but proceed below.\footnote{Alternatively, we could duplicate and adapt the proof below to the case where $\dm i : \rival{\tej_i}$.}
		We now assume that $\dm i : \rival{\jet_i\pmr{a_i}}$.
		
		\newcommand{\introtauxx}[0]{%
		Using \cref{thm:boolexp-to-const-assignment}, pick a bit-assignment $\tau$ of the dependencies of $t$ such that $t \psub \tau$ reduces to the neutral element $\neutralA$ of $\opA$.\footnote{If this were not possible, then $t$ would be a constant, which is in contradiction with the assumption that $e_{\dm i}$ was normal.}
		...
		\begin{itemize}
			...
			Let $\vfi = (\chi, s \opA t/\dm i)$ where $\opA \in \accol{{\vee}, {\wedge}}$ and $W = (U, \dm i : \rival{\edge_i})$.
			...
			\[
				...
				q = \EP(\chi, t/\dm i).
			\]
			Pick a non-reflexive jet $\vec v \jet_j\pmr{a_j} \vec v'$.
			We prove $p(\vec v) \jet_j\pmr{a_j} p(\vec v')$; by symmetry of the situation we do not have to prove the same for $q$.
			Let $\dm k$ be the variable of $V$ where $\vec v$ and $\vec v'$ differ.
			...
			\begin{itemize}
				...
				\begin{itemize}
					\item If $j \neq i$, then from $(g(\vec v), s_0 \opA t_0) = f(\vec v) \jet_j\pmr{a_j} f(\vec v') = (g(\vec v'), s_0 \opA t_0)$, it follows that $g(\vec v) \jet_j\pmr{a_j} g(\vec v')$ in $\JEP(U)$, whence $p(\vec v) = (g(\vec v), s_0) \jet_j\pmr{a_j} (g(\vec v'), s_0) = p(\vec v')$.
				...
				\introtauxx{}
				We have $\vec x \infrajet_j\pmr{a_j} \vec x'$, whence
				\[
					...
					\end{array}
				\]
				in $\JEP(W, \dm i : \rival{\edge_i})$.
				Now, since these vectors only differ at $\dm i : \rival{\edge_i}$, we can deduce equality if $j < i$ ($j$ is stronger than $i$) and otherwise that $p(\vec v) \edge_i p(\vec v')$, which implies $p(\vec v) \jet_j\pmr{a_j} p(\vec v')$ since $j \geq i$ ($j$ is weaker than or equal to $i$).
				
				\item If $t$ depends on $\dm k$, then $\chi$ and $s$ do not so $p(\vec v) = p(\vec v')$ and we are done.
			\end{itemize}
			
			\item Now assume that $a_i = \ypolar$.
			Let $\vfi = (\chi, t \opA s/\dm i)$ where $\opA \in \accol{{\vee}, {\wedge}}$ and $W = (U, \dm i : \rival{\jet_i\pmr{\ypolar}})$.
			\Cref{thm:weaken-connection} immediately tells us that $\chi$ is a jet cube morphism $\UFsymcube_i V \to U$.
			By \cref{thm:char-jet-cube-to-line}, we can assume that $s$ is either (the negation of) the last variable of degree $i$ in $V = (V_0, \dm i : \rival{P_i\pmr{\ypolar}})$ with $P \in \accol{\jet, \tej}$ or a boolean expression only depending on $i$-symmetric variables.
			\begin{itemize}
				\item We consider the first case, so $s = \dm i' \in \accol{\dm i, \lnot \dm i}$ with $V = (V_0, \dm i : \rival{P_i\pmr{\ypolar}})$ and $P \in \accol{\jet, \tej}$.
				
				Note that $(P, \dm i') \in \accol{(\jet, \dm i), (\tej, \lnot \dm i)}$.
				Indeed, by \cref{thm:boolexp-to-const-assignment}, there is an assignment $\sigma$ of the dependencies of $t$, such that $t[\sigma] = \neutralA$.
				By soundness of the calculus, there will be a jet cube $V'$ such that $\sigma : V' \to V$ is a jet cube morphism.
				Then $\vfi \circ \sigma = (\chi, \dm i' / \dm i) : V' \to W$ is a jet cube morphism.
				This shows that $\dm i'$ needs to be the negation of $\dm i$ if and only if its type in $V$ is backwards.
				
				Then depending on $\opA$ we have one of the following commutative diagrams in the category of jet cubes and morphisms between their erasures:
				\[
					\xymatrix{
						&&\Opcube_i V
							\ar[d]_{(0/\dm i)}
				...
				\begin{itemize}
					\item If $\vfi = (\chi, t \vee \dm i) : (V_0, \dm i : \rival{\jet_i\pmr{\ypolar}}) \to (U, \dm i : \rival{\jet_i\pmr{\ypolar}})$,
					then we can use \ruleref{conn:prism:src-neutral}.
					\item If $\vfi = (\chi, t \wedge \dm i) : (V_0, \dm i : \rival{\jet_i\pmr{\ypolar}}) \to (U, \dm i : \rival{\jet_i\pmr{\ypolar}})$,
					then we can use \ruleref{conn:prism:tgt-neutral}.
					\item If $\vfi = (\chi, t \vee \lnot \dm i) : (V_0, \dm i : \rival{\tej_i\pmr{\ypolar}}) \to (U, \dm i : \rival{\jet_i\pmr{\ypolar}})$,
					then we can use \ruleref{conn:prism-inv:src-neutral}.
					\item If $\vfi = (\chi, t \wedge \lnot \dm i) : (V_0, \dm i : \rival{\tej_i\pmr{\ypolar}}) \to (U, \dm i : \rival{\jet_i\pmr{\ypolar}})$,
					then we can use \ruleref{conn:prism-inv:tgt-neutral}.
				\end{itemize}
				Thus, we can invoke one of the rules \ruleref{conn:prism:src-neutral}, \allowbreak \ruleref{conn:prism:tgt-neutral}, \ruleref{conn:prism-inv:src-neutral}, \ruleref{conn:prism-inv:tgt-neutral}.\footnote{Recall that we assume that the last variable in $W$ is $\dm i : \rival{\jet_i\pmr{\ypolar}}$, i.e.\ we can instantiate the connection rules with $Q = \jetsym$.}
				...
				Recall that
				\begin{itemize}
					\item $\chi$ is a jet cube morphism $\UFsymcube_i V \to U$.
				\end{itemize}
				We show that
				\begin{itemize}
					\item $(\chi, s / \dm i)$ is a jet cube morphism $\UFsymcube_i V \to (W, \dm i : \rival{\jet_i\pmr{\ypolar}})$,
					\item $(\chi, t / \dm i)$ is a jet cube morphism $V \to (W, \dm i : \rival{\jet_i\pmr{\ypolar}})$,
				\end{itemize}
				so that we can invoke \ruleref{conn:degree-symmetric}.
				
				Pick a non-reflexive jet $\vec v \jet_j\pmr{a_j} \vec v'$ in $\JEP(V)$.
				We will prove $p(\vec v) \jet_j\pmr{a_j} p(\vec v')$ and $q(\vec v) \jet_j\pmr{a_j} q(\vec v')$ and, if $j = i$, even $p(\vec v) \equijet_i\pmr{\ypolar} p(\vec v')$, all the time in $\JEP(W, \dm i : \rival{\jet_i\pmr{\ypolar}})$.
				...
				\begin{itemize}
					...
					Note that, since $s$ depends only on $i$-symmetric variables, $\dm k$ is $i$-symmetric.
					%We pick $\tau$ and define $\vec x$ and $\vec x'$ in the same way as we did when $a_i = \npolar$ and all other circumstances were the same.
					
					\introtauxx{}
					Then we have $\vec x \infrajet_j\pmr{a_j} \vec x'$ in $\JEP(V)$, whence $p(\vec v) = p(\vec x) = f(\vec x) \infrajet_j\pmr{a_j} f(\vec x') = p(\vec x') = p(\vec v')$.
					If $j = i$, then since $\dm k$ is $i$-symmetric, we have more strongly $\vec x \equijet_i\pmr{\ypolar} \vec x'$ whence $p(\vec v) \equijet_i\pmr{\ypolar} p(\vec v')$.
					The case where $j = i$ is thus covered.
					
					Now, since $p(\vec v)$ and $p(\vec v')$ only differ at $\dm i$, we can deduce equality if $j < i$ ($j$ is stronger than $i$) and otherwise that $p(\vec v) \infrajet_i\pmr{\ypolar} p(\vec v')$, implying $p(\vec v) \jet_j p(\vec v')$ if $j > i$.
					...
					We have $\vec y \infrajet_j\pmr{a_j} \vec y'$ but, since all dependencies of $s$ are $i$-symmetric, they come to the left of all $i$-directed variables, so if $i = j$ we actually still have $\vec y \jet_i\pmr{\ypolar} \vec y'$.
					Now we have
					\[
						\begin{array}{c c c c c c c c c}
						q(\vec v)
						...
						\end{array}
					\]
					and $q(\vec v) \jet_i\pmr{\ypolar} q(\vec v')$ if $j = i$.
					Thus, the case $j = i$ has been handled.
					Since $q(\vec v)$ and $q(\vec v')$ can only differ at $\dm i$ which has degree $i$, we can deduce equality if $j < i$ ($j$ is stronger than $i$) and otherwise that $q(\vec v) \infrajet_i\pmr{\ypolar} q(\vec v')$ which implies $q(\vec v) \jet_j\pmr{a_j} q(\vec v')$ as required. \qedhere
				\end{itemize}
			\end{itemize}
		\end{itemize}
	\end{itemize}
...
The results in this section are not really used, but the purpose, especially of \cref{thm:sssfactorization}, is to aid the reader in getting a better grasp on what cube morphisms can and cannot be jet cube morphisms.
\begin{definition} \label{def:separated-product}
	We define the \textbf{separated product} functor
	\[
		\loch * \loch : \jetgph{\vec a} \times \jetgph{\vec a} \to \jetgph{\vec a}
	\]
	by letting $X * Y$ be the jet graph with carrier $UX \times UY$ such that $(x, y) \jet_j (x', y')$ if either
	\begin{itemize}
...
\begin{definition} \label{def:sssprod}
	...
	\begin{itemize}
		...
		\end{corollary}
		\begin{corollary}
			In $\jetgph{\vec a \sqcup \vec b}$, we have
			\[
				\JEP(V \sss W) \cong \Usym_{\vec a \sqsubseteq \vec a \sqcup \vec b}(V) * \Usym_{\vec b \sqsubseteq \vec a \sqcup \vec b}(W),
			\]
			where $\Usym_{\vec x \sqsubseteq \vec y} : \jetgph{\vec x} \to \jetgph{\vec y}$ is the forgetful functor, forgetting symmetry at the degrees where $x_i = \npolar \neq y_i = \ypolar$.
		\end{corollary}
		...
		\begin{itemize}
			...
			\item At the level of $\jetgph{\vec a \sqcup \vec b}$, by relying on functoriality of the separated product,
...
\end{definition}
\begin{corollary} \label{thm:sssprod-commut}
	The SSS-product is commutative up to natural isomorphism.
\end{corollary}
\begin{proof}
	To prove this, we need to be able to commute variables of $V$ and $W$ of a degree $i$ such that $a_i = b_i = \npolar$.
	This is possible by \cref{thm:commute-symmetric}.
\end{proof}
We can thus make sense of the notation $\bigsss_i W_i$ for a multi-operand SSS-product.
\begin{definition}
...
\begin{theorem}[SSS-factorization] \label{thm:sssfactorization}
	Let $\vec a$ be a mask of length $\ell$.
	For any jet cube morphism $\hat \vfi : V \to W$ in $\convjetcubetri \affine M {\vec a}$, define jet cubes as follows:
	\begin{itemize}
		\item $W_i$ for a directed degree $i$ is a cube of mask $\delta^i$ and consists of all $i$-directed variables of $W$, in order.
		\item $V_i$ for a directed degree $i$ is a cube of mask $\delta^i$ and consists of all variables occurring in the composite cube morphism $\erasejc{V} \xrightarrow{\vfi} \erasejc{W} \xrightarrow{\pi} \erasejc{W_i}$ where $\dm i \psub \pi := \dm i$.
		These are the dependencies of the expressions that $\vfi$ substitutes for the variables in $W_i$.
		They are listed in the same order in which they occur in $V$.
		Variables of a degree $j \neq i$ are retyped in $V_i$ with type $\rival{\edge_j\pmr{\npolar}}$.
		\item $W_\npolar$ is a cube of mask $\vec \npolar$ and consists of all non-directed variables of $W$, i.e.\ all variables which in $W$ have type $\rival{\equijet_i\pmr{\ypolar}}$ or $\rival{\edge_i\pmr{\npolar}}$ for some $i \in \Deg(\vec a)$.
		They are listed in the same order in which they occur in $W$.
		They are retyped in $W_\npolar$ with type $\rival{\edgesym_i\pmr{\npolar}}$.
		\item $V_\npolar$ is a cube of mask $\vec \npolar$ and consists of all variables of $V$ not yet included in some $V_i$, i.e. variables either occurring in the composite cube morphism $\erasejc{V} \xrightarrow{\vfi} \erasejc{W} \xrightarrow{\pi} \erasejc{W_\npolar}$ or not occurring in $\vfi$ at all.
		They are listed in the same order in which they occur in $V$.
		They are retyped in $V_\npolar$ with type $\rival{\edgesym_i\pmr{\npolar}}$.
	\end{itemize}
	Then there are jet cube morphisms $\vfi_i : V_i \to W_i$ and $\vfi_\npolar : V_\npolar \to W_\npolar$ and jet cube morphisms $\rho_0$ and $\rho_1$ that erase to cube renamings\footnote{Cube renamings: Morphisms in $\Kleisli(M)\op$ that come from $\Set$, i.e.\ are not effectful in the sense that they do not use the constants and operators provided by $M$.}, such that $\vfi$ factorizes as:
	\[
		\xymatrix{
			V
				\ar[rr]^{\rho_0}_\cong
			&& V_\npolar \sss \paren{\bigsss_i V_i}
				\ar[rr]^{\vfi_\npolar \sss \paren{\bigsss_i \vfi_i}}
			&& W_\npolar \sss \paren{\bigsss_i W_i}
				\ar[rr]^{\rho_1}_\cong
			&& W,
		}
	\]
...
\end{theorem}
\begin{proof}
	As cube morphisms, the renamings $\rho_0$ and $\rho_1$ are just defined by $\dm i \psub{\rho_0} := \dm i$ and $\dm i \psub{\rho_1} := \dm i$.
	The cube morphism $\vfi_i$ is defined by $\dm i \psub{\vfi_i} = \dm i \psub{\vfi}$, and $\vfi_\npolar$ by $\dm i \psub{\vfi_\npolar} = \dm i \psub{\vfi}$.
	Then it is clear that the entire thing composes to $\vfi$.
	
	Since $W$ is conventional, we actually have $W_\npolar \sss \paren{\bigsss_i W_i} = W$ so that $\rho_1$ is in fact the identity morphism and therefore a jet cube morphism.
	
	For $\rho_1$, observe that the variables in $V_\npolar \sss \paren{\bigsss_i V_i}$ are again typed the same way as in $V$, and the only point in which they can differ, is in the swapping of symmetric variables of the same degree.
	Then $\rho_1$ is a jet cube morphism by \cref{thm:commute-symmetric}.
	
	
	To see that $\vfi_i$ is a jet cube morphism: the composite $V \xrightarrow{\vfi} W \xrightarrow{\pi} \Usymcube_{\vec \delta^i \sqsubseteq \vec a} W_i$ is an $\vec a$-jet-cube morphism since $\pi$ is.
	Applying $\Fsymcube_{\vec \delta^i \sqsubseteq \vec a}$, we obtain $\pi \circ \vfi$ as a $\vec \delta^i$-jet-cube morphism $\Fsymcube_{\vec \delta^i \sqsubseteq \vec a} V \to W_i$.
	Now since $V_i$ consists of a final segment of the variables of degree $i$ in $V$ (as a indirect consequence of \cref{thm:char-jet-cube-to-line} and the completeness of our calculus), any bit-assignment $\sigma$ of the variables in $V \setminus V_i$ amounts to a jet cube morphism $\sigma : V_i \to \Fsymcube_{\vec \delta^i \sqsubseteq \vec a} V$.
	Then $\vfi_i = \pi \circ \vfi \circ \sigma : V_i \to W_i$ is a jet cube morphism.
	
	To see that $\vfi_\npolar$ is a jet cube morphism: the composite $V \xrightarrow{\vfi} W \xrightarrow{\pi} \Usymcube_{\vec \npolar \sqsubseteq \vec a} W_\npolar$ is an $\vec a$-jet-cube morphism since $\pi$ is.
	Applying $\Fsymcube_{\vec \npolar \sqsubseteq \vec a}$, we obtain $\pi \circ \vfi$ as a $\vec \npolar$-jet-cube morphism $\Fsymcube_{\vec \npolar \sqsubseteq \vec a} V \to W_\npolar$.
	Any bit-assignment $\sigma$ of the variables in $V \setminus V_\npolar$ amounts to a jet cube morphism $\sigma : V_\npolar \to \Fsymcube_{\vec \npolar \sqsubseteq \vec a} V$.
	Then $\vfi_\npolar = \pi \circ \vfi \circ \sigma : V_\npolar \to W_\npolar$ is a jet cube morphism.
\end{proof}
...
\begin{proof}
	...
	It is faithful, because $U : \jetgph{[\npolar]} \to \Set$ is faithful.
...
\end{proposition}
\begin{proof}
	Jet graphs obtained from $\JEP$ do not have diagonals.
\end{proof}
...
\begin{proposition} \label{thm:jetcubes:comparison:depth-bipointedsym}
	The category $\dcubeopt \affine \bipointedsym n$ is isomorphic to the category $\jetcube \omega \mycubex \bipointedsym {[\npolar]^n}$ for $\mycubex \in \accol{\cartes, \affine}$.
	Moreover, the isomorphism commutes with the erasure functors.
\end{proposition}
\begin{proof}
	The orientation kit $\omega$ does not matter as all degrees are symmetric.
...
\begin{proposition} \label{thm:jetcubes:comparison:depth-bipointed}
	The category $\dcubeopt \affine \bipointed n$ is isomorphic to the category $\jetcube \omega \mycubex \bipointed {[\npolar]^n}$ for $\mycubex \in \accol{\cartes, \affine}$.
\end{proposition}
\begin{proof}
...
$\jetgph{[\ypolar]}$ is the category of proof-irrelevant reflexive graphs.
Pinyo and Kraus use potentially-non-reflexive proof-irrelevant graphs, but since $\top$ is reflexive and the twisted prism functor \cite[def.\ 4]{pinyo-twisted} restricts to reflexive graphs, all twisted cubes are reflexive graphs anyway.
...
\begin{definition} \cite[def.\ 25]{pinyo-twisted}
	The category $\twistedcubegrp$ has as objects $[\ypolar]$-jet-cubes (i.e.\ natural numbers) and as morphisms $V \to W$ all jet graph morphisms (i.e. graph morphisms) $\JEP(V) \to \JEP(W)$.
\end{definition}
\begin{proposition}
...
\begin{definition} \cite[def.\ 34]{pinyo-twisted} \label{def:twisted-tri}
	The category $\twistedcubetri$ has as objects $[\ypolar]$-jet-cubes (i.e.\ natural numbers) and as morphisms $V \to W$ all jet graph morphisms (i.e. graph morphisms) $\JEP(V) \to \JEP(W)$ (equivalently by the previous proposition, all jet cube morphisms $V \to W$ in $\jetcubefwd \cartes \booleanalg {[\ypolar]}$)	
	 generated by the rules \ruleref{terminal}, [\ruleref{src:fwd} immediately below \ruleref{inv:bck} or \ruleref{terminal}], \ruleref{tgt:fwd} and \ruleref{prism:fwd} in \cref{fig:calculus}.
\end{definition}
...
\begin{proof}
	...
	To show fullness, consider a morphism $\vfi : V \to W$ in $\jetcubetri \affine \bipointedsym {[\ypolar]}$.
	By completeness \cref{thm:calculus-complete}, it has a derivation tree in the calculus for $\jetcubetri \affine \bipointedsym {[\ypolar]}$.
	We shall modify this tree until it is accepted by $\twistedcubetri$.
	\begin{enumerate}
		\item The cube $W$ has only forward jets. No rule (read bottom-up) introduces equijets in the codomain. Therefore, at no point in the derivation will there be any equijets in the codomain.
		\item In absence of equijets in the codomain, no rule changes the mask (except \ruleref{symmetrize} applied to the terminal codomain, where we can instead cut off a derivation branch by using \ruleref{terminal}). Now, we have a tree that does not use \ruleref{symmetrize} and we are at mask $[\ypolar]$ throughout the derivation.
		...
		\begin{itemize}
			\item Hence \ruleref{wkn} can only be used to derive constant morphisms, but these can be derived in $\twistedcubetri$ using \ruleref{terminal}, [\ruleref{src:fwd} below \ruleref{inv:bck}] and \ruleref{tgt:fwd}.
			\item Hence \ruleref{exchange} is useless and can be removed by sliding it upwards until it annihilates with \ruleref{wkn}.
		\end{itemize} 
		\item At mask $[\ypolar]$, the rule \ruleref{concursor} cannot be used as there is only one degree.
		\item Since \ruleref{inv:fwd} and \ruleref{inv:bck} are mutually inverse, they can together be freely inserted everywhere. Hence, we can replace the rule \ruleref{src:fwd} with [[\ruleref{src:fwd} below \ruleref{inv:bck}] below \ruleref{inv:fwd}].
		\item \ruleref{inv:fwd} and \ruleref{inv:bck} can be pushed up through any of the remaining rules except \ruleref{prism:fwd} and \ruleref{prism:bck}.
		We do this (except when an \ruleref{inv:bck} is part of [\ruleref{src:fwd} below \ruleref{inv:bck}]), so that we only involute right before using a variable.
		All other rules (still read bottom-up) do not introduce opposite jets.
		...
		These do not turn an $\orif$-domain into an $\orifbe$-domain. Thus, until we encounter $\lnot \dm i$, we can assume the domain is also an $\orif$-jet-cube.
		\item If both domain and codomain are forward, we cannot encounter $\lnot \dm i$. \qedhere
	\end{enumerate}
\end{proof}
...
In the following sections, we investigate how polarized reshuffles can act on jet graphs (\cref{sec:poresh-jetgph}), jet topes and presheaves over these  (\cref{sec:poresh-jettope-jtset}), and jet cubes and presheaves over these (\cref{sec:poresh-jetcube-jcset}).
...
Viewing poset-valued presheaves $P : \catC\op \to \Poset$ on a 2-poset $\catC$ as profunctors to the terminal 2-poset $\top$, the diagram we will construct is the following:
\[
	\xymatrix{
		&& \polcat
			\ar[rrdd]|{/}^(.6){\oripsh}
...
There will be 4 polarities $\isopol$, $\pospol$, $\negpol$ and $\mixpol$ which will ask for an infrajet, forward jet, backward jet and an equijet respectively.
...
Finally, we can generalize the above question: for a $p_i$-modal function $f$, in order to get $f(x) \mathrel{Q} f(y)$, what do I need between $x$ and $y$? Thus, we get an operation that takes an orientation $Q$ and a polarized degree $p_i$ and answers with a polarized orientation $P_i$; this is the filler of the triangular diagram.
...
Hence, for a modality $\mu$ to be consistent, we will require that $\jetsym_i \oriof \mu \leq \equijetsym_{i+1} \oriof \mu$.
...
This is the motivation of the somewhat peculiar relation $\ll$ introduced below, which is always transitive, and is reflexive on symmetric objects and antireflexive on non-symmetric objects.
...
\begin{definition} \label{def:polarity-cat}
	...
	\begin{itemize}
		...
		The identities are $\id_\ypolar = \pospol$ and $\id_\npolar = \nonpol$ and composition is defined in \cref{fig:polarity-orientation,fig:polarity-expanded} and is monotonically increasing in both arguments.
		
		We call these polarities: $\mixpol$ mixed-polar, $\pospol$ positive, $\negpol$ negative, $\isopol$ isopolar and $\nonpol$ non-polar.
	\end{itemize}
\end{definition}
\begin{notation} \label{not:polarmode-reminder-polarity}
	In the spirit of \cref{not:polarmode-reminder},
	if $p \in \Hom_\polcat(a, b)$, we will write $p \pmrhom a b$ to remind of its domain and codomain.
\end{notation}
\begin{definition} \label{def:orientation-psh}
	...
	The action on morphisms $\oripsh(p)$ is denoted as $\loch \oriof p$ and given in \cref{fig:polarity-orientation,fig:polarity-orientation-expanded}; it is monotonically increasing in both arguments.\footnote{So the domain of $\oripsh : \polcat\op \to \Poset$ has reversed 1-cells but not reversed 2-cells / inequalities.}
\end{definition}
\begin{notation} \label{not:polarmode-reminder-orientation}
	In the spirit of \cref{not:polarmode-reminder},
	if $P \in \oripsh(a)$, we will write $P \pmr a$ to remind of its polarmode.
\end{notation}
\begin{figure}[htb]
	\[
		\begin{array}{c | c c c c}
			{\downarrow} \circ {\rightarrow} &
			          \mixpol \pmrhom{a}{b} & \pospol \pmrhom{a}{b} & \negpol \pmrhom{a}{b} & \isopol \pmrhom{a}{b}
			        \\ \hline
			\mixpol \pmrhom{b}{c} & \mixpol \pmrhom{a}{c} & \mixpol \pmrhom{a}{c} & \mixpol \pmrhom{a}{c} & \isopol \pmrhom{a}{c}
			        \\
			\pospol \pmrhom{b}{c} & \mixpol \pmrhom{a}{c} & \pospol \pmrhom{a}{c} & \negpol \pmrhom{a}{c} & \isopol \pmrhom{a}{c}
			        \\
			\negpol \pmrhom{b}{c} & \mixpol \pmrhom{a}{c} & \negpol \pmrhom{a}{c} & \pospol \pmrhom{a}{c} & \isopol \pmrhom{a}{c}
			        \\
			\isopol \pmrhom{b}{c} & \mixpol \pmrhom{a}{c} & \isopol \pmrhom{a}{c} & \isopol \pmrhom{a}{c} & \isopol \pmrhom{a}{c}
			        \\
		\end{array}
		\qquad \qquad
		\begin{array}{c | c c c c}
			{\downarrow} \oriof {\rightarrow} &
			          \mixpol \pmrhom{a}{b} & \pospol \pmrhom{a}{b} & \negpol \pmrhom{a}{b} & \isopol \pmrhom{a}{b}
			        \\ \hline
			\equijetsym \pmr{b} & \equijetsym \pmr{a} & \equijetsym \pmr{a} & \equijetsym \pmr{a} & \infrajetsym \pmr{a}
			        \\
			\jetsym \pmr{b} & \equijetsym \pmr{a} & \jetsym \pmr{a} & \tejsym \pmr{a} & \infrajetsym \pmr{a}
			        \\
			\tejsym \pmr{b} & \equijetsym \pmr{a} & \tejsym \pmr{a} & \jetsym \pmr{a} & \infrajetsym \pmr{a}
			        \\
			\infrajetsym \pmr{b} & \equijetsym \pmr{a} & \infrajetsym \pmr{a} & \infrajetsym \pmr{a} & \infrajetsym \pmr{a}
			        \\
		\end{array}
	\]
...
\begin{figure}[htb]
	\[
		\begin{array}{c | c c c c | c c | c | c}
			{\downarrow} \circ {\rightarrow} &
			\mixpol \pmryy & \pospol \pmryy & \negpol \pmryy & \isopol \pmryy &
			\mixpol \pmryn & \isopol \pmryn &
			\nonpol \pmrny &
			\nonpol \pmrnn
					\\ \hline
			\mixpol \pmryy &
			\mixpol \pmryy & \mixpol \pmryy & \mixpol \pmryy & \isopol \pmryy &
			&&
			\nonpol \pmrny &
					\\
			\pospol \pmryy &
			...
			\nonpol \pmrny &
					\\
			\negpol \pmryy &
			\mixpol \pmryy & \negpol \pmryy & \pospol \pmryy & \isopol \pmryy &
			&&
			\nonpol \pmrny &
					\\
			\isopol \pmryy &
			\mixpol \pmryy & \isopol \pmryy & \isopol \pmryy & \isopol \pmryy &
			&&
			\nonpol \pmrny &
					\\ \hline
			\mixpol \pmryn &
			\mixpol \pmryn & \mixpol \pmryn & \mixpol \pmryn & \isopol \pmryn &
			&&
			\nonpol \pmrnn &
					\\
			\isopol \pmryn &
			\mixpol \pmryn & \isopol \pmryn & \isopol \pmryn & \isopol \pmryn &
			&&
			\nonpol \pmrnn &
					\\ \hline
			\nonpol \pmrny &
			&&&&
			\mixpol \pmryy & \isopol \pmryy &
			&
			\nonpol \pmrny
					\\ \hline
			\nonpol \pmrnn &
		...
		\end{array}
	\]
	\caption{Composition of polarities, expanded.}
	\label{fig:polarity-expanded}
\end{figure}
\begin{figure}[htb]
	\[
		\begin{array}{c | c c c c | c c | c | c}
			...
			\nonpol \pmrnn
					\\ \hline
			\equijetsym \pmry & \equijetsym \pmry & \equijetsym \pmry & \equijetsym \pmry & \infrajetsym \pmry &
			&&
			\edgesym \pmrn &
			        \\
			\jetsym \pmry & \equijetsym \pmry & \jetsym \pmry & \tejsym \pmry & \infrajetsym \pmry &
			&&
			\edgesym \pmrn &
			        \\
			\tejsym \pmry & \equijetsym \pmry & \tejsym \pmry & \jetsym \pmry & \infrajetsym \pmry &
			&&
			\edgesym \pmrn &
			        \\
			\infrajetsym \pmry & \equijetsym \pmry & \infrajetsym \pmry & \infrajetsym \pmry & \infrajetsym \pmry &
			&&
			\edgesym \pmrn &
					\\ \hline
			\edgesym \pmrn &
			&&&&
			\equijetsym \pmry & \infrajetsym \pmry &
			&
			\edgesym \pmrn
		\end{array}
	\]
	\caption{Action of polarities on orientations, expanded.}
	\label{fig:polarity-orientation-expanded}
\end{figure}
\begin{proposition}
	The operations in \cref{fig:polarity-orientation} are well-defined.
\end{proposition}
\begin{proof}
	This is evident from the expansion in \cref{fig:polarity-expanded,fig:polarity-orientation-expanded}.
	
	Alternatively, we prove this for composition in $\polcat$; the proof for $\oripsh$ is analogous.
	Consider a composition $a \xrightarrow{p} b \xrightarrow{q} c$.
	Special situations may occur if either $a$, $b$ or $c$ equals $\npolar$.
	\begin{itemize}
		\item If $a = \npolar$, then $p = \nonpol \pmrhom \npolar b$ and we need to make sense of $q \pmrhom b c \circ \nonpol \pmrhom \npolar b$, which also equals $\nonpol \pmrhom \npolar c$.
		\item If $b = \npolar$, then $q = \nonpol \pmrhom \npolar c$ and $p \in \accol{\mixpol \pmrhom a \npolar, \isopol \pmrhom a \npolar}$ and we need to make sense of $\nonpol \pmrhom \npolar c \circ p \pmrhom a \npolar$, which according to \cref{fig:polarity-orientation} equals $p \pmrhom a c$ (so the same polarity $p$ but used at a different codomain).
		\item If $c = \npolar$, then $q \in \accol{\mixpol \pmrhom b \npolar, \isopol \pmrhom b \npolar}$ and we need that $q \pmrhom b \npolar \circ p \pmrhom a b \in \accol{\mixpol \pmrhom a \npolar, \isopol \pmrhom a \npolar}$, which is indeed the case. \qedhere
	\end{itemize}
\end{proof}
\begin{proposition}
	...
	\end{itemize}
\end{proposition}
\begin{proposition} \label{thm:oripsh-yoneda}
...
\begin{proposition}
	...
	Composition of polarities and their action on orientations both preserve (binary) meets and joins in the \emph{left} argument. \qed
\end{proposition}
...
\begin{definition} \label{def:precede-polarity}
	We call a polarity $p : a \to b$ \textbf{symmetric} if $\tejsym \pmr b \oriof p = \jetsym \pmr b \oriof p$, which is the case if $a = \npolar$ or $b = \npolar$ or $p \in \accol{\isopol \pmrhom a b, \mixpol \pmrhom a b}$.
	
	For $p : a \to b$ and $q : a \to c$,\footnote{Note that for any anpolarities $a$ and $b$, $\Hom(a, b)$ is a full subposet of $\Hom(a, \ypolar)$.} we write $p \ll q$ (``$p$ can precede $q$'' / ``$q$ can succeed $p$'') if one of the following, equivalent conditions is satisfied:
	\begin{enumerate}
		\item $\jetsym\pmr{b} \oriof p \leq \jetsym\pmr{c} \oriof q$ and $\jetsym \oriof p \leq \tejsym \oriof q$,
		\item $\jetsym\pmr{b} \oriof p \leq \jetsym\pmr{c} \oriof q$ and $\tejsym \oriof p \leq \jetsym \oriof q$,
		\item $\jetsym\pmr{b} \oriof p \leq \equijetsym\pmr{c} \oriof q$,
		\item $\infrajetsym\pmr{b} \oriof p \leq \jetsym\pmr{c} \oriof q$,
		\item $\infrajetsym\pmr{b} \oriof p \leq \equijetsym\pmr{c} \oriof q$,
		\item $p \leq q$ in $\Hom_\polcat(a, \ypolar)$ and at least either of them is symmetric,
		\item $p = \mixpol\pmrhom{a}{b}$ or $q = \isopol\pmrhom{a}{c}$.
	\end{enumerate}
	Note that all these conditions are vacuously satisfied if $a = \npolar$.
\end{definition}
\begin{proof}[Proof of equivalence]
	\begin{description}
		...
		\item[$1 \Rightarrow 6$] (This proof assumes $a = \ypolar$; otherwise both premise and conclusion are vacuous.) Condition 1 implies $p \leq q$ because $\jetsym^x \oriof \loch : \Hom_\polcat(\ypolar, x) \to \oripsh(\ypolar)$ is a full poset injection. If $p \leq q$ but neither is symmetric, then $p = q \in \accol{\pospol \pmryy \neq \negpol \pmryy}$, contradicting 1.
		\item[$6 \Rightarrow 7$] If $p$ is symmetric, then either $p = \mixpol$ (done) or $p = \isopol \leq q$ so that $q = \isopol$ (done). If $q$ is symmetric, then either $q = \isopol$ (done) or $q = \mixpol \geq p$ so that $p = \mixpol$ (done).
		\item[$7 \Rightarrow 1 \wedge \ldots \wedge 5$] If $p = \mixpol$, all the left hand sides of all the listed inequalities are $\equijet$, the minimal element. If $q = \isopol$, then all the right hand sides are $\infrajet$, the maximal element. \qedhere
	\end{description}
\end{proof}

Using \cref{thm:oripsh-yoneda}, we extend the definition of symmetry from polarities to orientations:
\begin{definition}
...
\begin{definition}
	Given a polarmode mask $\vec a$, we define the following posets of \textbf{(generalized) degrees}:
	\begin{itemize}
...
\end{definition}
\begin{notation} \label{not:polarmode-reminder-poldeg}
	If $p_i \in \poldegeqtop(\vec a, b)$ (or one of its subsets), then we write $p_i\pmrhom{a_i}{b}$ for $(p \pmrhom{a_i}{b})_i$, i.e.\ the polarmode reminder concerns the polarity, not the polarized degree as a whole.
	The latter will be denoted $(p_i) \pmrhom{\vec a}{b}$ instead.
\end{notation}
\begin{corollary}
...
%		We abbreviate the last condition above as $j \cdot \mu \ll j' \cdot \mu$.
		\begin{itemize}
			...
			\begin{equation} \label{eq:poldeg-poresh}
				\loch \circ \mu : (\dpol j q : \poldegeqtop(\vec b, c)) \to \poldegeqtop(\vec a, c) : (\dpol j q)\pmrhom{\vec b}{c} \mapsto q\pmrhom{b_j}{c} \circ (j \cdot \mu)\pmrhom{\vec a}{b_j}.
			\end{equation}
			This has the property that if $\dpol j q \ll \dpol{{j'}}{q'}$, then $\dpol j q \circ \mu \ll \dpol{{j'}}{q'} \circ \mu$.
			\item The identity $\id : \vec a \to \vec a$ is given by $i \cdot \id := \dpol i \pospol \pmrhom{a_i}{a_i}$.
	...
	\end{itemize}
	We will also denote modalities $\mu : \vec a \to \vec b$ as $\mu = \reshlist{\eqtydeg \cdot \mu}{0 \cdot \mu, \ldots, (\lenpol{\vec a} - 1) \cdot \mu}$, e.g.\ $\id_{[\ypolar, \npolar, \ypolar]} = \reshlist{\pdeg \nonpol \eqtydeg \pmrnn}{\pdeg \pospol 0 \pmryy, \pdeg \nonpol 1 \pmrnn, \pdeg \pospol 2 \pmryy}$.
\end{definition}
\begin{corollary} \label{thm:deg-vs-poldeg-poresh}
	Let $\mu \in \Hom_{\poreshcat}(\vec a, \vec b)$. Then we have $(\pospol_j \pmrhom{b_j}{b_j})\pmrhom{\vec b}{b_j} \circ \mu\pmrhom{\vec a}{\vec b} = (j \cdot \mu)\pmrhom{\vec a}{b_j}$. \qed
\end{corollary}
\begin{proposition}
...
\begin{proof}
	...
	\begin{align*}
		(\pdeg p i)\pmrhom{\vec a}{b} \circ \id_{\vec a}
		&= p\pmrhom{a_i}{b} \circ (i \cdot \id_{\vec a})\pmrhom{\vec a}{a_i}
		= p\pmrhom{a_i}{b} \circ (\pdeg \pospol i\pmrhom{a_i}{a_i})\pmrhom{\vec a}{a_i}
		= \paren{  \pdeg {(p\pmrhom{a_i}{b} \circ \pospol\pmrhom{a_i}{a_i})}{i}  }\pmrhom{\vec a}{b}
		= (\pdeg p i)\pmrhom{\vec a}{b}.
	\end{align*}
	To see that functoriality on both sides commutes:
	\begin{align*}
		(r\pmrhom{c}{d} \circ (\pdeg q j)\pmrhom{\vec b}{c}) \circ \mu \pmrhom{\vec a}{\vec b}
		&= \pdeg{(r\pmrhom{c}{d} \circ q\pmrhom{b_j}{c})}{j} \circ \mu \pmrhom{\vec a}{\vec b}
		& \text{(\cref{def:poldeg})} \\
		&= (r\pmrhom{c}{d} \circ q\pmrhom{b_j}{c}) \circ (j \cdot \mu)\pmrhom{\vec a}{b_j}
	...
	For composition:
	\begin{align*}
		(\pdeg r k)\pmrhom{\vec c}{d} \circ (\nu\pmrhom{\vec b}{\vec c} \circ \mu\pmrhom{\vec a}{\vec b})
		&= r\pmrhom{c_k}{d} \circ (k \cdot (\nu\pmrhom{\vec b}{\vec c} \circ \mu\pmrhom{\vec a}{\vec b}))\pmrhom{\vec a}{c_k}
		& \text{(\cref{eq:poldeg-poresh})} \\
		&= r\pmrhom{c_k}{d} \circ ((k \cdot \nu)\pmrhom{\vec b}{c_k} \circ \mu\pmrhom{\vec a}{\vec b})
...
\begin{proposition}
	...
	\begin{enumerate}
		...
		\item The action $\pdeg q j \circ \mu$ of polarized degrees on polarized reshuffles, preserves (binary) meets and joins in its left argument.
		\item Composition of polarized reshuffles, preserves (binary) meets and joins in its left argument.
	\end{enumerate}
\end{proposition}
\begin{proof}
	\begin{enumerate}
		...
		For polarized degrees with the same degree, this follows, via \cref{eq:poldeg-poresh}, from \cref{thm:pol-poldeg-lattice}.
...
\end{definition}
\begin{notation} \label{not:polarmode-reminder-orideg}
	If $P_i \in \oridegeqtop(\vec a)$ (or one of its subsets), then we write $P_i\pmr{a_i}$ for $(p \pmr{a_i})_i$, i.e.\ the polarmode reminder concerns the orientation, not the oriented degree as a whole.
	The latter will be denoted $(P_i) \pmr{\vec a}$ instead.
\end{notation}
...
\subsubsection{Adjunctions of Polarities}
\begin{theorem} \label{thm:pol-adjunctions}
	We give an exhaustive list of adjunctions of polarities:
	\[
		\begin{array}{l c l c l c l c l c l c l c l}
			\pospol \pmryy &\dashv& \pospol \pmryy
			\\
			\negpol \pmryy &\dashv& \negpol \pmryy
			\\
			\isopol \pmryy &\dashv& \mixpol \pmryy
			&\qquad\qquad&
			\isopol \pmryn &\dashv& \nonpol \pmrny
			&\qquad\qquad&
			\nonpol \pmrny &\dashv& \mixpol \pmryn
			&\qquad\qquad&
			\nonpol \pmrnn &\dashv& \nonpol \pmrnn
			\\
			 &\not\dashv& \isopol \pmryy
			&\qquad\qquad&
			&&
			&\qquad\qquad&
			 &\not\dashv& \isopol \pmryn
			\\
			\mixpol \pmryy &\not\dashv&
			&\qquad\qquad&
			\mixpol \pmryn &\not\dashv&
...
\end{theorem}
\begin{proof}
	The (co-)unit inequalities for the listed adjunctions, can be straightforwardly checked.
	We prove the non-adjunctions:
	\begin{description}
		\item[\framebox{$\not\dashv \isopol \pmryy$}] Co-unit fails: There is no $\ell \pmryy$ such that $\isopol \pmryy = \ell \pmryy \circ \isopol \pmryy \leq \pospol \pmryy$.
		\item[\framebox{$\mixpol \pmryy \not\dashv$}] Unit fails: There is no $r \pmryy$ such that $\pospol \pmryy \leq r \pmryy \circ \mixpol \pmryy = \mixpol \pmryy$.
		\item[\framebox{$\not\dashv \isopol \pmryn$}] Co-unit fails: $\isopol \pmryy = \nonpol \pmrny \circ \isopol \pmryn \not\leq \pospol \pmryy$.
		\item[\framebox{$\mixpol \pmryn \not\dashv$}] Unit fails: $\pospol \pmryy \not\leq \nonpol \pmrny \circ \mixpol \pmryn = \mixpol \pmryy$.
	\end{description}
	Since every polarity can be looked up in this table both on the left and on the right of a non-adjunction, our list is exhaustive.
\end{proof}

\subsubsection{Characterization}
\dontuse{
We need an auxiliary categorical fact:
\begin{lemma}\label{thm:cat:restricted-adjoint-precomposition}
	Given adjoint functors $L \dashv R$
	\[
		\xymatrix{
			\catI
				\ar@{^{(}->}[r]
	...
	\]
	between categories $\catC$ and $\catD$ and full subcategories $\catI \subseteq \catC$ and $\catJ \subseteq \catD$ such that the image of $L|_\catI$ lands in $\catJ$. Assume functors $F : \catC \to \catE$ and $G : \catD \to \catE$ (the triangle depicted above is not required to commute).
	Then:
	\begin{enumerate}
		\item If $L\cat I \subseteq \catJ$, then every natural transformation $\delta : FR|_\catJ \to G|_\catJ$ gives rise to a natural transformation $\gamma : F|_\catI \to GL|_\catI$.
		\item If $R\catJ \subseteq \catI$, then every natural transformation $\gamma : F|_\catI \to GL|_\catI$ gives rise to a natural transformation $\delta : FR|_\catJ \to G|_\catJ$.
		\item If both are true, so that $L|_\catI \dashv R|_\catJ$, then the above operations are inverse.
	\end{enumerate}
\end{lemma}
\begin{proof}
	\begin{enumerate}
		\item Let $L\cat I \subseteq \catJ$ and $\delta : FR|_\catJ \to G|_\catJ$. Then we have
		\[
			\xymatrix{
				F|_\catI
					\ar[r]^{F\eta|_\catI}
		...
		\]
		\item Let $R\catJ \subseteq \catI$ and $\gamma : F|_\catI \to GL|_\catI$. Then we have
		\[
			\xymatrix{
				FR|_\catJ
					\ar[r]^{\gamma R|_\catJ}
		...
		\]
		\item Write $(\bar L, \bar R, \bar \eta, \bar \eps)$ for the restricted adjunction. On one hand, we have
		\[
			\delta
			\mapsto \delta \bar L \circ F \bar \eta
			\mapsto G \bar \eps \circ (\delta \bar L \circ F \bar \eta) \bar R
			= G \bar \eps \circ \delta \bar L \bar R \circ F \bar \eta \bar R
			= \delta \circ F\bar R \bar \eps \circ F \bar \eta \bar R
			= \delta.
		\]
		On the other hand, we have
		\[
			\gamma
			\mapsto G \bar \eps \circ \gamma \bar R
			\mapsto (G \bar \eps \circ \gamma \bar R) \bar L \circ F \bar \eta
			= G \bar \eps \bar L \circ \gamma \bar R \bar L \circ F \bar \eta
			= G \bar \eps \bar L \circ G \bar L \bar \eta \circ \gamma = \gamma.
			\qedhere
		\]
	\end{enumerate}
\end{proof}
...
\begin{theorem}\label{thm:poresh-adjoint}
	\begin{enumerate}
		...
		\begin{itemize}
			\item We have $\loch \oriof \rho \dashv \loch \oriof \lambda : \oridegeqtop(\vec b) \to \oridegeqtop(\vec a)$:
			\begin{align}
				&\forall P_i \in \oridegeqtop(\vec a), Q_j \in \oridegeqtop(\vec b). \nn \\
				&\qquad (P_i)\pmr{\vec a} \leq (Q_j)\pmr{\vec b} \oriof \lambda \pmrhom{\vec a}{\vec b} \Leftrightarrow (P_i)\pmr{\vec a} \oriof \rho\pmrhom{\vec b}{\vec a} \leq (Q_j)\pmr{\vec b},
				\label{eq:poresh-adjoint-crit-orideg}
			\end{align}
			\item For each polarmode $c$, we have $\loch \circ \rho \dashv \loch \circ \lambda : \poldegeqtop(\vec b, c) \to \poldegeqtop(\vec a, c)$:
			\begin{align}
				&\forall c \in \Polarmode, p_i \in \poldegeqtop(\vec a, c), q_j \in \poldegeqtop(\vec b, c). \nn \\
				&\qquad (p_i)\pmrhom{\vec a}{c} \leq (q_j)\pmrhom{\vec b}{c} \circ \lambda\pmrhom{\vec a}{\vec b} \Leftrightarrow (p_i)\pmrhom{\vec a}{c} \circ \rho\pmrhom{\vec b}{\vec a} \leq (q_j)\pmrhom{\vec b}{c}.
				\label{eq:poresh-adjoint-crit-poldeg}
			\end{align}
		\end{itemize}
		\item By (non-trivial) consequence:
		\begin{itemize}
			\item A reshuffle $\rho\pmrhom{\vec b}{\vec a}$ has a left adjoint $\lambda\pmrhom{\vec a}{\vec b}$ if and only if
			\begin{align}
				&(\eqtydeg \cdot \rho)\pmrhom{\vec b}{\npolar} = \nonpol_\eqtydeg \pmrnn, \text{and} \label{eq:poresh-withl-crit1} \\
				&\text{for all } i \in \Degeq(\vec a) \text{ with } (i \cdot \rho)\pmrhom{\vec b}{a_i} = q_j\pmrhom{b_j}{a_i}, \text{the polarity $q$ has a left adjoint.}
				%&\text{for all } i \in \Degeq(\vec a) \text{ with } i \cdot \rho = \isopol_j\pmrhom{b_j}{a_i} \text{, we have } b_j = \npolar \text{ (so $i \cdot \rho = \nonpol_j\pmrhom{\npolar}{a_i}$)}.
				%&\text{for all } \jetsym_i \in \oripshfwd(\vec a) \text{ with } a_i = \ypolar \text{, we have } \jetsym_i \cdot \rho \neq \infrajetsym_j.
				\label{eq:poresh-withl-crit2}
			\end{align}
			In this case,
			\begin{itemize}
				...
				\item for any $q_j \in \poldegeqtop(\vec b, c)$, we find $q_j \circ \lambda$ as the greatest polarized degree $p_i \in \poldegeqtop(\vec a, c)$ such that $p_i \circ \rho \leq q_j$. This allows us to compute $\lambda$ via $j \cdot \lambda = \pospol_j \circ \lambda$ (\cref{thm:deg-vs-poldeg-poresh}).
			\end{itemize}
			
			\item A reshuffle $\lambda$ has a right adjoint $\rho$ if and only if
			\begin{align}
				&\text{for all } j \in \Degeq(\vec b), \text{we have } (j \cdot \lambda)\pmrhom{\vec a}{b_j} < \nonpol_\top \pmrhom{\npolar}{b_j}, \text{and} \label{eq:poresh-withr-crit1} \\
				&\text{for all } j \in \Degeq(\vec b) \text{ with } (j \cdot \lambda)\pmrhom{\vec a}{b_j} = p_i \pmrhom{a_i}{b_j}, \text{the polarity $p$ has a right adjoint}.
				%&\text{for all } j \in \Degeq(\vec b) \text{ with } j \cdot \lambda = \mixpol_i\pmrhom{a_i}{b_j} \text{, we have } a_i = \npolar \text{ (so $j \cdot \lambda = \nonpol_i\pmrhom{\npolar}{b_j}$)}.
				\label{eq:poresh-withr-crit2}
			\end{align}
			In this case,
			\begin{itemize}
				...
				\item for any $p_i \in \poldegeqtop(\vec a, c)$, we find $p_i \circ \rho$ as the least polarized degree $q_j \in \poldegeqtop(\vec b, c)$ such that $p_i \leq q_j \circ \lambda$. This allows us to compute $\rho$ via $i \cdot \rho = \pospol_i \circ \rho$ (\cref{thm:deg-vs-poldeg-poresh}).
			\end{itemize}
		\end{itemize}
	\end{enumerate}
\end{theorem}
...
%			& \text{($\jetsym \oriof \loch$ is an isomorphism)} \\
		...
		\begin{equation}
			\loch \circ \rho \qquad \dashv \qquad \ell_c \quad : \quad \poldegeqtop(\vec b, c) \to \poldegeqtop(\vec a, c).
		\end{equation}
		We define a polarized reshuffle $\lambda \in \Hom(\vec a, \vec b)$ by
		\begin{equation}
			j \cdot \lambda := \ell_{b_j}(\pospol_j) \in \poldegeqtop(\vec a, b_j) \qquad \text{for all $j \in \Degeq(\vec b)$}.
		\end{equation}
		Note that the above equation even extends to the case where $j = \top$: on one hand, $\top \cdot \lambda = \top$ by definition of $\top \cdot \loch$.
		On the other hand, it is not hard to check that $\ell_c(\nonpol_\top) = \nonpol_\top$.
		
		We have to show that $\ell$ is the action of $\lambda$ if and only if \cref{eq:poresh-withl-crit2} holds.
		\begin{itemize}
			\item To prove `only if', assume that \cref{eq:poresh-withl-crit2} does not hold (which is decidable). Then from \cref{thm:pol-adjunctions}, we know we can find some $i \in \Degeq(\vec a)$ such that $i \cdot \rho\pmrhom{\vec b}{\vec a} = \isopol_j\pmrhom{b_j}{a_i} \in \poldegeqtop(\vec b, a_i)$ with $b_j = \ypolar$.
			
			This implies that $(\isopol_i)\pmrhom{\vec a}{\ypolar} \circ \rho\pmrhom{\vec b}{\vec a} = \isopol_j\pmryy \in \poldegeqtop(\vec b, \ypolar)$ and hence, by the adjunction $\loch \circ \rho \dashv \ell$, that $\isopol_i \leq \ell_{\ypolar}(\isopol_j)$.
			
			On the other hand, we have that $(\mixpol_i)\pmrhom{\vec a}{\ypolar} \circ \rho\pmrhom{\vec b}{\vec a} = \isopol_j\pmryy \not\leq \pospol_j\pmryy \in \poldegeqtop(\vec b, \ypolar)$ and hence, by the adjunction $\loch \circ \rho \dashv \ell$, that $\mixpol_i \not\leq \ell_\ypolar(\pospol_j) = j \cdot \lambda \in \poldegeqtop(\vec a, \ypolar)$.
			...
			\item To prove `if', assume that \cref{eq:poresh-withl-crit2} holds. We prove that $\lambda \dashv \rho$. This immediately implies that $\loch \circ \rho \dashv \loch \circ \lambda$ and since we already know that $\loch \circ \rho \dashv \ell$, by uniqueness of the adjoint, we find that $\ell = \loch \circ \lambda$.
			
			\begin{itemize}
				\item We first prove the co-unit inequality $\lambda \pmrhom{\vec a}{\vec b} \circ \rho \pmrhom{\vec b}{\vec a} \leq \id_{\vec b}$:
				\begin{align*}
					j \cdot (\lambda \pmrhom{\vec a}{\vec b} \circ \rho \pmrhom{\vec b}{\vec a})
					&= (j \cdot \lambda)\pmrhom{\vec a}{b_j} \circ \rho \pmrhom{\vec b}{\vec a}
					& \text{(def.\ of comp.\ in $\poreshcat$)}
					\\
					&= \ell_{b_j}(\pospol_j\pmrhom{b_j}{b_j})\pmrhom{\vec a}{b_j} \circ \rho \pmrhom{\vec b}{\vec a}
					& \text{(def.\ of $\lambda$)}
					\\
					&\leq \pospol_j\pmrhom{b_j}{b_j}
					& \text{(co-unit of $\loch \circ \rho \dashv \ell$)}
					\\
					&= j \cdot \id_{\vec b}.
				\end{align*}
				
				\item Next, we prove the unit inequality $\id_{\vec a} \leq \rho \pmrhom{\vec b}{\vec a} \circ \lambda \pmrhom{\vec a}{\vec b}$.
				To this end, we need to prove that for all $I \in \Degeq(\vec a)$, we have $(\pospol_I)\pmrhom{\vec a}{a_I} \leq (I \cdot \rho)\pmrhom{\vec b}{a_I} \circ \lambda \pmrhom{\vec a}{\vec b}$.
				Using \cref{thm:deg-vs-poldeg-poresh}, this becomes:
				\[
					\forall I \in \Degeq(\vec a). \qquad
					(\pospol_I)\pmrhom{\vec a}{a_I}
					\leq
					((\pospol_I)\pmrhom{\vec a}{a_I} \circ \rho \pmrhom{\vec b}{\vec a}) \circ \lambda \pmrhom{\vec a}{\vec b}.
				\]
				By the unit of $\loch \circ \rho \dashv \ell$, it is sufficient to prove:
				\[
					\forall I \in \Degeq(\vec a). \qquad
					\ell_{a_I}((\pospol_I)\pmrhom{\vec a}{a_I} \circ \rho \pmrhom{\vec b}{\vec a})
					\leq
					((\pospol_I)\pmrhom{\vec a}{a_I} \circ \rho \pmrhom{\vec b}{\vec a}) \circ \lambda \pmrhom{\vec a}{\vec b}.
				\]
				Now fix $I$, write $c = a_I$ and $(q_j)\pmrhom{\vec b}{c} := I \cdot \rho\pmrhom{\vec b}{\vec a} = (\pospol_I)\pmrhom{\vec a}{a_I} \circ \rho \pmrhom{\vec b}{\vec a}$.
				We need to prove:
				\[
					\ell_{c}(q_j)\pmrhom{\vec a}{c}
					\leq
					(q_j)\pmrhom{\vec b}{c} \circ \lambda \pmrhom{\vec a}{\vec b}.
				\]
				The right hand side expands to $(q_j)\pmrhom{\vec b}{c} \circ \lambda \pmrhom{\vec a}{\vec b} = q\pmrhom{b_j}{c} \circ (j \cdot \lambda)\pmrhom{\vec a}{b_j} = q\pmrhom{b_j}{c} \circ \ell_{b_j}(\pospol_j)\pmrhom{\vec a}{b_j}$ by \cref{eq:poldeg-poresh} and the definition of $\lambda$.
				
				By unfolding the definition of $\ell$ and noting that $q \circ \loch$ is monotonically increasing and therefore maximum-preserving, we need to prove:
				\begin{align*}
					&\forall p_i \in \poldegeqtop(\vec a, c) \text{ with $(p_i)\pmrhom{\vec a}{c} \circ \rho\pmrhom{\vec b}{\vec a} \leq (q_j)\pmrhom{\vec b}{c}$} .\\
					&\exists \hat p_{\hat i} \in \poldegeqtop(\vec a, b_j) \text{ with $(\hat p_{\hat i})\pmrhom{\vec a}{b_j} \circ \rho\pmrhom{\vec b}{\vec a} \leq (\pospol_j)\pmrhom{\vec b}{b_j}$ } .\\
					&(p_i)\pmrhom{\vec a}{c} \leq q\pmrhom{b_j}{c} \circ (\hat p_{\hat i})\pmrhom{\vec a}{b_j}.
				\end{align*}
				
				Because of \cref{eq:poresh-withl-crit2}, $q\pmrhom{b_j}{c}$ has a left adjoint $d\pmrhom{c}{b_j} \dashv q\pmrhom{b_j}{c}$.
				We set $\hat i = i$ and $\hat p \pmrhom{a_i}{b_j} = d\pmrhom{c}{b_j} \circ p\pmrhom{a_i}{c}$.
				Then the first required inequality
				\[
					(\hat p_{\hat i})\pmrhom{\vec a}{b_j} \circ \rho\pmrhom{\vec b}{\vec a} = d\pmrhom{c}{b_j} \circ (p_i)\pmrhom{\vec a}{c} \circ \rho\pmrhom{\vec b}{\vec a} \leq (\pospol_j)\pmrhom{\vec b}{b_j}
				\]
				follows by the adjunction $d \dashv q$ from the given inequality
				\[
					(p_i)\pmrhom{\vec a}{c} \circ \rho\pmrhom{\vec b}{\vec a} \leq q\pmrhom{b_j}{c} \circ (\pospol_j)\pmrhom{\vec b}{b_j} = (q_j)\pmrhom{\vec b}{c}.
				\]
				The second required inequality
				\[
					(p_i)\pmrhom{\vec a}{c} \leq q\pmrhom{b_j}{c} \circ (\hat p_{\hat i})\pmrhom{\vec a}{b_j} = q\pmrhom{b_j}{c} \circ d\pmrhom{c}{b_j} \circ (p_i)\pmrhom{\vec a}{c}
				\]
				follows immediately from the unit $\pospol\pmrhom{b_j}{b_j} \leq q\pmrhom{b_j}{c} \circ d\pmrhom{c}{b_j}$.
			\end{itemize}
		\end{itemize}
		...
		\begin{equation}
			r_c \qquad \dashv \qquad \loch \circ \lambda \quad : \quad  \poldegeqtop(\vec b, c) \to \poldegeqtop(\vec a, c).
		\end{equation}
		We define a polarized reshuffle $\rho \in \Hom(\vec b, \vec a)$ by
		\begin{equation} \label{eq:poresh-adjoint:def-rho}
			i \cdot \rho := r_{a_i}(\pospol_i) \in \poldegeqtop(\vec b, a_i)
			\quad \text{for all $i \in \Degeq(\vec a)$}.
		\end{equation}
		...
		\begin{itemize}
			...
			Then by the adjunction $r \dashv \loch \circ \lambda$, we can transpose $(\nonpol_\top)\pmrhom{\vec a}{b_j} \leq (\pospol_j)\pmrhom{\vec b}{b_j} \circ \lambda\pmrhom{\vec a}{\vec b}$ to obtain
			$r_{b_j}(\nonpol_\top)\pmrhom{\vec b}{b_j} \leq (\pospol_j)\pmrhom{\vec b}{b_j} < (\nonpol_\top)\pmrhom{\vec b}{b_j} = (\nonpol_\top)\pmrhom{\vec a}{b_j} \circ \rho\pmrhom{\vec b}{\vec a} \in \poldegeqtop(\vec b, b_j)$, implying that $r$ cannot be the action of $\rho$.
			
			\item Assume that \cref{eq:poresh-withr-crit2} does not hold (which is decidable).
			Then from \cref{thm:pol-adjunctions}, we know we can find some $j \in \Degeq(\vec b)$ such that $j \cdot \lambda = \mixpol_i \in \poldegeqtop(\vec a, b_j)$ with $a_i = \ypolar$.
			
			This implies that $(\mixpol_j)\pmrhom{\vec b}{\ypolar} \circ \lambda\pmrhom{\vec a}{\vec b} = \mixpol_i\pmryy \in \poldegeqtop(\vec a, \ypolar)$.
			Then by the adjunction $r \dashv \loch \circ \lambda$, we can transpose $(\mixpol_i)\pmrhom{\vec a}{\ypolar} \leq (\mixpol_j)\pmrhom{\vec b}{\ypolar} \circ \lambda\pmrhom{\vec a}{\vec b}$ to obtain
			$r_\ypolar(\mixpol_i)\pmrhom{\vec b}{\ypolar} \leq (\mixpol_j)\pmrhom{\vec b}{\ypolar}$.
			
			On the other hand, we have that $(\mixpol_j)\pmrhom{\vec b}{\ypolar} \circ \lambda\pmrhom{\vec a}{\vec b} = (\mixpol_i)\pmrhom{\vec a}{\ypolar} \not \geq (\pospol_i)\pmrhom{\vec a}{\ypolar} \in \poldegeqtop(\vec a, \ypolar)$ and hence, by the adjunction $r \dashv \loch \circ \lambda$, that $i \cdot \rho = r_\ypolar(\pospol_i)\pmrhom{\vec b}{\ypolar} \not\leq (\mixpol_j)\pmrhom{\vec b}{\ypolar} \in \poldegeqtop(\vec b, \ypolar)$.
			...
			We prove that $\lambda \dashv \rho$. This immediately implies that $\loch \circ \rho \dashv \loch \circ \lambda$ and since we already know that $r \dashv \loch \circ \lambda$, by uniqueness of the adjoint, we find that $r = \loch \circ \rho$.
			
			The fact that \cref{eq:poresh-withr-crit1} holds, implies that $r_c(\nonpol_\top) = \nonpol_\top$, so that the defining \cref{eq:poresh-adjoint:def-rho} of $i \cdot \rho$ even extends to the case where $i = \top$.
			\begin{itemize}
				\item We first prove the unit inequality $\id_{\vec a} \leq \rho \pmrhom{\vec b}{\vec a} \circ \lambda \pmrhom{\vec a}{\vec b}$.
				Let $i \in \Degeq(\vec a)$.
				\begin{align*}
					i \cdot \id_{\vec a}
					&= \pospol_i \pmrhom{a_i}{a_i} \\
					&\leq r_{a_i}(\pospol_i \pmrhom{a_i}{a_i})\pmrhom{\vec b}{a_i} \circ \lambda \pmrhom{\vec a}{\vec b}
					& \text{(unit of $r \dashv \loch \circ \lambda$)} \\
					&= (i \cdot \rho)\pmrhom{\vec b}{a_i} \circ \lambda \pmrhom{\vec a}{\vec b}
					& \text{(def.\ of $\rho$)} \\
					&= i \cdot (\rho \pmrhom{\vec b}{\vec a} \circ \lambda \pmrhom{\vec a}{\vec b})
					& \text{(def.\ of comp.\ in $\poreshcat$).}
				\end{align*}
				
				\item Next, we prove the co-unit inequality $\lambda \pmrhom{\vec a}{\vec b} \circ \rho \pmrhom{\vec b}{\vec a} \leq \id_{\vec b}$.
				To this end, we need to prove that for all $J \in \Degeq(\vec b)$, we have $(J \cdot \lambda)\pmrhom{\vec a}{b_J} \circ \rho \pmrhom{\vec b}{\vec a} \leq (\pospol_J)\pmrhom{\vec b}{b_J}$.
				Using \cref{thm:deg-vs-poldeg-poresh}, this becomes:
				\[
					\forall J \in \Degeq(\vec b). \qquad
					((\pospol_J)\pmrhom{\vec b}{b_J} \circ \lambda\pmrhom{\vec a}{\vec b}) \circ \rho \pmrhom{\vec b}{\vec a} \leq (\pospol_J)\pmrhom{\vec b}{b_J}.
				\]
				By the co-unit of $r \dashv \loch \circ \lambda$, it is sufficient to prove:
				\[
					\forall J \in \Degeq(\vec b). \qquad
					((\pospol_J)\pmrhom{\vec b}{b_J} \circ \lambda\pmrhom{\vec a}{\vec b}) \circ \rho \pmrhom{\vec b}{\vec a}
					\leq
					r_{b_J}((\pospol_J)\pmrhom{\vec b}{b_J} \circ \lambda\pmrhom{\vec a}{\vec b}).
				\]
				Now fix $J$, write $c = b_J$ and $(p_i)\pmrhom{\vec a}{c} := J \cdot \lambda\pmrhom{\vec a}{\vec b} = (\pospol_J)\pmrhom{\vec b}{b_J} \circ \lambda\pmrhom{\vec a}{\vec b}$.
				We need to prove:
				\[
					(p_i)\pmrhom{\vec a}{c} \circ \rho \pmrhom{\vec b}{\vec a}
					\leq
					r_{c}(p_i)\pmrhom{\vec b}{c}.
				\]
				The left hand side expands to $(p_i)\pmrhom{\vec a}{c} \circ \rho \pmrhom{\vec b}{\vec a} = p\pmrhom{a_i}{c} \circ (i \cdot \rho) \pmrhom{\vec b}{a_i} = p\pmrhom{a_i}{c} \circ r_{a_i}(\pospol_i)\pmrhom{\vec b}{a_i}$ by \cref{eq:poldeg-poresh} and the definition of $\rho$.\footnote{This is where we crucially use the fact that \cref{eq:poresh-withr-crit1} holds, by applying \cref{eq:poresh-adjoint:def-rho} even when $i = \top$.}
				So we need to prove:
				\[
					p\pmrhom{a_i}{c} \circ r_{a_i}(\pospol_i)\pmrhom{\vec b}{a_i}
					\leq
					r_{c}(p_i)\pmrhom{\vec b}{c}.
				\]
				
				By unfolding the definition of $r$ and noting that $p \circ \loch$ is monotonically increasing and therefore minimum-preserving, we need to prove:
				\begin{align*}
					&\forall q_j \in \poldegeqtop(\vec b, c) \text{ with } (p_i)\pmrhom{\vec a}{c} \leq (q_j)\pmrhom{\vec b}{c} \circ \lambda \pmrhom{\vec a}{\vec b} .\\
					&\exists \hat q_{\hat j} \in \poldegeqtop(\vec b, a_i) \text{ with } (\pospol_i)\pmrhom{\vec a}{a_i} \leq (\hat q_{\hat j})\pmrhom{\vec b}{a_i} \circ \lambda \pmrhom{\vec a}{\vec b} .\\
					& p\pmrhom{a_i}{c} \circ (\hat q_{\hat j})\pmrhom{\vec b}{a_i} \leq (q_j)\pmrhom{\vec b}{c}.
				\end{align*}
				
				Because of \cref{eq:poresh-withr-crit2}, $p\pmrhom{a_i}{c}$ has a right adjoint $p\pmrhom{a_i}{c} \dashv e\pmrhom{c}{a_i}$.
				We set $\hat j = j$ and $\hat q\pmrhom{b_j}{a_i} = e\pmrhom{c}{a_i} \circ q\pmrhom{b_j}{c}$.
				Then the first required inequality
				\[
					(\pospol_i)\pmrhom{\vec a}{a_i}
					\leq
					(\hat q_{\hat j})\pmrhom{\vec b}{a_i} \circ \lambda \pmrhom{\vec a}{\vec b}
					=
					e\pmrhom{c}{a_i} \circ (q_j)\pmrhom{\vec b}{c} \circ \lambda \pmrhom{\vec a}{\vec b}
				\]
				follows by the adjunction $p \dashv e$ from the given inequality
				\[
					(p_i)\pmrhom{\vec a}{c} = p\pmrhom{a_i}{c} \circ (\pospol_i)\pmrhom{\vec a}{a_i}
					\leq
					(q_j)\pmrhom{\vec b}{c} \circ \lambda \pmrhom{\vec a}{\vec b}.
				\]
				The second required inequality
				\[
					p\pmrhom{a_i}{c} \circ (\hat q_{\hat j})\pmrhom{\vec b}{a_i} = p\pmrhom{a_i}{c} \circ e\pmrhom{c}{a_i} \circ (q_j)\pmrhom{\vec b}{c} \leq (q_j)\pmrhom{\vec b}{c}
				\]
				follows immediately from the co-unit $p\pmrhom{a_i}{c} \circ e\pmrhom{c}{a_i} \leq \pospol\pmrhom{a_i}{a_i}$. \qedhere
			\end{itemize}
		\end{itemize}
	\end{enumerate}
\end{proof}

\subsubsection{Existence of a Second Adjoint}
\begin{definition} \label{def:suc-degree}
	If $\vec a$ is a polarmode mask, then $\Degeqtop(\vec a)$ is a finite linear order with at least two elements.
	\begin{enumerate}
		\item If $i < \top$, write $\sucdeg(i)$ for the successor of $i$. This is the minimum of all greater elements.
		\item If $i > \eqtydeg$, write $\predeg(i)$ for the predecessor of $i$.
		This is the maximum of all lesser elements.
	\end{enumerate}
\end{definition}
...
\begin{theorem} \label{thm:poresh-withll}
	...
	\begin{align}
		...
		&\text{$\forall j \in \Degeq(\vec b), i \in \Degeq(\vec a)$, if $(j \cdot \tau)\pmrhom{\vec a}{b_j} \leq \mixpol_i\pmrhom{a_i}{b_j}$ and $\getdeg(\sucdeg(j) \cdot \tau) > i$ then $b_j = \npolar$.} \label{eq:poresh-withll-crit2}
	\end{align}
\end{theorem}
...
\begin{itemize}
	...
	\begin{itemize}
		\item of the form $\mixpol_i\pmryn$ at a symmetric degree $j \in \Degeq(\vec b)$;
		\item or of the form $\pospol_i\pmryy$ or $\negpol_i\pmryy$ at an arbitrary degree $j \in \Degeq(\vec b)$;
		\item it already could not be of the form $\isopol_i\pmrhom{\ypolar}{b_j}$ as per \cref{eq:poresh-withl-crit2}, as $\isopol\pmrhom{\ypolar}{b_j}$ does not have a left adjoint;
	\end{itemize}
...
\begin{proof}
	...
	\begin{align*}
		...
		(\eqtydeg \cdot \rho)\pmrhom{\vec b}{\npolar} =
		(\nonpol_\eqtydeg)\pmrhom{\vec b}{\npolar} \\
		&\quad\Leftrightarrow\quad
		(\nonpol_\eqtydeg)\pmrhom{\vec a}{\npolar} \circ \rho\pmrhom{\vec b}{\vec a} =
		(\nonpol_\eqtydeg)\pmrhom{\vec b}{\npolar}
		& \text{(By \cref{thm:deg-vs-poldeg-poresh}.)} \\
		&\quad\Leftrightarrow\quad
		(\nonpol_\eqtydeg)\pmrhom{\vec a}{\ypolar} \circ \rho\pmrhom{\vec b}{\vec a} =
		(\nonpol_\eqtydeg)\pmrhom{\vec b}{\ypolar}
		& \text{(The poset inclusion $\poldegeq(\vec b, \npolar) \subseteq \poldegeq(\vec b, \ypolar)$ is full.)} \\
		&\quad\Leftrightarrow\quad
		(\nonpol_\eqtydeg)\pmrhom{\vec a}{\ypolar} \circ \rho\pmrhom{\vec b}{\vec a} <
		(\mixpol_{\sucdeg(\eqtydeg)})\pmrhom{\vec b}{\ypolar} \\
		&\quad\Leftrightarrow\quad
		(\nonpol_\eqtydeg)\pmrhom{\vec a}{\ypolar} \circ \rho\pmrhom{\vec b}{\vec a} <
		(\pospol_{\sucdeg(\eqtydeg)})\pmrhom{\vec b}{\ypolar}
		& \text{(Case distinction on $b_{\sucdeg(\eqtydeg)}$; $\rho$ satisfies \cref{eq:poresh-withr-crit2})} \\
		&\quad\Leftrightarrow\quad
		(\nonpol_\eqtydeg)\pmrhom{\vec a}{\ypolar} \circ \rho\pmrhom{\vec b}{\vec a} \not\geq
		(\pospol_{\sucdeg(\eqtydeg)})\pmrhom{\vec b}{\ypolar}
		& \text{(Sym.\ pol.\ degrees are comparable to everything)} \\
		&\quad\Leftrightarrow\quad
		(\nonpol_\eqtydeg)\pmrhom{\vec a}{\ypolar} \not\geq
		(\pospol_{\sucdeg(\eqtydeg)})\pmrhom{\vec b}{\ypolar} \circ \tau\pmrhom{\vec a}{\vec b}
		...
		&\quad\Leftrightarrow\quad
		\eqtydeg <
		\getdeg \paren{  (\pospol_{\sucdeg(\eqtydeg)})\pmrhom{\vec b}{\ypolar} \circ \tau\pmrhom{\vec a}{\vec b}  } = \getdeg({\sucdeg(\eqtydeg)} \cdot \tau).\hspace{-10em}
	\end{align*}
	...
	Using \cref{thm:pol-adjunctions}, we can rewrite \cref{eq:poresh-withl-crit2} as:
	\begin{align*}
%		\text{for all } i \in \Degeq(\vec a) \text{ with } (i \cdot \rho) = q_j\pmrhom{b_j}{a_i}, \text{the polarity $q$ has a left adjoint.}
		\text{for all } i \in \Degeq(\vec a) \text{ with } (i \cdot \rho)\pmrhom{\vec b}{a_i} = \isopol_j\pmrhom{b_j}{a_i} \text{, we have } b_j = \npolar \text{ (so $(i \cdot \rho)\pmrhom{\vec b}{a_i} = \nonpol_j\pmrhom{\npolar}{a_i}$)}.
	\end{align*}
	Since $\rho$ satisfies \cref{eq:poresh-withr-crit1}, we know that $j \neq \top$ and can regard $j$ as ranging over $\Degeq(\vec b)$.
	
	We can rewrite the condition $(i \cdot \rho)\pmrhom{\vec b}{a_i} = \isopol_j\pmrhom{b_j}{a_i}$ as $\isopol_j \leq (i \cdot \rho)\pmrhom{\vec b}{a_i}$ \textbf{AND} $\isopol_j \geq (i \cdot \rho)\pmrhom{\vec b}{a_i}$.
	%\todoi{Shortcut the reasoning below with $\isopol \dashv \mixpol$.}
	We have
	\begin{align*}
		&\quad\mathrel{\phantom{\Leftrightarrow}}\quad
		(\isopol_j)\pmrhom{\vec b}{a_i}
		\leq (i \cdot \rho)\pmrhom{\vec b}{a_i}
		= (\pospol_i)\pmrhom{\vec a}{a_i} \circ \rho\pmrhom{\vec b}{\vec a}
		& \text{(\cref{thm:deg-vs-poldeg-poresh})}
		\\ &\quad\Leftrightarrow\quad
		(\isopol_j)\pmrhom{\vec b}{a_i} \circ \tau\pmrhom{\vec a}{\vec b}
		\leq (\pospol_i)\pmrhom{\vec a}{a_i}
		& \text{($\loch \circ \tau \dashv \loch \circ \rho$)}
		\\ &\quad\Leftrightarrow\quad
		\isopol\pmrhom{b_j}{a_i} \circ (j \cdot \tau)\pmrhom{\vec a}{b_j}
		\leq (\pospol_i)\pmrhom{\vec a}{a_i}
		\\ &\quad\Leftrightarrow\quad
		(j \cdot \tau)\pmrhom{\vec a}{b_j}
		\leq \mixpol\pmrhom{a_i}{b_j} \circ (\pospol_i)\pmrhom{\vec a}{a_i}
		& \text{($\isopol\pmrhom{b_j}{a_i} \dashv \mixpol\pmrhom{a_i}{b_j}$)}
		\\ &\quad\Leftrightarrow\quad
		(j \cdot \tau)\pmrhom{\vec a}{b_j}
		\leq (\mixpol_i)\pmrhom{\vec a}{b_j}.
	\end{align*}
	and
	\begin{align*}
		&\quad\mathrel{\phantom{\Leftrightarrow}}\quad
		(\isopol_j)\pmrhom{\vec b}{a_i}
		\geq (i \cdot \rho)\pmrhom{\vec b}{a_i}
		= (\pospol_i)\pmrhom{\vec a}{a_i} \circ \rho\pmrhom{\vec b}{\vec a}
		\hspace{-10em}
		& \text{(By \cref{thm:deg-vs-poldeg-poresh}.)}
		\\ &\quad\Leftrightarrow\quad
		(\isopol_j)\pmrhom{\vec b}{\ypolar}
		\geq (\pospol_i)\pmrhom{\vec a}{\ypolar} \circ \rho\pmrhom{\vec b}{\vec a}
		& \text{The poset inclusion $\poldegeq(\vec b, \npolar) \subseteq \poldegeq(\vec b, \ypolar)$ is full.}
		\\ &\quad\Leftrightarrow\quad
		(\mixpol_{\sucdeg(j)})\pmrhom{\vec b}{\ypolar}
		> (\pospol_i)\pmrhom{\vec a}{\ypolar} \circ \rho\pmrhom{\vec b}{\vec a}
		%& \paren{\text{Let ${\sucdeg(j)} = \min \set{j' \in \Degeqtop(\vec b)}{j' > j}$}}
		\\ &\quad\Leftrightarrow\quad
		(\mixpol_{\sucdeg(j)})\pmrhom{\vec b}{\ypolar}
		\not\leq (\pospol_i)\pmrhom{\vec a}{\ypolar} \circ \rho\pmrhom{\vec b}{\vec a}
		& \text{(Sym.\ pol.\ degrees are comparable to everything)}
		\\ &\quad\Leftrightarrow\quad
		(\mixpol_{\sucdeg(j)})\pmrhom{\vec b}{\ypolar} \circ \tau\pmrhom{\vec a}{\vec b}
		\not\leq (\pospol_i)\pmrhom{\vec a}{\ypolar}
		& \text{($\loch \circ \tau \dashv \loch \circ \rho$)}
		\\ &\quad\Leftrightarrow\quad
		(\mixpol_{\sucdeg(j)})\pmrhom{\vec b}{\ypolar} \circ \tau\pmrhom{\vec a}{\vec b}
		> (\pospol_i)\pmrhom{\vec a}{\ypolar}
		& \text{(Sym.\ pol.\ degrees are comparable to everything)}
		\\ &\quad\Leftrightarrow\quad
		(\mixpol_{\getdeg({\sucdeg(j)} \cdot \tau)})\pmrhom{\vec a}{\ypolar}
		> (\pospol_i)\pmrhom{\vec a}{\ypolar}
...
\begin{theorem} \label{thm:poresh-withrr}
	...
	\begin{align}
		&\getdeg(\predeg(\top) \cdot \kappa) = \predeg(\top) \label{eq:poresh-withrr-crit1} \\
		&\text{$\forall j \in \Degeq(\vec b), i \in \Deg(\vec a)$, if $\getdeg(\predeg(i) \cdot \kappa) < j$ and $(i \cdot \kappa)\pmrhom{\vec b}{a_i} \geq \isopol_j\pmrhom{b_j}{a_i}$ then $a_i = \npolar$.} \label{eq:poresh-withrr-crit2}
	\end{align}
\end{theorem}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item it already could not be of the form $\mixpol_j$ as per \cref{eq:poresh-withr-crit2}, as $\mixpol$ does not have a right adjoint;
	\end{itemize}
...
\begin{proof}
	...
	\begin{align*}
		&\quad\mathrel{\phantom{\Leftrightarrow}}\quad
		(\predeg(\top) \cdot \lambda)\pmrhom{\vec a}{b_{\predeg(\top)}}
		< (\nonpol_\top)\pmrhom{\vec a}{b_{\predeg(\top)}}
		\\ &\quad\Leftrightarrow\quad
		(\pospol_{\predeg(\top)})\pmrhom{\vec b}{b_{\predeg(\top)}} \circ \lambda\pmrhom{\vec a}{\vec b}
		< (\nonpol_\top)\pmrhom{\vec a}{b_{\predeg(\top)}}
		&\text{(\Cref{thm:deg-vs-poldeg-poresh})}
		\\ &\quad\Leftrightarrow\quad
		(\pospol_{\predeg(\top)})\pmrhom{\vec b}{b_{\predeg(\top)}} \circ \lambda\pmrhom{\vec a}{\vec b}
		\leq (\isopol_{\predeg(\top)})\pmrhom{\vec a}{b_{\predeg(\top)}}
		\\ &\quad\Leftrightarrow\quad
		(\pospol_{\predeg(\top)})\pmrhom{\vec b}{b_{\predeg(\top)}}
		\leq (\isopol_{\predeg(\top)})\pmrhom{\vec a}{b_{\predeg(\top)}} \circ \kappa\pmrhom{\vec b}{\vec a}
		&\text{($\loch \circ \lambda \dashv \loch \circ \kappa$)}
		\\ &\quad\Leftrightarrow\quad
		(\pospol_{\predeg(\top)})\pmrhom{\vec b}{b_{\predeg(\top)}}
		\leq \isopol\pmrhom{a_{\predeg(\top)}}{b_{\predeg(\top)}} \circ (\predeg(\top) \cdot \kappa)\pmrhom{\vec b}{a_{\predeg(\top)}}
		\\ &\quad\Leftrightarrow\quad
		(\pospol_{\predeg(\top)})\pmrhom{\vec b}{b_{\predeg(\top)}}
		\leq (\isopol_{\getdeg(\predeg(\top) \cdot \kappa)})\pmrhom{\vec b}{b_{\predeg(\top)}}
		&\text{(Case dist.\ on $b_{\getdeg(\predeg(\top) \cdot \kappa)}$; $\kappa$ satisfies \cref{eq:poresh-withr-crit2})}
		\\ &\quad\Leftrightarrow\quad
		\predeg(\top) \leq \getdeg(\predeg(\top) \cdot \kappa).
	\end{align*}
	Since $\kappa$ satisfies \cref{eq:poresh-withr-crit1}, we can then conclude $\predeg(\top) = \getdeg(\predeg(\top) \cdot \kappa)$.
	
	Using \cref{thm:pol-adjunctions}, we can rewrite \cref{eq:poresh-withr-crit2} as:
	\[
		\text{for all } j \in \Degeq(\vec b) \text{ with } (j \cdot \lambda)\pmrhom{\vec a}{b_j} = \mixpol_i\pmrhom{a_i}{b_j} \text{, we have } a_i = \npolar \text{ (so $(j \cdot \lambda)\pmrhom{\vec a}{b_j} = \nonpol_i\pmrhom{\npolar}{b_j}$)}.
	\]
	Since $\lambda$ satisfies \cref{eq:poresh-withr-crit1}, we know that $i \neq \top$. Moreover, the case where $i = \eqtydeg$ is trivial, as $a_\eqtydeg = \npolar$ by definition. Hence, we can regard $i$ as ranging over $\Deg(\vec a)$.
	
	We can rewrite the condition as $(j \cdot \lambda)\pmrhom{\vec a}{b_j} \leq \mixpol_i$ \textbf{AND} $(j \cdot \lambda)\pmrhom{\vec a}{b_j} \geq \mixpol_i$. We have
	\begin{align*}
		&\quad\mathrel{\phantom{\Leftrightarrow}}\quad
		(\pospol_j)\pmrhom{\vec b}{b_j} \circ \lambda\pmrhom{\vec a}{\vec b} = (j \cdot \lambda)\pmrhom{\vec a}{b_j} \leq (\mixpol_i)\pmrhom{\vec a}{b_j}
		& \text{(\cref{thm:deg-vs-poldeg-poresh})}
		\\ &\quad\Leftrightarrow\quad
		(\pospol_j)\pmrhom{\vec b}{b_j} \leq (\mixpol_i)\pmrhom{\vec a}{b_j} \circ \kappa\pmrhom{\vec b}{\vec a}
		& \text{($\loch\circ  \lambda \dashv \loch \circ \kappa$)}
		\\ &\quad\Leftrightarrow\quad
		(\pospol_j)\pmrhom{\vec b}{b_j} \leq \mixpol\pmrhom{a_i}{b_j} \circ (i \cdot \kappa)\pmrhom{\vec b}{a_i}
		\\ &\quad\Leftrightarrow\quad
		\isopol\pmrhom{b_j}{a_i} \circ (\pospol_j)\pmrhom{\vec b}{b_j} \leq (i \cdot \kappa)\pmrhom{\vec b}{a_i}
		& \text{($\isopol\pmrhom{b_j}{a_i} \dashv \mixpol\pmrhom{a_i}{b_j}$)}
		\\ &\quad\Leftrightarrow\quad
		(\isopol_j)\pmrhom{\vec b}{a_i} \leq (i \cdot \kappa)\pmrhom{\vec b}{a_i}
	\end{align*}
	and
	\begin{align*}
		&\quad\mathrel{\phantom{\Leftrightarrow}}\quad
		(\pospol_j)\pmrhom{\vec b}{b_j} \circ \lambda\pmrhom{\vec a}{\vec b} = (j \cdot \lambda)\pmrhom{\vec a}{b_j} \geq (\mixpol_i)\pmrhom{\vec a}{b_j}
		& \text{(\cref{thm:deg-vs-poldeg-poresh})}
		\\ &\quad\Leftrightarrow\quad
		(\pospol_j)\pmrhom{\vec b}{b_j} \circ \lambda\pmrhom{\vec a}{\vec b} > (\isopol_{\predeg(i)})\pmrhom{\vec a}{b_j}
		\\ &\quad\Leftrightarrow\quad
		(\pospol_j)\pmrhom{\vec b}{b_j} \circ \lambda\pmrhom{\vec a}{\vec b} \not\leq (\isopol_{\predeg(i)})\pmrhom{\vec a}{b_j}
		& \text{(Sym.\ pol.\ degrees are comparable to everything)}
		\\ &\quad\Leftrightarrow\quad
		(\pospol_j)\pmrhom{\vec b}{b_j} \not\leq (\isopol_{\predeg(i)})\pmrhom{\vec a}{b_j} \circ \kappa\pmrhom{\vec b}{\vec a}
		& \text{($\loch\circ  \lambda \dashv \loch \circ \kappa$)}
		\\ &\quad\Leftrightarrow\quad
		(\pospol_j)\pmrhom{\vec b}{b_j} > (\isopol_{\predeg(i)})\pmrhom{\vec a}{b_j} \circ \kappa\pmrhom{\vec b}{\vec a}
		& \text{(Sym.\ pol.\ degrees are always comparable)}
		\\ &\quad\Leftrightarrow\quad
		(\pospol_j)\pmrhom{\vec b}{b_j} > (\isopol_{\getdeg(\predeg(i) \cdot \kappa)})\pmrhom{\vec b}{b_j}
...
\begin{example} \label{ex:poresh-op}
	Let $a_i = \ypolar$.
	Then we have a self-inverse and therefore self-adjoint polarized reshuffle $\Op_i : \vec a \to \vec a$ defined by $i \cdot \Op_i\pmrhom{\ypolar}{\ypolar} = \negpol_i$ and $j \cdot \Op_i = \pospol_j\pmrhom{a_j}{a_j}$ for all $i \neq j \in \Degeq(\vec a)$.
\end{example}
\begin{example} \label{ex:poresh-sym}
	...
	\[
		\Fsymresh_i : \vec b \to \vec a
		\qquad\dashv\qquad
		\Usymresh_i : \vec a \to \vec b
		\qquad\dashv\qquad
		\Csymresh_i : \vec b \to \vec a
	\]
	where $\Usymresh_i : \vec a \to \vec b$, defined by
	\begin{align*}
		i \cdot \Fsymresh_i &= \isopol_i\pmrhom{\ypolar}{\npolar}
		& i \cdot \Usymresh_i &= \nonpol_i\pmrhom{\npolar}{\ypolar}
		& i \cdot \Csymresh_i &= \mixpol_i\pmrhom{\ypolar}{\npolar}
		\\
		j \cdot \Fsymresh_i &= \pospol_i\pmrhom{b_j}{a_j}
		& j \cdot \Usymresh_i &= \pospol_i\pmrhom{a_j}{b_j}
		& j \cdot \Csymresh_i &= \pospol_i\pmrhom{b_j}{a_j}
	...
	\[
		\Fsymresh_i \circ \Usymresh_i = \id_{\vec a}, \qquad \Csymresh_i \circ \Usymresh_i = \id_{\vec a}.
	\]
	Thus, we get an idempotent monad $\UFsymresh_i = \Usymresh_i \circ \Fsymresh_i : \vec b \to \vec b$ and an idempotent comonad $\UCsymresh_i = \Usymresh_i \circ \Csymresh_i : \vec b \to \vec b$ such that $\UFsymresh_i \dashv \UCsymresh_i$.
	Both are given by
	\begin{align*}
		i \cdot \UFsymresh_i &= \isopol_i\pmrhom{\ypolar}{\ypolar}
		& i \cdot \UCsymresh_i &= \mixpol_i\pmrhom{\ypolar}{\ypolar}
		\\
		j \cdot \UFsymresh_i &= \pospol_j\pmrhom{b_j}{b_j}
		& j \cdot \UCsymresh_i &= \pospol_j\pmrhom{b_j}{b_j}
		& \text{for $i \neq j \in \Degeq(\vec b)$}
	\end{align*}
\end{example}
\begin{example} \label{ex:cohesion}
	...
	\[
		\cohfget_i = \reshlist{\nonpol_\eqtydeg\pmrhom{\npolar}{\npolar}}{\pospol_0\pmrhom{a_0}{b_0}, \ldots, \pospol_{i-1}\pmrhom{a_{i-1}}{b_{i-1}}, \pospol_{i+1}\pmrhom{a_{i+1}}{b_{i}}, \ldots, \pospol_{n-1}\pmrhom{a_{n-1}}{b_{n-2}}} : \vec a \to \vec b
	\]
	\begin{itemize}
		\item If $i = 0$, then $\cohfget_0$ has two left adjoints $\cohpi_0 \dashv \cohdisc_0 \dashv \cohfget_0$ with
		\begin{align*}
			\cohpi_0 &= \reshlist{\isopol_0\pmrhom{a_0}{\npolar}}{\pospol_1\pmrhom{a_1}{b_0}, \ldots, \pospol_{n-1}\pmrhom{a_{n-1}}{b_{n-2}}} &: \vec a \to \vec b, \\
			\dashv
			\cohdisc_0 &= \reshlist{\nonpol_\eqtydeg\pmrhom{\npolar}{\npolar}}{\nonpol_\eqtydeg\pmrhom{\npolar}{a_0}, \pospol_0\pmrhom{b_0}{a_1}, \ldots, \pospol_{n-2}\pmrhom{b_{n-2}}{a_{n-1}}} &: \vec b \to \vec a, \\
			\dashv
			\cohfget_0 &= \reshlist{\nonpol_\eqtydeg\pmrhom{\npolar}{\npolar}}{\pospol_1\pmrhom{a_1}{b_0}, \ldots, \pospol_{n-1}\pmrhom{a_{n-1}}{b_{n-2}}} &: \vec a \to \vec b.
		\end{align*}
		...
		\begin{align*}
			\shp_0 = \cohdisc_0 \cohpi_0 &= \reshlist{\isopol_0\pmrhom{a_0}{\npolar}}{\isopol_0\pmrhom{a_0}{a_0}, \pospol_1\pmrhom{a_1}{a_1}, \ldots, \pospol_{n-1}\pmrhom{a_{n-1}}{a_{n-1}}} &: \vec a \to \vec a, \\
			\dashv
			\flat_0 = \cohdisc_0 \cohfget_0 &= \reshlist{\nonpol_\eqtydeg\pmrhom{\npolar}{\npolar}}{\nonpol_\eqtydeg\pmrhom{\npolar}{a_0}, \pospol_1\pmrhom{a_1}{a_1}, \ldots, \pospol_{n-1}\pmrhom{a_{n-1}}{a_{n-1}}} &: \vec a \to \vec a.
		\end{align*}
		\item If $i > 0$, then $\cohfget_i$ has a left adjoint $\cohdisc_i \dashv \cohfget_i$ where
		\[
			\cohdisc_i = \reshlist{\nonpol_\eqtydeg\pmrhom{\npolar}{\npolar}}{\pospol_0\pmrhom{b_0}{a_0}, \ldots, \pospol_{i-1}\pmrhom{b_{i-1}}{a_{i-1}}, \isopol_{i-1}\pmrhom{b_{i-1}}{a_i}, \pospol_i\pmrhom{b_i}{a_{i+1}}, \ldots, \pospol_{n-2}\pmrhom{b_{n-2}}{a_{n-1}}} : \vec b \to \vec a.
		\]
		We have $\cohfget_i \cohdisc_i = \id_{\vec b}$. We get an idempotent comonad
		\[
			\flat_i = \cohdisc_i \cohfget_i = \reshlist{\nonpol_\eqtydeg\pmrhom{\npolar}{\npolar}}{\pospol_0\pmrhom{a_0}{a_0}, \ldots, \pospol_{i-1}\pmrhom{a_{i-1}}{a_{i-1}}, \isopol_{i-1}\pmrhom{a_{i-1}}{a_i}, \pospol_{i+1}\pmrhom{a_{i+1}}{a_{i+1}}, \ldots, \pospol_{n-1}\pmrhom{a_{n-1}}{a_{n-1}}} : \vec a \to \vec a
		\]
		\begin{itemize}
			...
			\begin{align*}
				\cohpi_i &= \reshlist{\nonpol_\eqtydeg\pmrhom{\npolar}{\npolar}}{\pospol_0\pmrhom{a_0}{b_0}, \ldots, \pospol_{i-2}\pmrhom{a_{i-2}}{b_{i-2}}, \isopol_i\pmrhom{a_i}{\npolar}, \pospol_{i+1}\pmrhom{a_{i+1}}{b_i}, \ldots, \pospol_{n-1}\pmrhom{a_{n-1}}{b_{n-2}}}
				&: \vec a \to \vec b, \\
				\dashv
				\cohdisc_i &= \reshlist{\nonpol_\eqtydeg\pmrhom{\npolar}{\npolar}}{\pospol_0\pmrhom{b_0}{a_0}, \ldots, \pospol_{i-2}\pmrhom{b_{i-2}}{a_{i-2}}, \nonpol_{i-1}\pmrhom{\npolar}{\npolar}, \nonpol_{i-1}\pmrhom{\npolar}{a_i}, \pospol_i\pmrhom{b_i}{a_{i+1}}, \ldots, \pospol_{n-2}\pmrhom{b_{n-2}}{a_{n-1}}}
				&: \vec b \to \vec a, \\
				\dashv
				\cohfget_i &= \reshlist{\nonpol_\eqtydeg\pmrhom{\npolar}{\npolar}}{\pospol_0\pmrhom{a_0}{b_0}, \ldots, \pospol_{i-2}\pmrhom{a_{i-2}}{b_{i-2}}, \nonpol_{i-1}\pmrhom{\npolar}{\npolar}, \pospol_{i+1}\pmrhom{a_{i+1}}{b_i}, \ldots, \pospol_{n-1}\pmrhom{a_{n-1}}{b_{n-2}}}
				&: \vec a \to \vec b.
			\end{align*}
			We have $\cohpi_i \cohdisc_i = \cohfget_i \cohdisc_i = \id_{\vec b}$. We get adjoint idempotent (co)monads
			\begin{align*}
				{}\hspace{-3em}
				\shp_i = \cohdisc_i \cohpi_i &= \reshlist{\nonpol_\eqtydeg\pmrhom{\npolar}{\npolar}}{\pospol_0\pmrhom{a_0}{a_0}, \ldots, \pospol_{i-2}\pmrhom{a_{i-2}}{a_{i-2}}, \isopol_i\pmrhom{a_i}{\npolar}, \isopol_i\pmrhom{a_i}{a_i}, \pospol_{i+1}\pmrhom{a_{i+1}}{a_{i+1}}, \ldots, \pospol_{n-1}\pmrhom{a_{n-1}}{a_{n-1}}} &: \vec a \to \vec a, \\
				{}\hspace{-3em}
				\dashv
				\flat_i = \cohdisc_i \cohfget_i &= \reshlist{\nonpol_\eqtydeg\pmrhom{\npolar}{\npolar}}{\pospol_0\pmrhom{a_0}{a_0}, \ldots, \pospol_{i-2}\pmrhom{a_{i-2}}{a_{i-2}}, \nonpol_{i-1}\pmrhom{\npolar}{\npolar}, \nonpol_{i-1}\pmrhom{\npolar}{a_i}, \pospol_{i+1}\pmrhom{a_{i+1}}{a_{i+1}}, \ldots, \pospol_{n-1}\pmrhom{a_{n-1}}{a_{n-1}}} &: \vec a \to \vec a.
			\end{align*}
			\begin{itemize}
		...
		\begin{align*}
			\cohfget_{n-1} &= \reshlist{\nonpol_\eqtydeg\pmrhom{\npolar}{\npolar}}{\pospol_0\pmrhom{a_0}{b_0}, \ldots, \pospol_{n-2}\pmrhom{a_{n-2}}{b_{n-2}}} &: \vec a \to \vec b, \\
			\dashv
			\cohcodisc_{n-1} &= \reshlist{\nonpol_\eqtydeg\pmrhom{\npolar}{\npolar}}{\pospol_0\pmrhom{b_0}{a_0}, \ldots, \pospol_{n-2}\pmrhom{b_{n-2}}{a_{n-2}}, \nonpol_\top\pmrhom{\npolar}{a_{n-1}}} &: \vec b \to \vec a,
		\end{align*}
		and there are no further right adjoints.
		We have $\cohfget_{n-1} \cohcodisc_{n-1} = \id_{\vec b}$.
		We get an idempotent monad
		\[
			\sharp_{n-1} = \cohcodisc_{n-1} \cohfget_{n-1} = \reshlist{\nonpol_\eqtydeg\pmrhom{\npolar}{\npolar}}{\pospol_0\pmrhom{a_0}{a_0}, \ldots, \pospol_{n-2}\pmrhom{a_{n-2}}{a_{n-2}}, \nonpol_\top\pmrhom{\npolar}{a_{n-1}}} : \vec a \to \vec a.
		\]
		\item If $i < n-1$, then $\cohfget_i$ has a right adjoint $\cohfget_i \dashv \cohcodisc_i$ with
		\[
			\cohcodisc_i = \reshlist{\nonpol_\eqtydeg\pmrhom{\npolar}{\npolar}}{\pospol_0\pmrhom{b_0}{a_0}, \ldots, \pospol_{i-1}\pmrhom{b_{i-1}}{a_{i-1}}, \mixpol_{i}\pmrhom{b_i}{a_i}, \pospol_{i}\pmrhom{b_i}{a_{i+1}}, \ldots, \pospol_{n-2}\pmrhom{b_{n-2}}{a_{n-1}}} : \vec b \to \vec a.
		\]
		We have $\cohfget_i \cohcodisc_i = \id_{\vec b}$. We get an idempotent monad
		\[
			\sharp_i = \cohcodisc_i \cohfget_i = \reshlist{\nonpol_\eqtydeg\pmrhom{\npolar}{\npolar}}{\pospol_0\pmrhom{a_0}{a_0}, \ldots, \pospol_{i-1}\pmrhom{a_{i-1}}{a_{i-1}}, \mixpol_{i+1}\pmrhom{a_{i+1}}{a_i}, \pospol_{i+1}\pmrhom{a_{i+1}}{a_{i+1}}, \ldots, \pospol_{n-1}\pmrhom{b_{n-1}}{a_{n-1}}} : \vec a \to \vec a.
		\]
		\begin{itemize}
			...
			\begin{align*}
				\cohfget_i &= \reshlist{\nonpol_\eqtydeg\pmrhom{\npolar}{\npolar}}{\pospol_0\pmrhom{a_0}{b_0}, \ldots, \pospol_{i-1}\pmrhom{a_{i-1}}{b_{i-1}}, \nonpol_{i+1}\pmrhom{\npolar}{\npolar}, \pospol_{i+2}\pmrhom{a_{i+2}}{b_{i+1}}, \ldots, \pospol_{n-1}\pmrhom{a_{n-1}}{b_{n-2}}} &: \vec a \to \vec b, \\
				\dashv
				\cohcodisc_i &= \reshlist{\nonpol_\eqtydeg\pmrhom{\npolar}{\npolar}}{\pospol_0\pmrhom{b_0}{a_0}, \ldots, \pospol_{i-1}\pmrhom{b_{i-1}}{a_{i-1}}, \nonpol_{i}\pmrhom{\npolar}{a_i}, \nonpol_{i}\pmrhom{\npolar}{\npolar}, \pospol_{i+1}\pmrhom{b_{i+1}}{a_{i+2}}, \ldots, \pospol_{n-2}\pmrhom{b_{n-2}}{a_{n-1}}} &: \vec b \to \vec a, \\
				\dashv
				\cohcopi_i &= \reshlist{\nonpol_\eqtydeg\pmrhom{\npolar}{\npolar}}{\pospol_0\pmrhom{a_0}{b_0}, \ldots, \pospol_{i-1}\pmrhom{a_{i-1}}{b_{i-1}}, \mixpol_i\pmrhom{a_i}{\npolar}, \pospol_{i+2}\pmrhom{a_{i+2}}{b_{i+1}}, \ldots, \pospol_{n-1}\pmrhom{a_{n-1}}{b_{n-2}}} &: \vec a \to \vec b.
			\end{align*}
			We have $\cohfget_i \cohcodisc_i = \cohcopi_i \cohcodisc_i = \id_{\vec b}$. We get adjoint idempotent (co)monads
			\begin{align*}
				{}\hspace{-3em}
				\sharp_i = \cohcodisc_i \cohfget_i &= \reshlist{\nonpol_\eqtydeg\pmrhom{\npolar}{\npolar}}{\pospol_0\pmrhom{a_0}{a_0}, \ldots, \pospol_{i-1}\pmrhom{a_{i-1}}{a_{i-1}}, \nonpol_{i+1}\pmrhom{\npolar}{a_i}, \nonpol_{i+1}\pmrhom{\npolar}{\npolar}, \pospol_{i+2}\pmrhom{a_{i+2}}{a_{i+2}}, \ldots, \pospol_{n-1}\pmrhom{a_{n-1}}{a_{n-1}}} &: \vec a \to \vec a, \\
				{}\hspace{-3em}
				\dashv
				\coshp_i = \cohcodisc_i \cohcopi_i &= \reshlist{\nonpol_\eqtydeg\pmrhom{\npolar}{\npolar}}{\pospol_0\pmrhom{a_0}{a_0}, \ldots, \pospol_{i-1}\pmrhom{a_{i-1}}{a_{i-1}}, \mixpol_{i}\pmrhom{a_i}{a_i}, \mixpol_{i}\pmrhom{a_i}{\npolar}, \pospol_{i+2}\pmrhom{a_{i+2}}{a_{i+2}}, \ldots, \pospol_{n-1}\pmrhom{a_{n-1}}{a_{n-1}}} &: \vec a \to \vec a.
			\end{align*}
			\begin{itemize}
...
\begin{example} \label{ex:pjb-paramty} \todo{Make sure to refer correctly.}
	...
	The modality $\cohcodisc_0^{\maskopfont{PJB}}$ will be used to lift structure inhabiting a covariant functor to $\Type^{\vec b}$, to mode $\vec a$, e.g. to build the category of pointed types. It could be called `covariant structurality', generalizing the structural modality in Degrees of Relatedness \cite{reldtt}.\todo{Not 100\% sure right now.}\todoi{Decide what you do with this example.}
\end{example}
%\begin{example}
...
\section{Semantics on Jet Graphs} \label{sec:poresh-jetgph}
\begin{definition} \label{def:cat-enriched-graph}
	A \textbf{category-enriched graph} $\catG$ consists of:
	\begin{itemize}
		\item a set of objects $\Obj(\catG)$,
		\item for every two objects $x, y \in \Obj(\catG)$, a category of edges $\Edg(x, y)$.
	\end{itemize}
	We call $\catG$ \textbf{reflexive} if it is furthermore equipped with:
	\begin{itemize}
		\item for every object $x$, a reflexive edge $\id_x \in \Edg(x, x)$.
	\end{itemize}
	
	\noindent A \textbf{category enriched graph morphism} $F : \catG \to \catH$ consists of:
	\begin{itemize}
		\item an action on objects $F : \Obj(\catG) \to \Obj(\catH)$,
		\item for every two objects $x, y \in \Obj(\catG)$, a functor on edges $F : \Edg_\catG(x, y) \to \Edg_\catH(Fx, Fy)$.
	\end{itemize}
	We call $F$:
	\begin{itemize}
		\item \textbf{reflexive} if $F\,\id_x = \id_{Fx}$,
		\item \textbf{pseudoreflexive} if $F\,\id_x \cong \id_{Fx}$.
	\end{itemize}
\end{definition}

\begin{definition}
	\label{def:poresh-jetgph}
	\begin{enumerate}
		\item We define a pseudoreflexive category-enriched graph morphism (reflexive under the strict quotienting \cref{ax:bishop}) $\jetgphsym : \poreshcat \to \Cat$ from the 2-category of polarized reshuffles $\Cat$:
		\begin{itemize}
			\item Sending an object $\vec a$ to $\jetgph{\vec a}$.
			\item Sending a morphism $\mu : \vec a \to \vec b$ to $\jetgph{\mu} : \jetgph{\vec a} \to \jetgph{\vec b}$ where $\jetgph{\mu}$ (more briefly denoted as $\mu$) sends a jet graph $X$ to the jet graph $\mu X$ whose
			\begin{itemize}
				\item carrier is the carrier of $X$, quotiented by the transitive closure of $\eqtysym \oriof \mu$ on $X$, denoted $X/\mu$ for short, with $[\loch]_\mu : X \to X/\mu$,
				%quotiented out by the transitive closure of $\eqtysym \cdot \mu$ in case $\eqtysym \cdot \mu \neq \eqtysym$,
				\item $i$-jet relation is generated by the implication
				\[
					x \mathrel{(\jetsym_i \oriof \mu)}^X y \qquad \Rightarrow \qquad [x]_\mu \jet_i^{\mu X} [y]_\mu,
				\]
				i.e. $\hat x \jet_i^{\mu X} \hat y$ holds if and only if $\hat x$ and $\hat y$ have representants $x$ and $y$ related by $\jetsym_i \oriof \mu$ in $X$.
			...
			\end{itemize}
			$\jetgph{\mu}$ is easily seen to have a functorial action on jet graph morphisms which preserves identity and composition.
			\item Sending a unique proof of inequality $\mu \leq \nu : \vec a \to \vec b$ to an obvious natural transformation $\ket{\mu \leq \nu} : \mu \to \nu : \jetgph{\vec a} \to \jetgph{\vec b}$ whose underlying function sends $[x]_\mu$ to $[x]_\nu$, which is necessarily a larger equivalence class.
		\end{itemize}
		
		\item If we consider only modalities $\mu$ satisfying \cref{eq:poresh-withl-crit1} (i.e.\ those for which $\eqtydeg \cdot \mu = \nonpol_\eqtydeg$) and denote the restricted 2-poset (with the same order relation) as $\poreshcat \noquot$, then we directly get a strict 2-functor $\jetgphsym \noquot : \poreshcat \noquot \to \Cat$ by the same construction as above, but without the necessity to quotient. (Under the strict quotienting \cref{ax:bishop}, it is equal to a restriction of the graph morphism above.)
	\end{enumerate}
\end{definition}
\begin{proof}[Proof of strict 2-functoriality of $\jetgphsym \noquot$]
	The functorial action of $\jetgphsym\noquot(\mu)$ on $X$ only affects the jet relations of $X$.
	Let $\vec a \xrightarrow{\mu} \vec b \xrightarrow{\nu} \vec c$ and $X \in \jetgph{\vec a}$.
	We have
	\[
		\begin{array}{ccc c ccc c ccc}
			x \mathrel{(\jetsym_i)}\pmr{\vec a} y &\in& \id_{\vec a} X
			&\Leftrightarrow&&&&&
			x \mathrel{(\jetsym_i \oriof \id_{\vec a})}\pmr{\vec a} y &\in& X,
			\\
			x \mathrel{(\jetsym_k)}\pmr{\vec c} y &\in& \nu(\mu X)
			&\Leftrightarrow&
			x \mathrel{(\jetsym_k \oriof \nu)}\pmr{\vec b} y &\in& \mu X
			&\Leftrightarrow&
			x \mathrel{((\jetsym_k \oriof \nu) \oriof \mu)}\pmr{\vec a} y &\in& X,
			\\
			x \mathrel{(\jetsym_k)}\pmr{\vec c} y &\in& (\nu \circ \mu) X
			&\Leftrightarrow&&&&&
			x \mathrel{(\jetsym_k \oriof (\nu \circ \mu))}\pmr{\vec a} y &\in& X.
		\end{array}
	\]
	so by functoriality of $\oridegeqtop$, we see that identity and composition are respected on the nose.
	
	The action of $\jetgphsym \noquot : \poreshcat \noquot \to \Cat$ on inequalities (2-cells) always produces the identity function on objects, and therefore respects all operations on 2-cells.
\end{proof}
%\begin{corollary} \label{thm:poresh-jetgphoid:bind}
%	Let $\mu : \vec a \to \vec b$ and $X \in \jetgphoid{\vec a}$.
...
%	
%	If $\mu$ has a left adjoint and $X \in \jetgph{\vec a}$, then for an arbitrary $Q_j \in \oripsh(\vec b)$, the relation $Q_j$ on $\mu X$ is given by  $Q_j \oriof \mu$. \qed
%\end{corollary}

\begin{corollary} \label{thm:poresh-jetgph-with-adjoint}
	We have:
	\begin{enumerate}
		\item A strict 2-functor $\jetgphsym\withr : \poreshcat\withr \to \Cat$ that restricts $\jetgphsym\noquot$ and therefore essentially restricts $\jetgphsym$,
		\item A pseudofunctor $\jetgphsym\withl : \poreshcat\withl \to \Cat$ that restricts $\jetgphsym$.
	\end{enumerate}
\end{corollary}
\begin{proof}
	\begin{enumerate}
		\item Obviously, we can restrict $\jetgphsym\noquot : \poreshcat\noquot \to \Cat$ along the 2-poset inclusion $\poreshcat\withr \subseteq \poreshcat\noquot$, producing another strict 2-functor.
		\item Obviously, we can restrict $\jetgphsym : \poreshcat \to \Cat$ along $\poreshcat\withl \subseteq \poreshcat$.
		Now if $\lambda \dashv \rho$ in $\poreshcat$, then $\jetgphsym\withr(\lambda) \cong \jetgphsym(\lambda) \dashv \jetgphsym(\rho) = \jetgphsym\withl(\rho)$.
		Then pseudofunctoriality of $\jetgphsym\withl$ follows from strict 2-functoriality of $\jetgphsym\withr$ and uniqueness of the adjoint. \qedhere
	\end{enumerate}
\end{proof}

As \cref{def:cat-enriched-graph} already foreboded, we have to make an abhorrent constatation for the most general operation $\jetgphsym : \poreshcat \to \Cat$, which however becomes irrelevant when we restrict to jet topes and jet cubes:
\begin{proposition}
	$\jetgphsym : \poreshcat \to \Cat$ does not preserve composition.
\end{proposition}
\begin{proof}
	Consider the following two mappings of the same jet graph $X \in \jetgph{[\npolar, \ypolar]}$:
	\begin{align*}
		\xymatrix{
			& {[\npolar, \ypolar]} &
			&&& %-----------
			...
			\\ % ==================================
			&&& {}
				\ar@{|->}[r]^{\reshlist{\nonpol_\eqtydeg\pmrnn}{\nonpol_0\pmrnn, \mixpol_1\pmryn}}
			& {}
			&&&& {}
				\ar@{|->}[r]^{\reshlist{\nonpol_0\pmrnn}{\nonpol_1\pmrnn}}
			& {}
	...
	\begin{align*}
		\xymatrix{
			...
			\\ % ==================================
			&&& {}
				\ar@{|->}[r]^{\reshlist{\nonpol_0\pmrnn}{\pospol_1\pmryy}}
			& {}
			&& {}
				\ar@{|->}[r]^{\reshlist{\nonpol_\eqtydeg\pmrnn}{\mixpol_0\pmryn}}
			& {}
			\\ % ==================================
			c
				\ar@/^/@{-}[rr]_(.7){0}
	...
	So we see that
	\[
		\reshlist{\nonpol_0\pmrnn}{\nonpol_1\pmrnn}\,\paren{\reshlist{\nonpol_\eqtydeg\pmrnn}{\nonpol_0\pmrnn, \mixpol_1\pmryn}\,X} \neq \reshlist{\nonpol_\eqtydeg\pmrnn}{\mixpol_0\pmryn}\,\paren{\reshlist{\nonpol_0\pmrnn}{\pospol_1\pmryy}\,X}
	\]
	despite the fact that
	\[
		\reshlist{\nonpol_0\pmrnn}{\nonpol_1\pmrnn} \circ \reshlist{\nonpol_\eqtydeg\pmrnn}{\nonpol_0\pmrnn, \mixpol_1\pmryn}
		= \reshlist{\nonpol_0\pmrnn}{\mixpol_1\pmryn}
		= \reshlist{\nonpol_\eqtydeg\pmrnn}{\mixpol_0\pmryn} \circ \reshlist{\nonpol_0\pmrnn}{\pospol_1\pmryy} : [\npolar, \ypolar] \to [\npolar].
	\]
	In fact, if we directly apply the composite $\reshlist{\nonpol_0\pmrnn}{\mixpol_1\pmryn}$ to $X$, given the precise definition above, we obtain the jet graph where $[a] \not\edge_0 [c]$.
\end{proof}
\begin{example} \label{ex:poresh-op-jetgph}
	The functor $\Op_i$ from \cref{def:op} arises from the reshuffle in \cref{ex:poresh-op} as $\jetgphl{\Op_i}$.
\end{example}
\begin{example} \label{ex:poresh-sym-jetgph}
	...
	\[
		\jetgphl{\Fsymresh_i} \dashv \jetgphl{\Usymresh_i} \dashv \jetgphl{\Csymresh_i},
	\]
	\[
		\jetgphl{\UFsymresh_i} \dashv \jetgphl{\UCsymresh_i}.
	\]
\end{example}
\begin{proposition} \label{thm:poresh-jetgph-nonexact}
	A functor of the form $\jetgph{\mu} : \jetgph{\vec a} \to \jetgph{\vec b}$ for some $\mu : \vec a \to \vec b$ does not necessarily preserve finite limits, nor finite colimits.	
\end{proposition}
\begin{proof}
	The modality $\reshlist{\nonpol_\eqtydeg\pmrnn}{\mixpol_0\pmryn} : [\ypolar] \to [\npolar]$ disrespects the following pushout diagram:
	\[
		\xymatrix{
			\accol{0 \quad 1}
				\ar[rr]
				\ar[dd]
				\ar@{}[rrdd]|(.8){\mbox{$\ulcorner$}}
			&& \rival{\jetsym_0}
				\ar[dd]
			&&& %----------------------
			\accol{0 \quad 1}
	...
	\]
	The modality $\reshlist{\nonpol_\eqtydeg\pmrnn}{\isopol_0\pmryn} : [\ypolar] \to [\npolar]$ disrespects the following pullback diagram:
	\[
		\xymatrix{
			\accol{0 \quad 1}
				\ar[rr]
				\ar[dd]
				\ar@{}[rrdd]|(.2){\mbox{$\lrcorner$}}
			&& \rival{\jetsym_0}
				\ar[dd]
			&&& %----------------------
			\accol{0 \quad 1}
...
\subsection{Intervals and Prisms}
\begin{definition} \label{def:interval-eqty} \label{def:interval-top}
	We define $\rival{\edgesym_\eqtydeg}$ as the singleton jet graph $\accol{0, 1}/\edgesym_\top$ (a quotient by the total relation)\footnote{The choice of singleton is especially relevant under the strict quotienting \cref{ax:bishop}.}, and $\rival{\topsym}$ as the discrete jet graph $\accol{0 \quad 1}$.
\end{definition}
\begin{proposition} \label{thm:poresh-jetgph-intervals}
	Let $\mu : \vec a \to \vec b$ be a polarized reshuffle with right adjoint $\mu \dashv \rho$, and $i \in \Deg(\vec a)$. Then we have%
	\footnote{By virtue of \cref{thm:poresh-adjoint}, all right hand sides are defined since $i \cdot \rho \neq \isopol_j$ whence $P_i \oriof \rho \neq \infrajetsym_j$ for $b_j = \ypolar$.}
	\begin{equation}
		\begin{array}{c c c}
			\jetgph{\mu} \rival{\jetsym_i\pmry} \cong \rival{\jetsym_i \oriof \rho},
			& \jetgph{\mu} \rival{\tejsym_i\pmry} \cong \rival{\tejsym_i \oriof \rho},
			& \jetgph{\mu} \rival{\equijetsym_i\pmry} \cong \rival{\equijetsym_i \oriof \rho},
			\\[1em]
			& \jetgph{\mu} \rival{\edgesym_i\pmrn} \cong \rival{\edgesym_i \oriof \rho}.
		\end{array}
		\label{eq:poresh-jetgph-intervals}
	\end{equation}
	Under the strict quotienting \cref{ax:bishop}, these isomorphisms are equalities.
\end{proposition}
\begin{proof}
	Let $P \in \accol{\jetsym\pmr{a_i}, \tejsym\pmr{a_i}, \equijetsym\pmr{a_i}}$.
	We prove that $\mu \rival{P_i} \cong \rival{P_i \oriof \rho}$.
	
	\begin{enumerate}
		\item First assume that $\edgesym_\eqtydeg \oriof \mu < P_i$.
		Then $[0]_\mu \neq [1]_\mu$.
		In this case, for any $Q_j \in \orideg(\vec b)$, we have
		\[
			[0]_\mu \mathrel{(Q_j)}^{\mu \rival{P_i}} [1]_\mu 
			\quad\Leftrightarrow\quad
			0 \mathrel{(Q_j \oriof \mu)}^{\rival{P_i}} 1
			\quad\Leftrightarrow\quad
			P_i \leq Q_j \oriof \mu
			\quad\Leftrightarrow\quad
			P_i \oriof \rho \leq Q_j.
		\]
		The latter step works because $\loch \oriof \rho \dashv \loch \oriof \mu$.
		
		As a sanity check, we also have
		\[
			\edgesym_\eqtydeg \oriof \mu < P_i
			\quad\Leftrightarrow\quad
			P_i \not\leq \edgesym_\eqtydeg \oriof \mu
			\quad\Leftrightarrow\quad
			P_i \oriof \rho \not\leq \edgesym_\eqtydeg
			\quad\Leftrightarrow\quad
			P_i \oriof \rho > \edgesym_\eqtydeg,
		\]
		so neither $\mu \rival{P_i}$ nor $\rival{P_i \oriof \rho}$ is a singleton.
		Then we can conclude that the jet relations in $\mu \rival{P_i}$ are generated by the fact that $[0]_\mu \mathrel{(P_i \oriof \rho)} [1]_\mu$, i.e. $\mu \rival{P_i} \cong \rival{P_i \oriof \rho}$.
		In the special case where $P_i \oriof \rho = \topsym$, we find that $[0]_\mu$ and $[1]_\mu$ are unrelated, as in $\rival{\topsym}$.
		
		\item Next, assume that $\edgesym_\eqtydeg \oriof \mu \geq P_i$, so that $\mu \rival{P_i}$ is a singleton.
		By the reasoning above, this is equivalent to $P_i \oriof \rho \leq \edgesym_\eqtydeg$, hence $P_i \oriof \rho = \edgesym_\eqtydeg$, so that $ \rival{P_i \oriof \rho}$ is also a singleton.
		Then these singletons are isomorphic. \qedhere
	\end{enumerate}
\end{proof}
\begin{proposition} \label{thm:poresh-jetgph-prism-commut}
	Let $\mu : \vec a \to \vec b$ be a polarized reshuffle with right adjoint $\mu \dashv \rho$, and $i \in \Deg(\vec a)$.
	Then the action of $\jetgph{\mu}$ on prisms can be looked up, up to isomorphism\footnote{On the nose under the strict quotienting \cref{ax:bishop}.}, in the following table, where $j = \getdeg(i \cdot \rho)$:%
	\footnote{Note that if $i \cdot \rho = \isopol_j$, then by \cref{eq:poresh-withl-crit2}, $b_j = \npolar$.}
	\[
		\begin{array}{c | l || c | c || c}
			& a_i\,{\rightarrow} & \multicolumn{2}{c ||}{\ypolar} & \npolar \\ \hline
			{b_j}\,{\downarrow}
			& {i \cdot \rho}\,{\downarrow}
			& \mu(X \multip \rival{\jetsym_i\pmry})
			& \mu(X \multip \rival{\tejsym_i\pmry})
			& \mu(X \multip \rival{\edgesym_i\pmrn})
			\\ \hline \hline
			\npolar
			& \nonpol_\eqtydeg\pmrhom{\npolar}{a_i}
			& (\mu X) \times \rival{\eqtysym\pmrn}
			& (\mu X) \times \rival{\eqtysym\pmrn}
			& (\mu X) \times \rival{\eqtysym\pmrn}
			\\ \hline
			\multirow{3}{*}{$\ypolar$}
			& \mixpol_j\pmrhom{\ypolar}{a_i}
			& (\mu X) * \rival{\equijetsym_j\pmry}
			& (\mu X) * \rival{\equijetsym_j\pmry}
			& (\mu X) * \rival{\equijetsym_j\pmry}
			\\
			& \pospol_j\pmrhom{\ypolar}{\ypolar}
			& (\mu X) \multip \rival{\jetsym_j\pmry}
			& (\mu X) \multip \rival{\tejsym_j\pmry}
			&
			\\
			& \negpol_j\pmrhom{\ypolar}{\ypolar}
			& (\Op_j\mu X) \multip \rival{\tejsym_j\pmry}
			& (\Op_j\mu X) \multip \rival{\jetsym_j\pmry}
			&
			\\ \hline
			\npolar
			& \nonpol_j\pmrhom{\npolar}{a_i}
			& (\mu X) \multip \rival{\edgesym_j\pmrn}
			& (\mu X) \multip \rival{\edgesym_j\pmrn}
			& (\mu X) \multip \rival{\edgesym_j\pmrn}
			\\ \hline
			\npolar
			& \nonpol_\top\pmrhom{\npolar}{a_i}
			& (\mu X) \times \rival{\topsym\pmrn}
			& (\mu X) \times \rival{\topsym\pmrn}
			& (\mu X) \times \rival{\topsym\pmrn}
		\end{array}
	\]
\end{proposition}
Note that a cartesian product with $\rival{\eqtysym}$ or $\rival{\topsym}$ is the same as a separated product, since neither jet graph has any non-reflexive jets.
Note also that, while the nature of the `product' varies, the second component for $\mu(X \multip \rival{P_i})$ is always $\rival{P_i\pmr{a_i} \oriof \rho\pmrhom{\vec b}{\vec a}}$.
\begin{proof}
	Let $P \in \accol{\jetsym\pmr{a_i}, \tejsym\pmr{a_i}}$, and $X \in \jetgph{\vec a}$.
	We prove that $\mu(X \multip \rival{P_i}) \in \jetgph{\vec b}$ is isomorphic to the prescribed jet graph.
	Write $\eqtysym \oriof \mu =: R_k \in \oridegeq(\vec a)$.
	\begin{enumerate}
		\item First, assume that $R_k = \edgesym_\eqtydeg \oriof \mu \geq P_i$, so that $r \mathrel{P_i} s$ implies $[r]_\mu = [s]_\mu$ and in particular, for any $x \in X$, we have $[(x, 0)]_\mu = [(x, 1)]_\mu$ in $\mu(X \multip \rival{P_i})$.
		
		By the same reasoning as in the proof of \cref{thm:poresh-jetgph-intervals}, the assumption is equivalent to $P_i \oriof \rho \leq \edgesym_\eqtydeg$, hence $i \cdot \rho = \nonpol_\eqtydeg$.
		It thus remains to show that $\mu(X \multip \rival{P_i})$ is isomorphic to $(\mu X) \times \rival{\eqtysym\pmrn}$ or, equivalently, simply to $\mu X$.
		...
		\begin{itemize}
			\item[\framebox{$\to$}]
			Take $(x, u)$ and $(y, v)$ in $X \multip \rival{P_i}$ such that $[(x, u)]_\mu = [(y, v)]_\mu$.
			Then $(x, u) \mathrel{(R_k)^*} (y, v)$ in $X \multip \rival{P_i}$, i.e.\ there is a chain
			\[
				(x, u) = (x_0, u_0) \mathrel{R_k} (x_1, u_1) \mathrel{R_k} \ldots \mathrel{R_k} (x_{n-1}, u_{n-1}) \mathrel{R_k} (x_n, u_n) = (y, v).
			\]
			Since $R_k$ is a symmetric relation, from the definition of the prism functors, it is easy to deduce
			\[
				(x, w) = (x_0, w) \mathrel{R_k} (x_1, w) \mathrel{R_k} \ldots \mathrel{R_k} (x_{n-1}, w) \mathrel{R_k} (x_n, w) = (y, w).
			\]
			for an arbitrary fixed $w \in \accol{0, 1}$.
			In particular, we can let $w$ be $1$ if $P = \jetsym$ and $0$ if $P = \tejsym$.
			Then the above is equivalent to
			\[
				x = x_0 \mathrel{R_k} x_1 \mathrel{R_k} \ldots \mathrel{R_k} x_{n-1} \mathrel{R_k} x_n = y.
			\]
			in $X$, so that $x \mathrel{(R_k)^*} y$ and $[x]_\mu = [y]_\mu$.
			
			\item[\framebox{$\leftarrow$}]
			Conversely, take $x, y \in X$ such that $[x]_\mu = [y]_\mu$.
			Then by the above reasoning in reverse, we deduce that $[(x, w)]_\mu = [(y, w)]_\mu$.
		\end{itemize}
		We now show that this isomorphism respects the $\ell$-jet relations in both directions, for $\ell \in \Deg(\vec b)$.
		Let $\jetsym_\ell \oriof \mu =: R'_{k'}$.
		Since $R'_{k'} \gg R_k \geq P_i$, we know that either $R'$ is symmetric, or $k' > i$.
		Either way, we have $x \mathrel{R'_{k'}} y \Leftrightarrow (x, 0) \mathrel{R'_{k'}} (y, 0) \Leftrightarrow (x, 0) \mathrel{R'_{k'}} (y, 0)$ in $X \multip \rival{P_i}$.
		Moreover, recall that since $\mu$ has a left adjoint, we have $R'_{k'} < \topsym$.
		\begin{itemize}
			\item[\framebox{$\to$}]
			Take a non-reflexive jet $[(x, u)]_\mu \jet_\ell [(y, v)]_\mu$ in $\mu(X \multip \rival{P_i})$.
			Then there exist representants, \WLOG{} $(x, u)$ and $(y, v)$, such that $(x, u) \mathrel{R'_{k'}} (y, v)$.
			Then either $x = y$ or $u = v$. Since the original jet was non-reflexive, $x \neq y$, so $u = v$.
			Then this is equivalent to $x \mathrel{R'_{k'}} y$, implying $[x]_\mu \jet_\ell [y]_\mu$.
			
			\item[\framebox{$\leftarrow$}]
			Take a non-reflexive jet $[x]_\mu \jet_\ell [y]_\mu$ in $\mu X$.
			Then there exist representants, \WLOG{} $x$ and $y$, such that $x \mathrel{R'_{k'}} y$.
			This is equivalent to $(x, 0) \mathrel{R'_{k'}} (y, 0)$, implying $[(x, 0)]_\mu \jet_\ell [(y, 0)]_\mu$.
		\end{itemize}
		
		\item Next assume that $R_k := \edgesym_\eqtydeg \oriof \mu < P_i$.
		Then we have $[(x, 0)]_\mu \neq [(y, 1)]_\mu$ in $\mu(X \multip \rival{P_i})$ for all $x, y \in X$, since $R_k$ can never relate pairs with a different second component.
		
		Write $Y \in \jetgph{\vec b}$ for the jet graph prescribed by the table.
		We show that there is an isomorphism
		\[
			\mu(X \multip \rival{P_i}) \cong Y : [(x, u)]_\mu \mapsfrom \mapsto ([x]_\mu, u).
		\]
		...
		\begin{itemize}
			\item[\framebox{$\to$}]
			Let $[(x, u)]_\mu = [(y, v)]_\mu$ in $\mu(X \multip \rival{P_i})$.
			We already established that in that case, $u = v$.
			We prove that $[x]_\mu = [y]_\mu$ in $\mu X$.
			We have $(x, u) \mathrel{(R_k)^*} (y, u)$ in $X \multip \rival{P_i}$.
			The entire chain of $R_k$-steps must consist of pairs of the form $(x_i, u)$ for the same $u$.
			Since $R_k$ is a symmetric relation, it is then straightforward to conclude that we also have $x \mathrel{(R_k)^*} y$, which implies $[x]_\mu = [y]_\mu$.
			
			\item[\framebox{$\leftarrow$}]
			Conversely, let $[x]_\mu = [y]_\mu$ in $\mu X$, and $u \in \accol{0, 1}$.
			By the above reasoning in reverse, show that $[(x, u)]_\mu = [(y, u)]_\mu$.	
		\end{itemize}
		We now show that this isomorphism respects the $\ell$-jet relations in both directions, for $\ell \in \Deg(\vec b)$.
		Let $\jetsym_\ell \oriof \mu =: R'_{k'}$.
		\begin{itemize}
			\item[\framebox{$\to$}]
			Take a non-reflexive jet $[(x, u)]_\mu \jet_\ell\pmr{b_\ell} [(y, v)]_\mu$ in $\mu(X \multip \rival{P_i})$.
			Then there exist representants, \WLOG{} $(x, u)$ and $(y, v)$, such that $(x, u) \mathrel{R'_{k'}} (y, v)$.
			Then either $x = y$ or $u = v$.
			\begin{itemize}
				\item If $x = y$ and $u \neq v$, then we have $(x, u) \mathrel{R'_{k'}} (x, v)$.
				Let $P' = P$ if $(u, v) = (0, 1)$ and $P' = P \oriof \negpol$ if $(u, v) = (1, 0)$.
				We have that $P'_i \leq R'_{k'} = \jetsym_\ell \oriof \mu$, or equivalently (since $\loch \oriof \rho \dashv \loch \oriof \mu$) that $P'_i \oriof \rho \leq \jetsym_\ell$.
				This implies that $u \jet_\ell v$ in $\rival{P_i \oriof \rho}$, so that $([x]_\mu, u) \jet_\ell ([x]_\mu, v)$ in $Y$.
				
				\item If $x \neq y$ and $u = v$, then we have $(x, u) \mathrel{R'_{k'}} (y, u)$.
				Let $w = 1$ if $P = \jetsym$ and $w = 0$ if $P = \tejsym$.
				We consider four landmark propositions, between which we will navigate:
				\begin{itemize}
					\item[\textbf{(A)}] $(x, u) \mathrel{R'_{k'}} (y, u)$ in $X \multip \rival{P_i}$.
					\item[\textbf{(B)}] $x \mathrel{R'_{k'}} y$ in $X$, implying $[x]_\mu \jet_\ell [y]_\mu$ in $\mu X$. Additionally, either $k' \neq i$ or $u = w$.
					\item[\textbf{(C)}] $y \mathrel{R'_{k'}} x$ in $X$, implying $[y]_\mu \jet_\ell [x]_\mu$ in $\mu X$. Additionally, $k' = i$ and $u \neq w$.
					\item[\textbf{(D)}] $([x]_\mu, u) \jet_\ell ([y]_\mu, u)$ in $Y$.
				\end{itemize}
				We are currently at (A).
				\begin{description}
					\item[From (A) to either (B) or (C)]
					We prove that we can go from (A) to either (B) or (C).
					We make a case distinction on $k'$:
					\begin{itemize}
						\item[\framebox{$k' \neq i$}] In this case, (A) directly implies (B), as the twisted prism functor $\loch \multip \rival{P_i}$ does not interfere with $R'_{k'}$.
						\item[\framebox{$k' = i$}] We make a case distinction on $u$:
						\begin{itemize}
							\item[\framebox{$u = w$}] In this case, (A) directly implies (B), as the twisted prism functor does not flip $i$-jets at its target side.
							\item[\framebox{$u \neq w$}] In this case, (A) directly implies (C), as the twisted prism functor flips $i$-jets at its source side.
						\end{itemize}
					\end{itemize}
					
					\item[From (B) to (D)]
					We now show that we can go from (B) to (D).
					We make a case distinction on $\ell$:
					\begin{itemize}
						\item[\framebox{$\ell \neq j$}] Then (B) directly implies (D), as the \emph{potential} usage of a twisted prism functor at level $j$ does not interfere at level $\ell$.
						\item[\framebox{$\ell = j$}] We make a case distinction on $b_j$:
						\begin{itemize}
							\item[\framebox{$b_j = \npolar$}] Then (B) directly implies (D), as no twisted prism functor is involved in constructing $Y$.
							\item[\framebox{$b_j = \ypolar$}] We make a case distinction on $(i \cdot \rho)\pmrhom{\vec b}{a_i}$:
							\begin{itemize}
								\item[\framebox{$i \cdot \rho = \mixpol_j\pmrhom{\ypolar}{a_i}$}] Then (B) directly implies (D), as no twisted prism functor is involved in constructing $Y$.
								\item[\framebox{$i \cdot \rho = \pospol_j\pmryy$}]
								In this case, by \cref{thm:poresh-adjoint-directed}, we also have $R'_{k'} = j \cdot \mu = \pospol_i\pmryy$ and in particular $k' = i$, so that we can deduce from (B) that $u = w$.
								Then we can conclude (D), since we are at the target side of the twisted prism functor, where it does not flip $j$-jets.
								\item[\framebox{$i \cdot \rho = \negpol_j\pmryy$}]
								In this case, by \cref{thm:poresh-adjoint-directed}, we also have $R'_{k'} = j \cdot \mu = \negpol_i\pmryy$ and in particular $k' = i$, so that we can deduce from (B) that $u = w$.
								Then we can conclude (D), since we are at the \emph{source} side of the twisted prism functor, where it \emph{does} flip $j$-jets, but $\Op_j$ flips them again.
							\end{itemize}
						\end{itemize}
					\end{itemize}
					\item[From (C) to (D)] We now show that we can go from (C) to (D).
					We make a case distinction on $a_i$:
					\begin{itemize}
						\item[\framebox{$a_i = \npolar$}] Then $R'_{k'} = R'_i$ is a symmetric relation, so (C) implies also that $x \mathrel{R_i} y$ in $X$ such that $[x]_\mu \jet_\ell [y]_\mu$ and indeed $[x]_\mu \equijet_\ell [y]_\mu$.
						Then we will have $([x]_\mu, u) \equijet_\ell ([y]_\mu, u)$ and in particular (D).
						
						\item[\framebox{$a_i = \ypolar$}] We make a case distinction on $\ell \cdot \mu$, which necessarily has degree $i$:
						\begin{itemize}
							\item[\framebox{$\ell \cdot \mu = \isopol_i\pmrhom{\ypolar}{b_\ell}$}]
							In this case $R_i = \infrajetsym_i$, a symmetric relation, so that we can conclude (D) by similar reasoning as in the previous point.
							
							\item[\framebox{$\ell \cdot \mu = \pospol_i\pmryy$}]
							Then by \cref{thm:poresh-adjoint-directed}, we also have $i \cdot \rho = \pospol_\ell$, implying that $\ell = j$.
							From (C), we get that $[y]_\mu \jet_j [x]_\mu$ in $\mu X$, and that $u \neq w$, i.e.\ that we are at the source side of the twisted prism functor.
							Thus, we conclude that $([x]_\mu, u) \jet_j ([y]_\mu, u)$ in $Y$.
							
							\item[\framebox{$\ell \cdot \mu = \negpol_i\pmryy$}]
							Then by \cref{thm:poresh-adjoint-directed}, we also have $i \cdot \rho = \negpol_\ell$, implying that $\ell = j$.
							From (C), we get that $[y]_\mu \jet_j [x]_\mu$ in $\mu X$ and hence $[x]_\mu \jet_j [y]_\mu$ in $\Op_j \mu X$, and that $u \neq w$, i.e.\ that we are at the \emph{target} side of the flipped twisted prism functor.
							Thus, we conclude that $([x]_\mu, u) \jet_j ([y]_\mu, u)$ in $Y$.
						\end{itemize}
					\end{itemize}
				\end{description}
			...
			\item[\framebox{$\leftarrow$}]
			Take a non-reflexive jet $([x]_\mu, u) \jet_\ell\pmr{b_\ell} ([y]_\mu, v)$ in $Y$.
			Then either $[x]_\mu = [y]_\mu$ or $u = v$.
			\begin{itemize}
				\item If $[x]_\mu = [y]_\mu$ and $u \neq v$, then we have $u \jet_\ell v$ in $\rival{P_i \oriof \rho}$.
				Let $P' = P$ if $(u, v) = (0, 1)$ and $P' = P \oriof \negpol$ if $(u, v) = (1, 0)$.
				We have that $P'_i \oriof \rho \leq \jetsym_\ell$ or equivalently, $P'_i \leq \jetsym_\ell \oriof \mu = R'_{k'}$.
				Then we have $(x, u) \mathrel{R'_{k'}} (x, v)$ in $X \multip \rival{P_i}$ and thus $[(x, u)]_\mu \jet_\ell [(x, v)]_\mu$ in $\mu(X \multip \rival{P_i})$.
				\item If $[x]_\mu \neq [y]_\mu$ and $u = v$, then we have $([x]_\mu, u) \jet_\ell\pmr{b_\ell} ([y]_\mu, u)$.
				Let $w = 1$ if $P = \jetsym$ and $w = 0$ if $P = \tejsym$.
				We consider four landmark propositions, between which we will navigate:
				\begin{itemize}
					\item[\textbf{(E)}] $([x]_\mu, u) \jet_\ell ([y]_\mu, u)$ in $Y$.
					\item[\textbf{(F)}] $[x]_\mu \jet_\ell [y]_\mu$ in $\mu X$, implying that there must be representants, \WLOG{} $x$ and $y$, such that $x \mathrel{R'_{k'}} y$ in $X$. Additionally, one of the following must be true:
					\begin{itemize}
						\item $j \neq \ell$,
						\item $b_j = \npolar$,
						\item $i \cdot \rho = \mixpol_j$,
						\item $k' = i$ and $u = w$.
					\end{itemize}
					\item[\textbf{(G)}] $[y]_\mu \jet_\ell [x]_\mu$ in $\mu X$, implying that there must be representants, \WLOG{} $y$ and $x$, such that $y \mathrel{R'_{k'}} x$ in $X$. Additionally,
%					$i \cdot \rho \in \accol{\pospol_j\pmryy, \negpol_j\pmryy}$,
					$k' = i$ and $u \neq w$.
					\item[\textbf{(H)}] $(x, u) \mathrel{R'_{k'}} (y, u)$ in $X \multip \rival{P_i}$, implying that $[(x, u)]_\mu \jet_\ell [(y, u)]_\mu$ in $\mu(X \multip \rival{P_i})$.
				\end{itemize}
				We are currently at (E).
				\begin{description}
					\item[From (E) to either (F) or (G)]
					We show that we can go from (E) to either (F) or (G).
					We make a case distinction on $\ell$:
					\begin{itemize}
						\item[\framebox{$\ell \neq j$}] In this case, (E) directly implies (F), as the \emph{potential} usage of a twisted prism functor at level $j$ does not interfere at level $\ell$.
						\item[\framebox{$\ell = j$}] We make a case distinction on $b_j$:
						\begin{itemize}
							\item[\framebox{$b_j = \npolar$}] Then (E) directly implies (F), as no twisted prism functor is involved in constructing $Y$.
							\item[\framebox{$b_j = \ypolar$}] We make a case distinction on $(i \cdot \rho)\pmrhom{\vec b}{a_i}$:
							\begin{itemize}
								\item[\framebox{$i \cdot \rho = \mixpol_j\pmrhom{\ypolar}{a_i}$}] Then (E) directly implies (F), as no twisted prism functor is involved in constructing $Y$.
								\item[\framebox{$i \cdot \rho = \pospol_j\pmryy$}]
								In this case, by \cref{thm:poresh-adjoint-directed}, we also have $R'_{k'} = j \cdot \mu = \pospol_i\pmryy$ and in particular $k' = i$.
								We make a case distinction on $u$:
								\begin{itemize}
									\item[\framebox{$u = w$}]
									In this case, (E) directly implies (F) as the twisted prism functor does not flip $j$-jets at its target side.
									
									\item[\framebox{$u \neq w$}]
									In this case, (E) directly implies (G) as the twisted prism functor \emph{does} flip $j$-jets at its source side.
								\end{itemize}
								
								\item[\framebox{$i \cdot \rho = \negpol_j\pmryy$}]
								In this case, by \cref{thm:poresh-adjoint-directed}, we also have $R'_{k'} = j \cdot \mu = \negpol_i\pmryy$ and in particular $k' = i$.
								We make a case distinction on $u$:
								\begin{itemize}
									\item[\framebox{$u = w$}]
									In this case, we are at the source side of the \emph{flipped} twisted prism functor, and can conclude that $[x]_\mu \tejsym_j [y]_\mu$ in $\Op_j \mu X$, so that (F) holds.
									
									\item[\framebox{$u \neq w$}]
									In this case, we are at the target side of the \emph{flipped} twisted prism functor, and can conclude that $[x]_\mu \jetsym_j [y]_\mu$ in $\Op_j \mu X$, so that (G) holds.
								\end{itemize}
							\end{itemize}
						\end{itemize}
					\end{itemize}
					\item[From (F) to (H)]
					We show that we can go from (F) to (H).
					We make a case distinction on $k'$:
					\begin{itemize}
						\item[\framebox{$k' \neq i$}] Then (F) directly implies (H), as the twisted prism functor $\loch \multip \rival{P_i}$ does not interfere with $R'_{k'}$.
						\item[\framebox{$k' = i$}] We make a case distinction on $a_i$:
						\begin{itemize}
							\item[\framebox{$a_i = \npolar$}] Then the twisted prism functor $\loch \multip \rival{P_i\pmrn} = \loch \multip \rival{\edgesym_i\pmrn}$ does not actually twist, and we can conclude (H).
							\item[\framebox{$a_i = \ypolar$}]  We make a case distinction on $\ell \cdot \mu$, which necessarily has degree $i$:
							\begin{itemize}
								\item[\framebox{$\ell \cdot \mu = \isopol_i\pmrhom{\ypolar}{b_\ell}$}]
								In this case $R_i = \infrajetsym_i$, a symmetric relation, which is unaffected by the twisted prism functor $\loch \multip \rival{P_i}$.
								\item[\framebox{$\ell \cdot \mu = \pospol_i\pmryy$}]
								Then by \cref{thm:poresh-adjoint-directed}, we also have $i \cdot \rho = \pospol_\ell$, implying that $\ell = j$.
								Moreover, this option is only possible when $b_j = \ypolar$.
								We have thus ruled out the first three subpoints of (F), and can conclude that $k' = i$ and $u = w$.
								
								In this case, $R_i = \jetsym_i$, and (F) implies (H) as the twisted prism functor does not flip $i$-jets at its target side.
								...
								In this case, $R_i = \tejsym_i$, and (F) implies (H) as the twisted prism functor does not flip $i$-jets at its target side.
							\end{itemize}
						\end{itemize}
					\end{itemize}
					\item[From (G) to (H)]
					We show that we can go from (G) to (H).
					From (G), we know that $k' = i$ and that we are at the source side of the twisted prism functor, where $i$-jets will be flipped.
					This leads to (H). \qedhere
				\end{description}
			\end{itemize}
		\end{itemize}
	\end{enumerate}
...
\begin{theorem} \label{thm:poresh-jettope}
	...
	Then the functor $\mu : \jetgph{\vec a} \to \jetgph{\vec b}$ sends jet topes to jet topes if and only if $\rho$ has a further right adjoint $\mu\pmrhom{\vec a}{\vec b} \dashv \rho\pmrhom{\vec b}{\vec a} \dashv \tau\pmrhom{\vec a}{\vec b}$.
	
	By consequence, there is a pseudofunctor $\jettopesym : \poreshcat\withrr \to \Cat$ sending
	\begin{itemize}
		\item an object $\vec a$ to $\jettope{\vec a}$,
		\item a morphism $\mu : \vec a \to \vec b$ to the restriction of $\jetgphsym\withr(\mu) : \jetgph{\vec a} \to \jetgph{\vec b}$ to $\jettope{\vec a}$,
		\item a 2-cell $\alpha : \mu \leq \nu : \vec a \to \vec b$ to the restriction of $\jetgphsym\withr(\alpha)$ to $\jettope{\vec a}$.
	\end{itemize}
\end{theorem}
\begin{proof}
	...
	\begin{itemize}
		...
		Then by \cref{thm:poresh-jetgph-intervals}, $\mu \rival{\jetsym_{\predeg(\top)}} = \rival{\edgesym_\top}$, where $\rival{\jetsym_{\predeg(\top)}}$ is a jet tope but $\rival{\edgesym_\top}$ is not, as it is not $(n-1)$-connected (if $n > 0$) or not a subsingleton (if $n = 0$).
		...
		Then by \cref{thm:poresh-jetgph-intervals}, $\mu \rival{\jetsym_i} = \rival{\equijetsym_j}$, where $\rival{\jetsym_i}$ is a jet tope but $\rival{\equijetsym_j}$ is not, as it is not $j$-antisymmetric.
	\end{itemize}
	...
	\begin{enumerate}
		\item Clearly, $\mu X$ is finite and non-empty, as $X$ was finite and non-empty.
		
		\item To see that $\mu X$ is $\predeg(\top)$-connected (i.e.\ $(n-1)$-connected if $n > 0$, or $\eqtydeg$-connected if $n = 0$), take $[x]_\mu, [y]_\mu \in \mu X$.
		We prove that $[x]_\mu \infrajet_{\predeg(\top)}^* [y]_\mu$ in $\mu X$.
		It is sufficient to prove that $x \mathrel{(\isopol_{\predeg(\top)} \oriof \mu)^*} y$ in $X$.
		Since $X$ is $\predeg(\top)$-connected, this is the case as soon as $\infrajetsym_{\predeg(\top)} \leq \infrajetsym_{\predeg(\top)} \oriof \mu$, or equivalently (since $\loch \oriof \rho \dashv \loch \oriof \mu$) if $\infrajetsym_{\predeg(\top)} \oriof \rho \leq \infrajetsym_{\predeg(\top)}$.
		This follows from \cref{eq:poresh-withr-crit1} as $\rho$ has a right adjoint.
		
		\item We show that $\mu X$ is $j$-orientable for every $j \in \Deg(\vec b)$. Let $[x]_\mu \infrajet_j^* [y]_\mu$ in $\mu X$.
		Then by \cref{thm:poresh-jetgph-trcl-from-trcl}, we have $x \mathrel{(\infrajetsym_j \oriof \mu)^*} y$ in $X$ irrespective of the choice of representants.
		If $j \cdot \mu$ is a symmetric relation, then this is equivalent to $x \mathrel{(\jetsym_j \oriof \mu)^*} y$; otherwise $x \mathrel{(\jetsym_j \oriof \mu)^*} y$ follows from $\getdeg(j \cdot \mu)$-orientability.
		Either way, we conclude that $[x]_\mu \jet_j^* [y]_\mu$ in $\mu X$.
		
		\item We show that $\mu X$ is $j$-acyclic for every $j \in \Deg(\vec b)$ by similar reasoning.
		
		\item Assume that $b_j = \ypolar$.
		We show that $\mu X$ is $j$-antisymmetric, i.e. that $[x]_\mu \equijet_j\pmry [y]_\mu$ implies $[x]_\mu \infrajet_{\predeg(j)}\pmr{b_{\predeg(j)}} [y]_\mu$.
		From the premise we deduce that there are representants, \WLOG{} $x$ and $y$, such that $x \mathrel{(\equijetsym_j\pmry \oriof \mu)} y$.
		It is then sufficient to prove $x \mathrel{(\infrajetsym_{\predeg(j)}\pmr{b_{\predeg(j)}} \oriof \mu)} y$
		
		Let $p'_{i'} = \predeg(j) \cdot \mu$ and $p_i = j \cdot \mu$.
		We make a case distinction on how $i'$ compares to $i$:
		\begin{description}
			\item[\framebox{$i' = i$}] Then we have $p' \ll p$, which by \cref{def:precede-polarity} implies that either $p' = \mixpol\pmrhom{a_i}{b_{\predeg(j)}}$ or $p = \isopol\pmrhom{a_i}{\ypolar}$.
			\begin{itemize}
				\item If $a_i = \npolar$, then we have $p = p' = \nonpol\pmrny$.
				Then we have
				\[
					\begin{array}{r c c c c c c}
						(\equijetsym_j)\pmr{\vec b} \oriof \mu\pmrhom{\vec a}{\vec b}
						&=& \equijetsym\pmry \oriof (j \cdot \mu)\pmrhom{\vec a}{\ypolar}
						&=& \equijetsym\pmry \oriof \nonpol_i\pmrny
						&=& \edgesym_i\pmrn,
						\\
						(\infrajetsym_{\predeg(j)})\pmr{\vec b} \oriof \mu\pmrhom{\vec a}{\vec b}
						&=& \infrajetsym\pmr{b_{\predeg(j)}} \oriof (\predeg(j) \cdot \mu)\pmrhom{\vec a}{b_{\predeg(j)}}
						&=& \infrajetsym\pmr{b_{\predeg(j)}} \oriof \nonpol_i\pmrhom{\npolar}{b_{\predeg(j)}}
						&=& \edgesym_i\pmrn,
					\end{array}
				\]
				so that what we know is identical to what remained to be proven.
				
				\item If $a_i = \ypolar$, then by \cref{eq:poresh-withr-crit1} for $\mu$, we know that $p' \neq \mixpol\pmrhom{\ypolar}{b_{\predeg(j)}}$.
				Hence, $p = \isopol\pmryy$.
				Then we have
				\[
					\begin{array}{r c c c c c c}
						(\equijetsym_j)\pmr{\vec b} \oriof \mu\pmrhom{\vec a}{\vec b}
						&=& \equijetsym\pmry \oriof (j \cdot \mu)\pmrhom{\vec a}{\ypolar}
						&=& \equijetsym\pmry \oriof \isopol_i\pmryy
						&=& \infrajetsym_i\pmry,
						\\
						(\infrajetsym_{\predeg(j)})\pmr{\vec b} \oriof \mu\pmrhom{\vec a}{\vec b}
						&=& \infrajetsym\pmr{b_{\predeg(j)}} \oriof (\predeg(j) \cdot \mu)\pmrhom{\vec a}{b_{\predeg(j)}}
						&=& \infrajetsym\pmr{b_{\predeg(j)}} \oriof (p')_i\pmrhom{\ypolar}{b_{\predeg(j)}}
						&\overset{p' \neq \mixpol}{=}& \infrajetsym_i\pmry,
					\end{array}
				\]
				because $p' \neq \mixpol$.
				Then what we know is identical to what remained to be proven.
			\end{itemize}
			
			\item[\framebox{$i' = \predeg(i)$}]
			We show that $a_i = \ypolar$ and $\equijetsym_j\pmry \oriof \mu = \equijetsym_i\pmr{a_i}$.
			To this end, we invoke \cref{eq:poresh-withrr-crit2} for $\mu$:
			\begin{itemize}
				\item $i' := \getdeg(\predeg(j) \cdot \mu) < i$,
				\item $b_j \neq \npolar$,
			\end{itemize}
			therefore, $p_i\pmrhom{a_i}{b_j} := (j \cdot \mu)\pmrhom{\vec a}{b_j} \not\geq \isopol_i\pmrhom{a_i}{b_j}$, i.e.\ $p\pmrhom{a_i}{b_j} \neq \isopol\pmrhom{a_i}{b_j}$.
			This is only possible if $a_i = \ypolar$.
			
			Then we have
			\[
				(\equijetsym_j)\pmr{\vec b} \oriof \mu\pmrhom{\vec a}{\vec b}
				= \equijetsym\pmry \oriof (j \cdot \mu)\pmrhom{\vec a}{\ypolar}
				= \equijetsym\pmry \oriof p_i\pmryy
				\overset{p \neq \isopol}{=} \equijetsym_i\pmry.
			\]
			
			Next, we show that $\infrajetsym_{\predeg(j)}\pmr{b_{\predeg(j)}} \oriof \mu = \infrajetsym_{\predeg(i)}\pmr{a_{\predeg(i)}}$:
			\begin{description}
				\item[\framebox{$a_{\predeg(i)} = \npolar$}] We have
				\[
					(\infrajetsym_{\predeg(j)})\pmr{\vec b} \oriof \mu\pmrhom{\vec a}{\vec b}
					= \infrajetsym\pmr{b_{\predeg(j)}} \oriof (\predeg(j) \cdot \mu)\pmrhom{\vec a}{b_{\predeg(j)}}
					= \infrajetsym\pmr{b_{\predeg(j)}} \oriof \nonpol_{\predeg(i)}\pmrhom{\npolar}{b_{\predeg(j)}}
					= \edgesym_{\predeg(i)}\pmr{\npolar}.
				\]
				\item[\framebox{$a_{\predeg(i)} = \ypolar$}]
				Then by \cref{eq:poresh-withr-crit2} for $\mu$, we have $(p')\pmrhom{\ypolar}{b_{\predeg(j)}} \neq \mixpol\pmrhom{\ypolar}{b_{\predeg(j)}}$.
				Then we have
				\[
					(\infrajetsym_{\predeg(j)})\pmr{\vec b} \oriof \mu\pmrhom{\vec a}{\vec b}
					= \infrajetsym\pmr{b_{\predeg(j)}} \oriof (\predeg(j) \cdot \mu)\pmrhom{\vec a}{b_{\predeg(j)}}
					= \infrajetsym\pmr{b_{\predeg(j)}} \oriof (p')_{\predeg(i)}\pmrhom{\ypolar}{b_{\predeg(j)}}
					= \infrajetsym_{\predeg(i)}\pmry.
				\]
			\end{description}
			...
			\item[\framebox{$i' < \predeg(i)$}]
			This is impossible.
			To see this, we invoke \cref{eq:poresh-withrr-crit2} for $\mu$:
			\begin{itemize}
				\item $i' := \getdeg(\predeg(j) \cdot \mu) < i$,
				\item $p_i\pmrhom{a_i}{b_j} := (j \cdot \mu)\pmrhom{\vec a}{b_j} \geq \isopol_{\predeg(i)}\pmrhom{b_j}{a_{\predeg(i)}}$,
				\item $b_j \neq \npolar$. \qedhere
			\end{itemize}
		\end{description}
	\end{enumerate}
\end{proof}
\begin{lemma} \label{thm:poresh-jetgph-equality-eq-from-trcl}
	Let $\mu \in \Hom_\poreshcat(\vec a, \vec b)$ and $X \in \jetgph{\vec a}$.
	Then for all $x, y \in X$ and $Q_j \in \oridegeqtop(\vec b)$, if $[x]_\mu = [y]_\mu$, then $x \mathrel{(Q_j \oriof \mu)^*} y$.
\end{lemma}
\begin{proof}
	From $[x]_\mu = [y]_\mu$, we conclude that $x \mathrel{(\eqtysym \oriof \mu)^*} y$, and $\eqtysym \leq Q_j$.
\end{proof}
\begin{lemma} \label{thm:poresh-jetgph-rel-from-trcl}
	Let $\mu \in \Hom_\poreshcat(\vec a, \vec b)$ and $X \in \jetgph{\vec a}$.
	Then for all $x, y \in X$ and $Q_j \in \oridegeqtop(\vec b)$, if $[x]_\mu \mathrel{Q_j} [y]_\mu$, then $x \mathrel{(Q_j \oriof \mu)^*} y$.
\end{lemma}
\begin{proof}
	By definition of $\mu X$, there will be representants $x'$ and $y'$ such that $x' \mathrel{(Q_j \oriof \mu)} y'$.
	Then by the previous lemma, we have
	\[
		x \mathrel{(Q_j \oriof \mu)^*} x' \mathrel{(Q_j \oriof \mu)} y' \mathrel{(Q_j \oriof \mu)^*} y. \qedhere
	\]
\end{proof}
\begin{corollary} \label{thm:poresh-jetgph-trcl-from-trcl}
	Let $\mu \in \Hom_\poreshcat(\vec a, \vec b)$ and $X \in \jetgph{\vec a}$.
	Then for all $x, y \in X$ and $Q_j \in \oridegeqtop(\vec b)$, if $[x]_\mu \mathrel{Q_j^*} [y]_\mu$, then $x \mathrel{(Q_j \oriof \mu)^*} y$. \qed
\end{corollary}
...
\begin{theorem} \label{thm:poresh-jtset:nogo}
	The following, equivalent\footnote{Assuming an adjoint functor can be picked if one exists.}, objects \textbf{do not exist}:
	\begin{itemize}
...
\begin{proof}
	...
	\]
	from \cref{ex:cohesion} (which, as explained there, have a right adjoint), which compose to $\cohfget_0 \cohdisc_1 = \reshlist{\nonpol_\eqtydeg}{\isopol_0} : [\ypolar] \to [\ypolar]$.
	...
	\begin{align*}
		...
		& \text{(\cref{thm:poresh-jetgph-intervals})} \\
	...
	Since any two nodes in $\rival{\jetsym_0}$ are connected by a $1$-equijet, any function $\accol{0, 1} \to \accol{0, 1}$ between the carriers is a jet graph (hence jet tope) morphism.
	...
	\begin{align*}
		...
		&\quad=\quad
		\existscoend X^{{}\in \jettope{[\ypolar]}}.(\rival{\jetsym_1} \to \cohdisc_1 X) \times (\DSub{X}{\fpsh{(\cohdisc_0)} \yoneda \rival{\jetsym_0}})
		& \text{(construction of left Kan extension)} \\
		&\quad=\quad
		\existscoend X^{{}\in \jettope{[\ypolar]}}.(\rival{\jetsym_1} \to \cohdisc_1 X) \times (\cohdisc_0 X \to \rival{\jetsym_0}) \\
		&\quad\cong\quad
		\colim_{(X, f) \in \int_{X \in \jettope{[\ypolar]}} (\rival{\jetsym_1} \to \cohdisc_1 X)}
		(\cohdisc_0 X \to \rival{\jetsym_0})
	\end{align*}
	We will now use \cref{thm:reindex-colim} to simplify this colimit.
	Write $\catC$ for the category over which it ranges.
	
	A $1$-jet in $\cohdisc_1 X = \reshlist{\nonpol_\eqtydeg}{\pospol_0, \isopol_0} X$ is a $0$-infrajet in $X$.
	We thus obtain a cospan in $\catC$, where we denote jet tope morphisms by their carrier:
	\[
		(\rival{\jetsym_0}, \id_{\accol{0, 1}}) \to (\top, \_) \leftarrow (\rival{\tejsym_0}, \id_{\accol{0, 1}}).
	\]
	This amounts to a functor $I : \walkingcospan \to \catC$ from the walking cospan $\walkingcospan = \accol{\walkingcospanL \to \walkingcospanM \leftarrow \walkingcospanR}$.
	We construct a right adjoint $I \dashv R : \catC \to \walkingcospan$:
	\[
		R : \paren{
			\int_{X \in \jettope{[\ypolar]}} (\rival{\jetsym_1} \to \cohdisc_1 X)
			} \to \walkingcospan : (X, f) \mapsto 
		\left\{
			\begin{array}{l l}
				\walkingcospanM & \text{if $f : \rival{\equijetsym_0} \to X$, and otherwise:} \\
				\walkingcospanL & \text{if $f : \rival{\jetsym_0} \to X$}, \\
				\walkingcospanR & \text{if $f : \rival{\tejsym_0} \to X$}.
			\end{array}
		\right.
	\]
	This is easily checked to have an action on morphisms.
	Note that by $0$-antisymmetry of $X$, we can only get $R(X, f) = \walkingcospanM$ if $f$ is constant.
	It is immediately clear that $RI = \Id_\walkingcospan$.
	We construct the co-unit $\eps : IR \to \Id_\catC$ as follows:
	\[
		\eps_{(X, f)} =
		\left\{
			\begin{array}{l c l l}
				() \mapsto f(0) = f(1) &:& (\top, \_) \to (X, f)
				& \text{if $R(X, f) = \walkingcospanM$,} \\
				f &:& (\rival{\jetsym_0}, \id_{\accol{0, 1}}) \to (X, f)
				& \text{if $R(X, f) = \walkingcospanL$}, \\
				f &:& (\rival{\tejsym_0}, \id_{\accol{0, 1}}) \to (X, f)
				& \text{if $R(X, f) = \walkingcospanR$}.
			\end{array}
		\right.
	\]
	The triangle identity at $\walkingcospan$ is trivial, whereas the one at $\catC$ requires that $\eps_{Ix} = \id_{Ix}$, which is the case.
	
	We have established $I \dashv R$ and therefore $R\op \dashv I\op$, so that by \cref{thm:reindex-colim}, we can reindex along $I\op$. Write $\hat I x$ for the first component of $I x$.
	\begin{align*}
		&\quad\mathrel{\phantom{=}}\quad
		\colim_{(X, f) \in \int_{X \in \jettope{[\ypolar]}} (\rival{\jetsym_1} \to \cohdisc_1 X)}
		(\cohdisc_0 X \to \rival{\jetsym_0}) \\
		&\quad\cong\quad
		\colim_{x \in \walkingcospan} (\cohdisc_0 \hat I x \to \rival{\jetsym_0}) \\
		&\quad=\quad
	...
	\[
		\jtsetsym\withl(\cohfget_1 \cohcodisc_0) \cong \fpsh{(\cohdisc_1)} \circ \rpsh{(\cohdisc_0)},
	\]
	which is right adjoint to $\fpsh{(\cohdisc_0)} \circ \lpsh{(\cohdisc_1)}$ and therefore also non-idempotent.
	The intuitive reason is that a $0$-jet $\rival{\jetsym_0} \Rightarrow \jtsetsym\withl(\cohfget_1 \cohcodisc_0)\,X$ will consist of a pair of $0$-jets between the same nodes in $X$: one forward and one backward.
	Then a $0$-jet $\rival{\jetsym_0} \Rightarrow \jtsetsym\withl(\cohfget_1 \cohcodisc_0)^2\,X$ will consist of a quadruple of $0$-jets between the same nodes in $X$: one forward, one backward, another one backward and another one forward.
...
For this reason, not unlike our usage of jet graphoids in \cref{sec:poresh-jetgph}, we will take a detour via alternative categories that are equivalent to the categories of jet-topic sets but where our semantics do not act on adjoint triples as in \cref{thm:poresh-jtset:triple}, but rather on adjoint pairs (\cref{sec:poresh-jtset:pair-sin}).
That is:
\begin{itemize}
...
\begin{definition} \label{def:poresh-sin:obj}
	...
	\]
\end{definition}
When we let $\cohfget^\Sin$ act on a jet graph, its effect is to forget the inserted formal relations.
...
\end{proof}
The action of $\cohcodisc^\Sin$ on an $\vec a$-jet graph is to insert formal relations for $i$-equijets and for the true relation, and actually define them as the original $i$-equijet and true relations.
...
Composing sinistrification (\cref{sec:poresh-jtset:sin}) with \cref{thm:poresh-jtset:triple}, we get
\begin{align*}
...
\begin{proof}
	...
	Since $\jettope{\vec x}$ is a full subcategory of $\jetgph{\vec x}$,
	it is sufficient for $\cohdisc^\Sin : \jetgph{\vec a} \to \jetgph{\Sin(\vec a)}$ to be fully faithful.
	This is equivalent to invertibility of the unit of the monad $\cohfget^\Sin \cohdisc^\Sin : \jetgph{\vec a} \to \jetgph{\vec a}$, which follows from pseudofunctoriality of $\jetgphsym : \poreshcat \to \Cat$ and the fact that the polarized reshuffle $\cohfget^\Sin \cohdisc^\Sin : \vec a \to \vec a$ is the identity (we do not need to consider equality of 2-cells in the 2-poset $\poreshcat$).
\end{proof}
...
We do not want to allow equijets, because if the cubes in the base category have equijet dimensions, then these will give rise to a special sort of edges in the presheaf category which is distinct from, and stronger than, having jets pointing both ways.
If we want such a thing, we can always insert a symmetric degree in the polarmode mask.
...
Since jet cubes are realized as jet graphs by repeated application of twisted prism functors to the terminal jet graph (\cref{def:jep}), we can deduce the action of $\mu$ on jet cubes from its action on twisted prisms, which is given by \cref{thm:poresh-jetgph-prism-commut}.
We see that the outcome may be either: again a twisted prism, or a separated/cartesian product with $\rival{\edgesym_\eqtydeg}$ (i.e.\ the terminal jet graph), $\rival{\equijet_j}$ or $\rival{\edgesym_\top}$ (i.e.\ a discrete jet graph with two elements).
...
%Analogous to what we did in \cref{sec:poresh-jetgph}, we will first work on a variant of jet cubes, called jet cuboids%
%\footnote{The -oid is in the sense of setoid here.}
%$V$, which can have variables typed with the `equality interval' $\rival{\edgesym_\eqtydeg}$, and whose jet endpoint model $\JEP(V)$ is not a jet graph but a jet graphoid.
...
%	For any $(\myorix, \vec a)$-jet-cuboid $W$, we define the $\vec a$-jet-graphoid $\JEP(W)$ as in \cref{def:jep} with the additional equation
%	\[
...
%	
%	Moreover, the isomorphisms in \cref{def:cuboid-ep} and \cref{def:jet-cuboid-jep} are compatible along $U : \jetgph{\vec a} \to \Set$ and $\erasejc \loch$. \qed
%\end{corollary}
...
%	as the full subcategory of $\cuboidcat^\mycubex_M \times_\Set \jetgph{\vec a}$ on $(\myorix, \vec a)$-jet-cuboids, as justified by \cref{thm:jet-cuboid-pullback}.
...
\begin{definition}
	Let $M \in \accol{\bipointedsym, \booleanalg}$.
	We define a strict 2-functor $\jetcube{\orifb}{\affine}{M}{\loch} : \poreshcat\withrr \to \Cat$ which sends:
	\begin{itemize}
		...
		\begin{itemize}
			\item sends $()$ to $\mu () := ()$,
			\item has an action on other cubes that can be looked up in the following table\footnote{to be compared with the table in \cref{thm:poresh-jetgph-prism-commut}.}, where $j = \getdeg(i \cdot \rho)$:%
			\footnote{Note that if $i \cdot \rho = \isopol_j$, then by \cref{eq:poresh-withl-crit2}, $b_j = \npolar$.}
			\[
				\begin{array}{c | l || c | c || c}
					& a_i\,{\rightarrow} & \multicolumn{2}{c ||}{\ypolar} & \npolar \\ \hline
					{b_j}\,{\downarrow}
					& {i \cdot \rho}\,{\downarrow}
					& \mu(W, \dm i : \rival{\jetsym_i\pmry})
					& \mu(W, \dm i : \rival{\tejsym_i\pmry})
					& \mu(W, \dm i : \rival{\edgesym_i\pmrn})
					\\ \hline \hline
					\npolar
					& \nonpol_\eqtydeg\pmrhom{\npolar}{a_i}
					& \mu W
					...
					\multirow{2}{*}{$\ypolar$}
					& \pospol_j\pmrhom{\ypolar}{\ypolar}
					& (\mu W, \dm i : \rival{\jetsym_j\pmry})
					& (\mu W, \dm i : \rival{\tejsym_j\pmry})
					&
					\\
					& \negpol_j\pmrhom{\ypolar}{\ypolar}
					& (\Op_j\mu W, \dm i : \rival{\tejsym_j\pmry})
					& (\Op_j\mu W, \dm i : \rival{\jetsym_j\pmry})
					&
					\\ \hline
					\npolar
					& \nonpol_j\pmrhom{\npolar}{a_i}
					& (\mu W, \dm i : \rival{\edgesym_j\pmrn})
					& (\mu W, \dm i : \rival{\edgesym_j\pmrn})
					& (\mu W, \dm i : \rival{\edgesym_j\pmrn})
				\end{array}
			\]
			\item thus satisfies $\JEP(\mu W) \cong \mu \JEP(W)$ by \cref{thm:poresh-jetgph-prism-commut},
			\item has an action on jet cube morphisms defined as follows. Recall that a jet cube morphism $\vfi : V \to W$ is a cube morphism $\erasejc \vfi : \erasejc V \to \erasejc W$ whose endpoint model $\EP(\erasejc \vfi) = U(\JEP(\vfi))$ is in fact a morphism of jet graphs $\JEP(\vfi) : \JEP(V) \to \JEP(W)$ (\cref{def:jet-cube-cat}).
			\begin{itemize}
				\item We define $\dm j \psub{\erasejc{\mu(\vfi)}} := \dm j \psub{\erasejc{\vfi}}$.\footnote{Although dimension variables should in principle be desugared to De Bruijn indices before being formally meaningful, here we abuse the name-based representation to avoid headaches about dimension variables of type $\rival{P_i\pmry}$ being discarded when $i \cdot \rho = \nonpol_\eqtydeg\pmrhom{\npolar}{a_i}$.}
				\begin{itemize}
					\item This is well defined:
					From the calculus for jet cube morphisms (\cref{sec:jetcubes:calculus}), it can be deduced that if $\dm j : \rival{P_i}$ and $\dm j \psub{\erasejc{\vfi}}$ mentions $\dm i : \rival{P'_{i'}}$, then $i' \geq i$.
					Therefore, if $\dm i$ is deleted by $\mu$, then $i' \cdot \rho = \nonpol_\eqtydeg\pmrhom{\npolar}{a_{i'}}$.
					Since $i \cdot \rho \leq i' \cdot \rho$, we conclude that $i \cdot \rho = \nonpol_\eqtydeg\pmrhom{\npolar}{a_{i}}$ and therefore $\dm j$, too, is deleted by $\mu$.
				\end{itemize}
				\item It is the case that $\EP(\erasejc{\mu(\vfi)})$ is in fact a morphism of jet graphs $\JEP(\mu V) \to \JEP(\mu W)$;
				indeed it is essentially the morphism $\mu(\JEP(\vfi))$, as can be trivially proven by induction on the codomain $W$ of $\vfi$.
				\[
					\xymatrix{
						\EP(\erasejc{\mu V})
							\ar[rr]^{\EP(\erasejc{\mu(\vfi)})}
							\ar@{=}[d]
						&& \EP(\erasejc{\mu W})
							\ar@{=}[d]
						\\
						...
						\\
						U(\mu(\JEP(V)))
							\ar[rr]_{U(\mu(\JEP(\vfi)))}
						&& U(\mu(\JEP(W)))
					}
				\]
				This defines $\JEP(\mu \vfi)$ and thus completes the definition of $\mu(\vfi)$.
			\end{itemize}
			\item If $\mu \leq \nu$, then we have to give a morphism $\ket{\mu \leq \nu} : \mu W \to \nu W$, naturally w.r.t. $W$.
			We define $\erasejc{\ket{\mu \leq \nu}}$ by $\dm i \psub{\erasejc{\ket{\mu \leq \nu}}} = \dm i$.
			\begin{itemize}
				\item This is well defined: if $\dm i : \rival{P_i}$ is deleted by $\mu$, then $i \cdot \rho = \nonpol_\eqtydeg\pmrhom{\npolar}{a_i}$.
				Since $\mu \leq \nu$, we have $\sigma \leq \rho$ (where $\mu \dashv \rho$ and $\nu \dashv \sigma$), so that $i \cdot \sigma = \nonpol_\eqtydeg\pmrhom{\npolar}{a_i}$ and $\dm i$ is also deleted by $\nu$.
				\item This makes $\EP(\erasejc{\ket{\mu \leq \nu}})$ a jet graph morphism, precisely because $\mu \leq \nu$, thus defining $\JEP(\ket{\mu \leq \nu})$ and completing also the definition of $\ket{\mu \leq \nu}$.
			\end{itemize}
			
			This definition is clearly natural on cubes, therefore on sets, and therefore (by faithfulness of $U : \jetgph{\vec a} \to \Set$) also on jet graphs. By conclusion, it is natural in $\jetcube{\orifb}{\affine}{M}{\vec a}$.
		\end{itemize}
	\end{itemize}
\end{definition}
\begin{corollary}
	Let $M \in \accol{\bipointedsym, \booleanalg}$.
	We have a pseudofunctor $\jetcube{\orif}{\affine}{M}{\loch} : \poreshcat\withrr \to \Cat$ which sends $\vec a$ to $\jetcube{\orif}{\affine}{M}{\vec a}$.
\end{corollary}
\begin{proof}
	By transporting $\jetcube{\orifb}{\affine}{M}{\loch}$ along the equivalences given by \cref{thm:backward-jets-equivalent}.
\end{proof}
...
\begin{theorem} \label{thm:poresh-jcset:triple}
	...
	\begin{itemize}
		...
		\item 2-cells are also respected.
	\end{itemize}
	Moreover $\jcset{\orifb}{\affine}{M}{\loch}\withlr$ is in fact a strict 2-functor.
\end{theorem}
\begin{proof}
	By the exact same reasoning as in the proof of \cref{thm:poresh-jtset:triple}.
	The strict 2-functoriality of $\jcset{\omega}{\affine}{M}{\loch}\withlr$ follows from the strict 2-functoriality of $\jetcube{\omega}{\affine}{M}{\loch}\withrr$
\end{proof}
...
\begin{proof}
	...
	\begin{itemize}
		\item That the interval jet graph $\rival{\jetsym_0}$ needs to be replaced with the 1-dimensional jet cube $(\dm i : \rival{\jetsym_0})$.
		\item That all jet graph morphisms of $\JEP$s of jet cubes, whose domain is 1-dimensional, are in fact jet cube morphisms. \qedhere
	\end{itemize}
\end{proof}
...
\todoi{Revisit \cref{ex:pjb-paramty}.}
...
\begin{enumerate}
	\item[v0.1]
	\begin{itemize}
		\item Wrote \cref{ch:modes}.
	\end{itemize}
	\item[v0.1.1]
	\begin{itemize}
		\item Changed title.
		\item Proofread \cref{ch:modes}.
		\item Added missing clause for \ruleref{concursor} in proof of \cref{thm:calculus-sound}.
		\item Cleaned up affine boolean normalization lemmas in \cref{sec:jetcubes:calculus:lemmas}.
		\item Introduced orientation kits (\cref{def:orientation-kit}).
		\item Introduced conventional cubes (\cref{def:conv-jet-cube-cat}).
		\item General cleanup.
	\end{itemize}
	\item[v0.2]
	\begin{itemize}
		\item Wrote \cref{ch:modalities}.
		\item Changed title.
		\item Renamed jet jewels to jet topes; modified their definition (\cref{def:jet-tope}).
	\end{itemize}
	\item[v0.3]
	\begin{itemize}
		\item Renamed anpolarities to polarmodes.
		\item New notation for both polarmodes.
		\item Added colored polarmode annotations.
		\item Very thorough proofread, involving serious modifications of certain proofs, purging a number of incorrect lemmas, and restructuring of some sections.
		\item Abolished jet setoids and jet cuboids in \cref{ch:modalities}.
		\item Inserted \cref{ch:prereq} on prerequisites.
		\item Renamed jet sets to jet graphs.
	\end{itemize}
\end{enumerate}
