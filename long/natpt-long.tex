...
\begin{definition} \label{def:jetsetoid-interval}
	We view the intervals from \cref{def:interval} not just as jet sets but also as jet setoids. We additionally introduce $\rival{\eqtysym}$, the jet setoid on $\accol{0, 1}$ generated by $0 $
\end{definition}
\begin{proposition} \label{thm:poresh-jetsetoid-intervals}
	...
	\end{equation}
	where we define $\rival{\topsym}$ as the jet set with carrier $\accol{0, 1}$ on which all $i$-jet relations are reflexive, and $\rival{\eqtysym}$ as the terminal jet set.%
	\footnote{By virtue of \cref{thm:poresh-adjoint}, we do not have to define $\rival{\infrajet_j}$ for $b_j = \ypolar$.}
\end{proposition}
\begin{proof}
	Suppose $\jetsym_i \cdot \rho = \eqtysym$. Then each of the right hand sides in \cref{eq:poresh-jetsetoid-intervals} is the terminal jet set.
	From \cref{thm:poresh-adjoint}, we know that $\eqtysym \cdot \mu \geq \jetsym_i$ and therefore, by symmetry of $\eqtysym \cdot \mu$, that $\eqtysym \cdot \mu \geq \infrajetsym_i$.
	Then $0$ and $1$ are related by $\eqtysym \cdot \mu$ in $\rival{\jetsym_i}$, $\rival{\tejsym_i}$ and $\rival{\equijetsym_i}$ and therefore identified in the jet sets obtained by applying $\mu$ to the left hand sides of \cref{eq:poresh-jetsetoid-intervals}.
	
	Suppose $\jetsym_i \cdot \rho > \eqtysym$. Then the right hand sides of \cref{eq:poresh-jetsetoid-intervals} have carrier $\accol{0, 1}$.
	From \cref{thm:poresh-adjoint}, we know that $\eqtysym \cdot \mu < \jetsym_i$ and therefore, by symmetry of $\eqtysym \cdot \mu$, that $\eqtysym \cdot \mu < \equijetsym_i$.
	Then $0$ and $1$ are \emph{not} related by $\eqtysym \cdot \mu$ in $\rival{\jetsym_i}$, $\rival{\tejsym_i}$ and $\rival{\equijetsym_i}$ and therefore \emph{not} identified in the jet sets obtained by applying $\mu$ to the left hand sides of \cref{eq:poresh-jetsetoid-intervals}.
