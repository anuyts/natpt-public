...
\begin{definition} \label{def:jet-cube-erasure}
	We define the \textbf{jet-erasure function} $\erasejc{\loch}$, which sends jet cubes of any mode to cubes (i.e. objects of any of the cube categories defined in \cref{sec:cubes}), by
	\[
...
\subsubsection{Jet Cube Morphisms}
\begin{definition} \label{def:jet-cube-cat}
	Let $\vec a$ be a mode and $\eps \in \accol{\affine, \cartes}$.
	We define the category $\jetcubeopt{\eps}{M}{\vec a}$ of \textbf{affine/cartesian jet $M$-cubes of mode $\vec a$} as the category
	\begin{itemize}
...
\begin{definition} \label{def:prism-cube}
	Let $\vec a$ be a mode, $\eps \in \accol{\affine, \cartes}$ and $i < \lenpol{\vec a}$. We define the \textbf{$i$-twisted prism functor $\loch \multip (\dm i : \rival{\jet_i}) : \jetcubeopt{\eps}{M}{\vec a} \to \jetcubeopt{\eps}{M}{\vec a}$} as follows:\footnote{We still think of $\dm i$ as a De Bruijn index.}
	\begin{itemize}
		...
		\end{itemize}
	\end{itemize}
	If $a_i = \npolar$, then we call this simply the \textbf{$i$-prism functor $\loch \multip (\dm i : \rival{\edge_i})$.}
\end{definition}
\begin{proposition} \label{thm:prism-commute}
	Twisted prism functors for different dimensions commute.
\end{proposition}
\begin{proof}
	On jet-erased cubes, we have an isomorphism
	\begin{description}
		\item[\framebox{$\eps = \affine$}] $(i/j, j/i) \quad:\quad \loch * (\dm i : \IX) * (\dm j : \IX) \quad\cong\quad \loch * (\dm i : \IX) * (\dm j : \IX)$,
		\item[\framebox{$\eps = \cartes$}] $(i/j, j/i) \quad:\quad \loch \times (\dm i : \IX) \times (\dm j : \IX) \quad\cong\quad \loch \times (\dm i : \IX) \times (\dm j : \IX)$.
	\end{description}
	One easily checks that this isomorphism respects the jet relations.
\end{proof}

\subsubsection{A Calculus of Polarized Jet Cube Morphisms}
In this section, we develop a calculus that inductively generates the morphisms of the category $\jetcubeopt{\eps}{M}{\vec a}$.
By construction, this category is a full subcategory of the strict pullback of categories $\cubecat^\eps_M \times_{\Set} \jetset{\vec a}$.
Our calculus will consider morphisms in a larger full subcategory, namely that on objects generated by the terminal jet cube $()$, the twisted prism functors (\cref{def:prism-cube}) and the opposite endofunctors $\Op_i := (\Id, \Op_i)$ on $\cubecat^\eps_M \times_{\Set} \jetset{\vec a}$.
\begin{definition} \label{def:prism-op}
	The \textbf{opposite twisted prism functors} on $\jetset{\vec a}$ and $\jetcubeopt{\eps}{M}{\vec a}$ are defined by
	\begin{itemize}
		\item $\loch \multip \rival{\tej_i} := \Op_i \circ (\loch \multip \rival{\jet_i}) : \jetset{\vec a} \to \jetset{\vec a}$,
		\item $\loch \multip (\dm i : \rival{\tej_i}) := \Op_i \circ (\loch \multip (\dm i : \rival{\jet_i}))$.
	\end{itemize}
	If $a_i = \npolar$ then these are equal to the twisted prism functors.
\end{definition}
Note that the twisted prism functor takes the opposite of its input jet set/cube at its source but not at its target. If we reverse the direction of the jets in the prism dimension, we also swap source and target and thus take the opposite of the entire construction. As such, a sensible notation for the $i$-opposite of $(\dm i : \rival{\jet_i}, \dm j : \rival{\jet_i}, \dm k : \rival{\jet_i})$ is $(\dm i : \rival{\jet_i}, \dm j : \rival{\jet_i}, \dm k : \rival{\tej_i})$.
\begin{definition} \label{def:polarized-jet-cube-cat}
	The \textbf{category of polarized jet cubes} $\pjetcubeopt{\eps}{M}{\vec a}$ is defined as the full subcategory of $\cubecat^\eps_M \times_{\Set} \jetset{\vec a}$ on objects generated by the twisted prism functors and opposite functors.
\end{definition}
\begin{corollary}
	$\Op_i$ acts on a polarized jet cube by reversing the last arrow of degree $i$.
\end{corollary}
\begin{proof}
	$\Op_i$ is easily seen to commute with $\Op_j$ and $j$-twisted prism functors for $j \neq i$.
\end{proof}
\begin{corollary}
	Objects of $\pjetcubeopt{\eps}{M}{\vec a}$ are given by the grammar
	\[
		W \quad ::= \quad () \quad | \quad (W, \dm i : \rival{\jet_i}) \quad | \quad (W, \dm i : \rival{\tej_i}),
	\]
	with $0 \leq i < \lenpol{\vec a}$ and $\dm i$ really being a binder for a De Bruijn index. \qed
\end{corollary}
\begin{convention}
	When presenting the calculus, with permission from \cref{thm:prism-commute}, we will order the dimensions of a polarized jet cube by \emph{decreasing} degree.
\end{convention}
\begin{remark}
...
Companion: \todoi{Either here or in conjunction/disjunction, we have to allow for more complicated expressions}
\[
	\inference{
...
Positive conjunction and negative disjunction (affine):
\[
	\inference{
		(\vfi, t_1/\dm i_1, \ldots, t_n/\dm i_n) : V \to (W, \dm i_1 : \rival{\jet_i}, \ldots, \dm i_n : \rival{\jet_i})
		\qquad
		\dom(\vfi) = \shp^\ypolar_i V
	}{
		(\vfi, t_1 \wedge \ldots \wedge t_n/\dm i) : V \to (W, \dm i : \rival{\jet_i})
	}{$\booleanalg$, $\affine$}
\]
\[
	\inference{
		(\vfi, t_1/\dm i_1, \ldots, t_n/\dm i_n) : V \to (W, \dm i_1 : \rival{\tej_i}, \ldots, \dm i_n : \rival{\tej_i})
		\qquad
		\dom(\vfi) = \shp^\ypolar_i V
	}{
		(\vfi, t_1 \vee \ldots \vee t_n/\dm i) : V \to (W, \dm i : \rival{\tej_i})
	}{$\booleanalg$, $\affine$}
\]

Positive disjunction and negative conjunction (affine):
\[
	\inference{
		(\vfi, t_1/\dm i_1, \ldots, t_n/\dm i_n) : V \to (W, \dm i_1 : \rival{\tej_i}, \ldots, \dm i_{n-1} : \rival{\tej_i}, \dm i_n : \rival{\jet_i})
		\qquad
		\dom(\vfi) = \shp^\ypolar_i V
	}{
		(\vfi, t_1 \vee \ldots \vee t_n/\dm i) : V \to (W, \dm i : \rival{\jet_i})
	}{$\booleanalg$, $\affine$}
\]
\[
	\inference{
		(\vfi, t_1/\dm i_1, \ldots, t_n/\dm i_n) : V \to (W, \dm i_1 : \rival{\jet_i}, \ldots, \dm i_{n-1} : \rival{\jet_i}, \dm i_n : \rival{\tej_i})
		\qquad
		\dom(\vfi) = \shp^\ypolar_i V
	}{
		(\vfi, t_1 \wedge \ldots \wedge t_n/\dm i) : V \to (W, \dm i : \rival{\tej_i})
	}{$\booleanalg$, $\affine$}
\]
