...
\begin{corollary} \label{thm:commute}
	\begin{enumerate}
		\item Let $F_i$ be a functor between jet set categories of any of the following forms: $\Op_i$, $\Fsym_i$, $\Usym_i$, $\Csym_i$, $\loch \multip \rival{\jet_i}$, $\loch \multip \rival{\tej_i}$, $\loch \multip \rival{\edge_i}$.
		...
		\item The functor $\loch \multip \rival{\edge_i}$ commutes with itself, i.e. $(x, v, w) \mapsto (x, w, v)$ is a natural automorphism of $\loch \multip \rival{\edge_i} \multip \rival{\edge_i}$. \qed
	\end{enumerate}
\end{corollary}
...
\begin{proposition} \label{thm:cube-commute}
	\begin{enumerate}
		\item Any two functors on jet cubes concerned in \cref{thm:lift-to-jet-cubes}, instantiated on different degrees, commute.
	In other words, the natural transformation given in \cref{thm:commute} (1.) lifts to (forward/trioriented) jet cubes when the associated functors lift.
	
		\item Moreover, the functor $\loch \multip (\dm i : \rival{\edge_i})$ commutes with itself, i.e.\ the natural automorphism given in \cref{thm:commute} (2.) lifts to (forward/trioriented) jet cubes as $(\dm i / \dm i, \dm j / \dm j) : \loch \multip (\dm i : \rival{\edge_i}) \multip (\dm j : \rival{\edge_i}) \cong \loch \multip (\dm j : \rival{\edge_i}) \multip (\dm i : \rival{\edge_i})$.
	\end{enumerate}
\end{proposition}
\begin{proof}
...
\begin{convention} \label{conv:sort-cubes}
	When presenting the calculus, as justified by \cref{thm:cube-commute} (1.), we will order the dimensions of a (forward/trioriented) jet cube by \emph{decreasing} degree.
\end{convention}
\begin{figure} \label{fig:calculus}
	...
	\[
		\inferencel{exchange}{
			\sez \vfi : V \to (W, \dm j : \rival{\edge_i}, U_1, \dm i : \rival{\edge_i}, U_2)
		}{
			\sez \vfi : V \to (W, \dm i : \rival{\edge_i}, U_1, \dm j : \rival{\edge_i}, U_2)
		}{}
	\]
	\[
...
\begin{proof}
	...
	\begin{itemize}
		...
		\item Soundness of \ruleref{exchange} follows from \cref{thm:cube-commute} (2.).
...
\begin{theorem}[Completeness] \label{thm:calculus-complete}
	For any morphism $\hat \vfi : V \to W$ in $\jetcubeopt \affine M {\vec a}$ between cubes compliant to \cref{conv:sort-cubes}, writing $\vfi = \erasejc{\hat \vfi}$, we have $\sez \vfi : V \to W$.
\end{theorem}
\begin{proof}
	First, note that $\vfi$ assigns to each variable $\dm k$ in $W$ an boolean expression over the variables in $V$.
	After pushing down negations, eliminating double negations, and simplifying conjunctions and disjunctions with $0$ and $1$, we obtain an expression $e_{\dm k}$ that is either $0$, $1$ or a binary tree with $\vee$ and $\wedge$ in the nodes and (negations of) variables of $V$ in the leaves.

	We prove completeness by induction on the number of nodes and leaves in $(e_{\dm k})_{\dm k \in W}$.
	...
	If [$\dm i : \rival{\jet_i}$ and $e_{\dm i} = 0$] or [$\dm i : \rival{\tej_i}$ and $e_{\dm i} = 1$], then $\vfi$ being a jet cube morphism $V \to W = (U, \dm i : \_)$ (equivalently: $\EP(\vfi)$ being a jet set morphism $\JEP(V) \to \JEP(W)$) is equivalent to $(\novar / \dm i) \circ \vfi$ being a jet cube morphism $V \to \Opcube_i(U)$, so we can apply \ruleref{zero:fwd} or \ruleref{one:bck}.
	
	If [$\dm i : \rival{\jet_i}$ and $e_{\dm i} = 1$] or [$\dm i : \rival{\tej_i}$ and $e_{\dm i} = 0$], then $\vfi$ being a jet cube morphism $V \to W = (U, \dm i : \_)$ (equivalently: $\EP(\vfi)$ being a jet set morphism $\JEP(V) \to \JEP(W)$) is equivalent to $(\novar / \dm i) \circ \vfi$ being a jet cube morphism $V \to U$, so we can apply \ruleref{one:fwd} or \ruleref{zero:bck}.
	
	If $\dm i : \rival{\jet_i}$ and $e_{\dm i} = \lnot \dm j$, then $\vfi$ being a jet cube morphism $V \to W = (U, \dm i : \rival{\jet_i})$ is equivalent to $(\lnot \dm i / \dm i) \circ \vfi$ being a jet cube morphism $V \to (U, \dm i : \rival{\tej_i})$, so we can apply \ruleref{inv:fwd}.
	Similarly, if $\dm i : \rival{\tej_i}$ and $e_{\dm i} = \lnot \dm j$, we can apply \ruleref{inv:bck}.
	
	If $\dm i : \rival{\jet i}$ and $e_{\dm i} = \dm j$ where $V$ specifies that $\dm j$ has degree $j$, then we know that $\vfi = (\chi, \dm j / \dm i)$ is a jet cube morphism $V = (V_0, \dm j : \rival{R_j}, V_1) \to W = (U, \dm i : \rival{\jet_i})$ for some $R \in \accol{\jet, \tej, \equijet}$.
	...
	\begin{itemize}
		\item If $j = i$, then $R = {\jet}$ and $\chi$ is both a jet cube morphism $(V_0, V_1) \to U$ and $(V_0, \Opcube_i(V_1)) \to U$.
		...
		\begin{itemize}
			...
			\begin{itemize}
				\item If $V_1$ contains any variables, then we can apply \ruleref{wkn:fwd}.
				\item Otherwise, we can apply \ruleref{prism:fwd}.
			\end{itemize}
			
			\item If $a_i = \npolar$, then by the reordering applied before, we know that $\dm j$ is the last variable in $V$, so we can apply \ruleref{prism:fwd} (or equivalently \ruleref{prism:bck}).
		\end{itemize}
		
		If $j > i$, then $\chi$ is necessarily a jet cube morphism 
	\end{itemize}
	...
	\begin{itemize}
		\item If $j = i$ and $a_i = \ypolar$, then clearly we must have $\dm j : \rival{\jet_i}$, i.e.\ $\dm j$ is a jet variable (and not a reverse jet or equijet variable).
		We first show that $\dm j$ must be the last variable of $V$.
		A priori, we know that $\vfi = (\chi, \dm j / \dm i)$ is a jet cube morphism $V = (V_0, \dm j : \rival{\jet_i}, V_1) \to W = (U, \dm i : \rival{\jet_i})$.
	\end{itemize}
