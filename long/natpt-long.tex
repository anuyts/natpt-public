...
\begin{definition} \label{def:jetset}
	...
	such that
	\begin{itemize}
		\item when $a_i = \npolar$, then $\jet_i$ is an equivalence relation (i.e.\ symmetric), in which case we will denote it as $\edge_i$ and call it the \textbf{$i$-edge relation} (notwithstanding that we still consider it a special case of a jet relation),
...
\begin{definition}
	...
	We define $\Op_i(X)$ as the jet set with the same carrier and relations as $X$ except that the $i$-jet relation is reversed: $x \jet_i^{\Op_i(X)} y$ if and only if $x \tej_i^X y$.
...
\end{definition}
Note that it would be meaningless to define an $i$-ripple interval in the same way.
