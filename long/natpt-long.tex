...
Note also that, while the nature of the `product' varies, the second component is always $\rival{\jetsym_i\pmr{a_i} \oriof \rho\pmrhom{\vec b}{\vec a}}$.
\begin{proof}
		...
		\begin{itemize}
			...
			\begin{itemize}
				...
				\begin{itemize}
					...
					\item[\textbf{(C)}] $y \mathrel{R'_{k'}} x$ in $X$, implying $[y]_\mu \jet_\ell [x]_\mu$ in $\mu X$. Additionally, $k' = 1$ and $u \neq w$.
				...
				\begin{description}
					...
					\begin{itemize}
						...
						\begin{itemize}
							\item[\framebox{$\ell \cdot \mu = \mixpol_i\pmrhom{\ypolar}{b_\ell}$}]
							In this case $R_i = \equijet_i$, a symmetric relation, so that we can conclude (D) by similar reasoning as in the previous point.
				...
				\begin{itemize}
					\item[\framebox{$k' \neq i$}] Then we have $x \mathrel{R'_{k'}} y$ in $X$, hence $[x]_\mu \jet_\ell\pmr{b_\ell} [y]_\mu$ in $\mu X$.
				\end{itemize}
				...
				\todoi{Further case distinguish}
				We distinguish several cases, bearing in mind \cref{thm:poresh-adjoint-directed}:
				\begin{description}
					\item[\framebox{$k' = i$, $a_i = b_j = \ypolar$ and $j \cdot \mu = \pospol_i\pmryy$ or equivalently $i \cdot \rho = \pospol_j\pmryy$}]
					Then $R'_{k'} = \jetsym_j\pmry \oriof \mu = \jetsym_i\pmry$, so we have $(x, u) \jet_i (y, u)$ in $X \multip \rival{P_i}$, i.e.\ $x \jet_i y$ in $X$ if $u = w$ and $y \jet_i x$ in $X$ if $u \neq w$.
					Then in $\mu X$, we have $[x]_\mu \jet_j [y]_\mu$ if $u = w$ and $[y]_\mu \jet_j [x]_\mu$ if $u \neq w$.
					
					Furthermore, $P_i\pmry \oriof \rho\pmrhom{\vec b}{\vec a} = P\pmry \oriof (i \cdot \rho)\pmrhom{\vec b}{\ypolar} = P_j\pmry$, so that $Y = (\mu X) \multip \rival{P_j\pmry}$.
					Then we finally find $([x]_\mu, u) \jet_i ([y]_\mu, u)$.
					
					\item[\framebox{$a_i = b_j = \ypolar$ and $i \cdot \rho = \negpol_j\pmryy$ or equivalently $j \cdot \mu = \negpol_i\pmryy$}]
					Then $R'_{k'} = \jetsym_j\pmry \oriof \mu = \tejsym_i\pmry$, so we have $(x, u) \tej_i (y, u)$ in $X \multip \rival{P_i}$, i.e.\ $x \tej_i y$ in $X$ if $u = w$ and $y \tej_i x$ in $X$ if $u \neq w$.
					Then in $\mu X$, we have $[x]_\mu \jet_j [y]_\mu$ if $u = w$ and $[y]_\mu \jet_j [x]_\mu$ if $u \neq w$.
					
					Furthermore, $P_i\pmry \oriof \rho\pmrhom{\vec b}{\vec a} = P\pmry \oriof (i \cdot \rho)\pmrhom{\vec b}{\ypolar} = (P \oriof \negpol)_j\pmry$, so that $Y = (\Op_j \mu X) \multip \rival{(P \oriof \negpol)_j\pmry}$.
					We have $[x]_\mu \tej_j [y]_\mu$ in $\mu X$ if $u = w$ and $[y]_\mu \tej_j [x]_\mu$ in $\Op_j \mu X$ if $u \neq w$.
					In $\Op_j \mu X$, we have $[x]_\mu \tej_i [y]_\mu$ if $u = w$ and $[y]_\mu \tej_i [x]_\mu$ if $u \neq w$.
					Then in $(\Op_j \mu X) \multip \rival{(P \oriof \negpol)_j\pmry}$, we finally find $([y]_\mu, u) \tej_i ([x]_\mu, u)$, i.e.\ $([x]_\mu, u) \jet_i ([y]_\mu, u)$.
				\end{description}
				\todoi{Am I mixing up $j$ in the theorem and in the proof?}
			\end{itemize}
			
		\end{itemize}
	\end{enumerate}
		\todoi{Finish the proof}
\end{proof}
