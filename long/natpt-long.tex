...
\begin{proof}
	...
	\begin{enumerate}
		...
		\end{itemize}
		We now show that this isomorphism respects the $j$-jet relations in both directions, for $j \in \Deg(\vec b)$.
		Let $\jetsym_j \oriof \mu =: R'_{k'}$.
		...
		\begin{itemize}
			\item[\framebox{$\to$}]
			Take a non-reflexive jet $[(x, u)]_\mu \jet_j [(y, v)]_\mu$ in $\mu(X \multip \rival{P_i})$.
			...
			Then this is equivalent to $x \mathrel{R'_{k'}} y$, implying $[x]_\mu \jet_j [y]_\mu$.
			
			\item[\framebox{$\leftarrow$}]
			Take a non-reflexive jet $[x]_\mu \jet_j [y]_\mu$ in $\mu X$.
			Then there exist representants, \WLOG{} $x$ and $y$, such that $x \mathrel{R'_{k'}} y$.
			This is equivalent to $(x, 0) \mathrel{R'_{k'}} (y, 0)$, implying $[(x, 0)]_\mu \jet_j [(y, 0)]_\mu$.
		\end{itemize}
		...
		\begin{itemize}
			...
			\begin{itemize}
				...
				We have that $P'_i \leq R'_{k'} = \jetsym_j \oriof \mu$, or equivalently (since $\loch \oriof \rho \dashv \loch \oriof \mu$) that $P'_i \oriof \rho \leq \jetsym_j$.
				This implies that $u \jet_j v$ in $\rival{P_i \oriof \rho}$, so that $([x]_\mu, u) \jet_j ([x]_\mu, v)$ in $Y$.
				
				\item If $x \neq y$ and $u = v$, then we have $(x, u) \mathrel{R'_{k'}} (y, u)$.
				Let $w = 1$ if $P = \jetsym$ and $w = 0$ if $P = \tejsym$.
				We distinguish three cases, bearing in mind \cref{thm:poresh-adjoint-directed}:
				\begin{description}
					\item[\framebox{$a_i = b_j = \ypolar$ and $i \cdot \rho = \pospol_j\pmryy$ or equivalently $j \cdot \mu = \pospol_i\pmryy$}]
