...
\begin{definition}
	...
	\begin{itemize}
		...
		\item We say that $X$ is \textbf{$i$-orientable} if on any $i$-connected component, the relation $\jetsym_i^*$ is a total pre-order.
		\item We say that $X$ is \textbf{$i$-antisymmetric} if $x \jet_i^* y$ and $x \tej_i^* y$ together imply $x \infrajet_{i-1}^* y$ if $i > 0$ and $x = y$ if $i = 0$.
	\end{itemize}
\end{definition}
%\begin{definition} 
...
\begin{definition} \label{def:jet-jewel}
	...
	\begin{enumerate}
		...
		\item it is $i$-orientable for every directed degree $i$,
		\item it is $i$-antisymmetric for every directed degree $i$.
...
\begin{proposition} \label{thm:lift-to-jet-jewels}
	Jet jewels are closed under the functors $\Op_i$, $\Fsym_i$,
...
\begin{proof}
	\begin{description}
		\item[$\Op_i$] Trivial.
		\item[$\Fsym_i$] This functor clearly preserves $(n-1)$-connectedness even if $i = n-1$. Meanwhile, $j$-orientability and $j$-antisymmetry are unaffected for $j \neq i$ and not required for $j = i$. In the case of $(i+1)$-antisymmetry, we use that $\infrajetsym_i$ remains unchanged.
...
%		\item[$\UCsym_{i \neq n-1}$] Since $\UCsym_i = \Usym_i \circ \Csym_i$.
		...
		\end{itemize}
		\todoi{Continue proof}
		Assuming $a_j = \ypolar$, for $j$-orientability, we consider 3 situations:
		\begin{description}
...
\section{Semantics on Jet Jewels} \label{sec:poresh-jetjewel}
