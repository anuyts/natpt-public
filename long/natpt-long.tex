...
\begin{definition} \label{def:prism-cube}
	Let $\vec a$ be a mode, $\eps \in \accol{\affine, \cartes}$ and $i < \lenpol{\vec a}$. We define the \textbf{$i$-twisted prism functor $\loch \multip (\dm i : \rival{\jet_i}) : \jetcubeopt{\eps}{M}{\vec a} \to \jetcubeopt{\eps}{M}{\vec a}$} on jet cubes as follows:\footnote{We still think of $\dm i$ as a De Bruijn index.}
	\begin{itemize}
...
\begin{proposition} \label{thm:prism-commute}
	((Opposable) twisted) prism functors for different dimensions commute.
\end{proposition}
\begin{proof}
