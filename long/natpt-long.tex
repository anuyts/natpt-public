...
%			& \text{($\jetsym \oriof \loch$ is an isomorphism)} \\
		...
		\begin{equation}
			j \cdot \lambda = \ell_{b_j}(\pospol_j) \in \poldegeqtop(\vec a, b_j).
		\end{equation}
		...
		\begin{itemize}
			...
			\begin{itemize}
				...
				To this end, we need to prove that for all $I \in \Degeq(\vec a)$, we have $(\pospol_I)\pmrhom{\vec a}{a_I} \leq (I \cdot \rho) \circ \lambda \pmrhom{\vec a}{\vec b}$.
				...
				By unfolding the definition of $\ell$ and noting that $q_j \circ \loch$ is monotonically increasing and therefore maximum-preserving, we need to prove, for each $c$ and $q_j$ as above:
				\begin{align*}
				...
				We set $\hat i = i$ and $\hat p \pmrhom{a_i}{b_j} = d\pmrhom{c}{b_j} \circ p d\pmrhom{a_i}{c}$.
