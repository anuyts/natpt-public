...
\begin{example} \label{ex:pjb-paramty}
	...
	The modality $\cohcodisc_0^{\maskopfont{PJB}}$ will be used to lift structure inhabiting a covariant functor to $\Type^{\vec b}$, to mode $\vec a$, e.g. to build the category of pointed types. It could be called `covariant structurality'.\todo{Not 100\% sure right now.}
\end{example}
%\begin{example}
...
\subsection{Sinistrification} \label{sec:poresh-jtset:sinistrification}
...
\footnote{Shulman \cite{matt} introduces a construction called \emph{co-dextrification} which adds left adjoints to existing morphisms in a certain 2-category in such a way that the result satisfies a co-free property. He should perhaps have called it \emph{co-free dextrification}, so that the free construction could be called \emph{free dextrification} and the word \emph{dextrification} per se would be available when no universal property is proven. We mean the words sinistrification (Lat.\ \emph{sinister}: `left'), dextrification (Lat.\ \emph{dexter}: `right') and ambidextrification (Lat.\ \emph{ambidexter}: `ambidextrous') in this latter sense: we do not prove any universal property.}
These operation make use of \cref{def:pb} and its \emph{big degree} notation.

\bigskip

By \cref{thm:poresh-adjoint}, left adjoints cannot mention $\nonpol_\top$ or $\mixpol_i$ for symmetric degrees $i$.
...
\begin{definition} \label{def:poresh-sin:obj}
	...
	\[
		\left\{ \begin{array}{r l l l}
			...
			i \cdot \cohfget^\Sin &:=& \nwnonpol_i &\text{if $a_i = \npolar$}, \\
			i \cdot \cohfget^\Sin &:=& \nwpospol_i &\text{if $a_i = \ypolar$}.
		\end{array} \right.
	\]
\end{definition}
...
\begin{proposition} \label{thm:poresh-sin:cohesion}
	The polarized reshuffle $\cohfget^\Sin$ is part of an adjoint triple $\cohdisc^\Sin \dashv \cohfget^\Sin \dashv \cohcodisc^\Sin$ where $\cohdisc^\Sin, \cohcodisc^\Sin : \vec a \to \Sin(\vec a)$ are defined by
	\[
		\left\{ \begin{array}{r l l l}
...
\end{proposition}
\begin{proof}
	We let the operations act degree-wise.
\end{proof}
...
\begin{proof}
	...
	\begin{align*}
		...
		& (\text{\cref{thm:poresh-sin:div-poresh}}) \\
		&= ((q \circ (j \cdot \mu)) \divleft \cohcodisc^\Sin)
		& (\text{argued above}) \\
		&= ((q_j \circ \mu) \divleft \cohcodisc^\Sin) \\
		&= ((k \cdot (\nu \circ \mu)) \divleft \cohcodisc^\Sin)
...
\begin{definition}
	...
	\end{itemize}
\end{definition}
Thus, the action of $\loch \cdot \Sin(\mu)$ is to first rewrite formal polarized degrees to their original counterparts ($\loch \cdot \cohcodisc^\Sin$), then look up those degrees in $\mu$, and finally rewrite illegal degrees to their formal counterparts ($\loch \divleft \cohcodisc^\Sin$).
\begin{proposition}
...
\subsection{Strict Semantics on Sinistrified Modal Jet Topic Setoids} \label{sec:poresh-jtset:strict}
...
%\todoi{Sinistrification or dextrification?}
%\todoi{Endofunctor on $\poreshcat$}

\subsection{Strict Semantics on Sinistrified Modal Jet Cubical Sets} \label{sec:poresh-jcset:strict}
