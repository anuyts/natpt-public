...
\begin{proof}
	...
	\begin{align*}
		...
		&\quad\cong\quad
		\colim_{(X, \vfi) \in \int_{X \in \jettope{[\ypolar]}} (\rival{\jetsym_1} \to \cohdisc_1 X)}
	...
	\[
		R : \paren{
			...
			} \to \walkingcospan : (X, \vfi) \mapsto 
		\left\{
			\begin{array}{l l}
				\walkingcospanM & \text{if $\vfi : \rival{\equijetsym_0} \to X$, and otherwise:} \\
				\walkingcospanL & \text{if $\vfi : \rival{\jetsym_0} \to X$}, \\
				\walkingcospanR & \text{if $\vfi : \rival{\tejsym_0} \to X$}.
			\end{array}
		\right.
	\]
	This is easily checked to have an action on morphisms.
	Note that by $0$-antisymmetry of $X$, we can only get $R(X, \vfi) = \walkingcospanM$ if $\vfi$ is constant.
	...
	\[
		...
		\left\{
			\begin{array}{l c l l}
				() \mapsto \vfi(0) = \vfi(1) &:& (\top, \_) \to (X, \vfi)
				& \text{if $R(X, \vfi) = \walkingcospanM$,} \\
				\vfi &:& (\rival{\jetsym_0}, \id_{\accol{0, 1}}) \to (X, \vfi)
				& \text{if $R(X, \vfi) = \walkingcospanL$}, \\
				\vfi &:& (\rival{\tejsym_0}, \id_{\accol{0, 1}}) \to (X, \vfi)
				& \text{if $R(X, \vfi) = \walkingcospanR$}.
			\end{array}
		\right.
	\]
