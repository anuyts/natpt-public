...
\begin{proof}
	...
	In the case of $\rho_0$, this is only non-trivial if $a_i = \ypolar$.
	From \cref{thm:char-jet-cube-to-line} and the completeness of our calculus we know that the union of the dependencies of all $\dm i \psub \vfi$ for all directed variables $\dm i$ of $W_i$, will be a final segment of the $i$-directed variables of $V$.
	Hence, an initial segment of the $i$-directed variables in $V$, together with all symmetric variables of degree $i$ in $V$, will be either not used or used at a lower degree $j < i$, or used in $\dm i \psub \vfi$ for some variable $\dm i : \rival{\equijetsym_i\pmr{\ypolar}}$.
	In all of these cases, they will either be absent from $\bigsss_i V_i$ or have type $\rival{\equijetsym_i\pmr{\ypolar}}$.
	Thus we can form $\rho_0$ as the composite of two jet cube morphisms: the first one erases to the identity morphism and just promotes an initial segment of the $i$-directed variables to symmetric ones, while the second one subsequently discards some of the (now or already) symmetric variables.
	The latter is possible by the rule \ruleref{wkn}, which leaves the source cube unaffected under the described circumstances.
	
	%this is relatively trivial: we are promoting an initial segment of the variables of every degree to equijet variables of the same degree, and then projecting away some of these.
