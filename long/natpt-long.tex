...
\end{definition}
\begin{definition} \label{def:bool:prune}
	Let $d$ and $e$ be normal expressions. We say that $d$ is a \textbf{prune} of $e$ if any of the following conditions hold (inductively):
	\begin{itemize}
		\item $d$ is a literal occurring in $e$,
		\item $e$ has a subexpression $e'$ which has the same root label as $d$ ($\vee$ or $\wedge$) and such that every root operand of $d$ is a prune of a root operand in $e'$, and moreover a single operand of $e'$ cannot be used more often than its multiplicity in the multiset of root operands of $e'$ (taking into account the multiplicity of root operands of $d'$).
	\end{itemize}
\end{definition}
\begin{proposition} \label{thm:bool:assign-prune}
	...
	Let $e[\sigma]$ reduce to $d$. Then $d$ is either a constant or a prune of $e$.
\end{proposition}
\begin{proof}
...
\end{proof}
\begin{lemma} \label{thm:boolexp-to-prune-assignment}
...
\end{definition}
\begin{lemma} \label{thm:bool:prune-in-connection}
	Let $d, e \in \booleanexpr^\#(X)$ be normal affine expressions and $d$ a prune of $e$.
	Let $d$ (hence $e$) depend on $x, y \in X$.
	Then $x$ and $y$ are in $\opA$-connection in $d$ if and only if they are in $\opA$-connection in $e$.
\end{lemma}
\begin{proof}
	Take the closest common ancestor of $x$ and $y$ in $d$; call $d'$ the subtree rooted there.
	By definition of a prune, there is a corresponding node in $e$ with the same label; call $e'$ the subtree rooted there.
	The immediate subtrees of $d'$ mentioning $x$ and $y$ are prunes of different immediate subtrees of $e'$, so that the root node of $e'$ is also the closest common ancestor of $x$ and $y$ in $e$.
\end{proof}
...
\begin{proof}
	...
	Then $d$ is a prune of both $e$ and $e'$, so by \cref{thm:bool:prune-in-connection}, $x$ and $y$ are in $\opA$-connection in $e'$.
\end{proof}
...
\begin{proof}
	...
	This means that $e$ has a prune of the form $d = \tilde x \opA (\tilde y \opB \tilde z)$.
	But then we have
	\[
...
\begin{proof}
	...
	\begin{itemize}
		...
		\item $e$ is of the form $e = d_1 \opB d_2 \opB \ldots \opB d_m$. Then $X$ is $\opB$-connected meaning that $n = 1$, violating the assumptions.
	\end{itemize}
\end{proof}
\begin{theorem} \label{thm:bool:normal-from-graph}
...
\begin{proof}
	...
	Since $d_i$ is a prune of $e$ and $d_i'$ is a prune of $e'$, we have $\getconn{d_i} \loch \loch = \getconn{d_i'} \loch \loch$.
...
\begin{proof}
	...
	Let $n > 0$.
	We reduce $\dm i \psub \vfi$ to an expression $e$ by associative reduction (\cref{def:bool:reduction}).
	Write $\dm j_k'$ to mean $\dm j_k$ if $\dm j_k : \rival{\jet_i}$ and to mean $\lnot \dm j_k$ if $\dm j_k : \rival{\tej_i}$.
