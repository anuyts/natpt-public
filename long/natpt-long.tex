...
\begin{proof}
	...
	\begin{itemize}
		...
		Let the root node of $e$ be a conjunction (disjunction), and let $t$ be the child of $e$ mentioning $\dm j_n$.
		...
		Then this is only possible if any two $i$-directed variables in $t$ are also in conjunction (disjunction), so that the parentheses can be moved around.
		
		We know that if $\dm j'_n$ is not a direct child of $e$, then it must depend on other variables and the root node of $t$ is a disjunction (conjunction).
		If one of the immediate operands $s$ of $t$ depends on $\dm j_n$,
		then any other operand $r$ of $t$ can only depend on $i$-symmetric variables, as $\dm j_n$ is in conjunction (disjunction) with all $i$-directed variables in $t$.
		Then for every $r$ (and in particular for some $r$) there exists a bit assignment of its dependencies that makes $r$ reduce to $1$ ($0$), such that the entire disjunction (conjunction) $t$ reduces to $1$ ($0$).
