...
\begin{definition} \label{def:setoid}
	A \textbf{setoid} is a set equipped with an equivalence relation $\eqty$.
	A \textbf{setoid morphism} is a function between setoids respecting $\eqty$.
...
\begin{corollary} \label{thm:jetsetoid-equiv-jetset}
	The equivalence from \cref{thm:setoid-equiv-set} carries over to an equivalence of setoid-enriched categories $\jetsetoid{\vec a} \simeq \jetset{\vec a}$. \qed
\end{corollary}
\begin{definition} \label{def:poresh-jetsetoid} \label{def:poresh-jetset}
	\begin{enumerate}
		...
		\begin{itemize}
			...
			\begin{itemize}
				...
				%quotiented out by the transitive closure of $\eqty \cdot \mu$ in case $\eqty \cdot \mu \neq \eqty$,
				\item equivalence relation $\eqty^{\mu X}$ is given by transitive closure of $\eqty \cdot \mu$ on $X$,
				\item $i$-jet relation $\jetsym_i^{\mu X}$ is given by the relation $\jetsym_i \cdot \mu$ on $X$, made stable under $\eqty^{\mu X}$, i.e. we have
				$\jetsym_i^{\mu X} = (\eqty \cdot \mu)^* \otimes (\jetsym_i \cdot \mu) \otimes (\eqty \cdot \mu)^*$, where $\otimes$ is composition of relations via existential quantification.
	...
	%			This is in turn equivalent to saying that all representants of $\hat x$ and $\hat y$ are related by $(\eqty \cdot \mu)^* \otimes (\jetsym_i \cdot \mu) \otimes (\eqty \cdot \mu)^*$ in $X$, where $\otimes$ is composition of relations via existential quantification.
		...
		\item Additionally, if we consider only modalities $\mu$ which have a left adjoint and for which, by \cref{thm:poresh-adjoint}, $\eqty \cdot \mu = \eqty$, then we directly get a strict 2-functor $\jetsetsym \withl : \poreshcat \to \Cat$, equivalent to the pseudofunctor above:
		\begin{itemize}
			\item Sending an object $\vec a$ to $\jetset{\vec a}$,
...
\begin{corollary} \label{thm:poresh-jetsetoid:bind}
	...
	For an arbitrary $Q_j \in \orisymbol(\vec b)$, the relation $Q_j$ on $\mu X$ is given by $(\eqty \cdot \mu)^* \otimes (Q_j \cdot \mu) \otimes (\eqty \cdot \mu)^*$.
...
\end{proposition}
\begin{proof}
	Suppose $\jetsym_i \cdot \rho = \eqty$. Then each of the right hand sides in \cref{eq:poresh-jetsetoid-intervals} is the terminal jet set.
	...
	Then $0$ and $1$ are related by $\eqty \cdot \mu$ in $\rival{\jetsym_i}$, $\rival{\tejsym_i}$ and $\rival{\equijetsym_i}$ and therefore identified in the jet sets obtained by applying $\mu$ to the left hand sides of \cref{eq:poresh-jetsetoid-intervals}.
	
	Suppose $\jetsym_i \cdot \rho > \eqty$. Then the right hand sides of \cref{eq:poresh-jetsetoid-intervals} have carrier $\accol{0, 1}$.
	...
	Then $0$ and $1$ are \emph{not} related by $\eqty \cdot \mu$ in $\rival{\jetsym_i}$, $\rival{\tejsym_i}$ and $\rival{\equijetsym_i}$ and therefore \emph{not} identified in the jet sets obtained by applying $\mu$ to the left hand sides of \cref{eq:poresh-jetsetoid-intervals}.
