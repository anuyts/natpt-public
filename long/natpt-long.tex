...
For this reason, not unlike our usage of jet setoids in \cref{sec:poresh-jetset}, we will take a detour via setoid enriched categories that are equivalent to the categories of jet topic sets but where a single strict 2-functorial semantics of our mode theory can be achieved.

\subsection{Mask Bi-Enhancement} \label{sec:poresh-jtset:enhancement}
...
We will call this \textbf{mask bi-enhancement}.
To get a handle on equality, we will resort to setoid-valued presheaves.

We will then take the full subcategory of jet topic sets for the enhanced mask, on those presheaves where formal and actual infrajets (equijets) actually coincide (\cref{sec:poresh-jtset:strict}).
This category will be equivalent to the original category of jet topic sets for the original mask.

\todoi{Endofunctor on $\poreshcat$}

\subsection{Strict Semantics on Enhanced Bimodal Jet Topic Setoids} \label{sec:poresh-jtset:strict}
