...
\begin{proof}
	...
	By \cref{thm:poresh-jetsetoid-intervals}, $\mu \rival{P_i} = \rival{P_i \oriof \rho}$.
	Thus, we see that $\mu (X \multip \rival{P_i})$ is a colimit of $\mu X$, $\mu \Op_i X$ and $\rival{P_i \oriof \rho}$.
	We case distinguish on $P_i \oriof \rho$.
	\begin{description}
		\item[\framebox{$\eqtysym$}] If $P_i \oriof \rho = \eqtysym$, then from \cref{eq:poresh-adjoint-crit-orideg} in the statement of \cref{thm:poresh-adjoint}, we conclude that $P_i \leq \eqtysym \oriof \mu$.
		By symmetry of $\eqtysym \oriof \mu$, we can conclude that $\infrajetsym \oriof P_i = \infrajetsym_i \leq \eqtysym \oriof \mu$, so that $\mu$ cannot mention degree $i$ in any form other than $\infrajetsym_i$ and therefore $\mu = \mu \Op_i$. Then the colimit produces $(\mu X) * \rival{\eqty} = (\mu X) \times \rival{\eqty}$.
		
		\item[\framebox{$\equijetsym_j$}] If $P_i \oriof \rho = P \oriof (i \cdot \rho) = \equijetsym_j$, then by invertibility of $P \oriof \loch$, we can conclude that $i \cdot \rho = mixpol_j$, so that $\Op_i \circ \rho = \rho$.
		Then by uniqueness of the adjoint, $\mu \circ \Op_i = \mu$.
		Then the colimit produces $(\mu X) * \rival{\equijetsym_j}$.
