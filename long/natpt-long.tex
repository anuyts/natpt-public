...
\begin{definition} \label{def:polarity-cat}
	...
	\begin{itemize}
		...
		Composition is defined in \cref{fig:polarity-orientation} and is monotonically increasing in both arguments.
...
\subsubsection{Polarized Reshuffles and Oriented Degrees}
\begin{definition}
...
\begin{definition}
	...
	\end{itemize}
	We denote a dependent pair $(i, p)$ of a degree and a polarity as $p_i$ and write $\getdeg(p_i) = i$ and $\getpol(p_i) = p$.
	We write $p_i \ll q_j$ if \textbf{EITHER} $i < j$ \textbf{OR} $i = j$ and $p \ll q$.
\end{definition}
...
%			\item A function $\getpol(\mu) : (j \in \Degeqtop(\vec b)) \to \Hom_{\polsymbol}(a_{\getdeg(j \cdot \mu)}, b_j) : j \mapsto \getpol(j \cdot \mu)$,
