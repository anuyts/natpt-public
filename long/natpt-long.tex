...
\]
To fix this, we just throw in all truth tables that actually amount to a graph morphism between the corresponding hypercubes, which can alternatively be phrased as being non-expansive with respect to the $\ell^1$-distance.
Note that we need cartesianness already to simply use a boolean expression as general as $t$ in a cube morphism, i.e.\ in order to even build the preterm we are talking about.
...
\begin{proof}
	...
	\begin{itemize}
		...
		\end{itemize}
		Soundness of \ruleref{disj:bck}, \ruleref{disj:fwd} and \ruleref{conj:bck} is proven analogously, but with a different case analysis in the table.
		...
		\begin{itemize}
			...
			\item $i = j$, $\vec w = \vec w'$ and $\vec u \edge_i \vec u'$ in $\JEP(U)$. The latter implies that the $\ell^1$-distance $d^1(\vec u, \vec u') \leq 1$, hence $d^1(\vec t(\vec u), \vec t(\vec u')) \leq 1$, hence $\vec t(\vec u) \edge_i \vec t(\vec u')$, so that $(\vec w, \vec t(\vec u)) \edge_i (\vec w, \vec t(\vec u'))$. \qedhere
		\end{itemize}
	\end{itemize}
\end{proof}
